...
C   PRGMMR: D. A. KEYSER     ORG: NP22       DATE: 2005-01-03
C
...
C   TO SURFACE (LAND, MARINE AND MESONET) DATA TYPES [OTHER DATA
C   TYPES WITH REPORTED SENSIBLE TEMPERATURE EITHER HAVE MISSING
C   MOISTURE (E.G., ALL AIRCRAFT TYPES EXCEPT FOR SOME ACARS,
C   SATELLITE WIND TYPES), FLAGGED MOISTURE (E.G., SOME ACARS) OR
C   CALCULATE SPECIFIC HUMIDITY/VIRTUAL TEMPERATURE IN SUBSEQUENT
C   PROGRAMS (E.G., RADIOSONDES WHICH CALCULATE THESE IN PROGRAM
C   "CQCBUFR")]. FOR CASES WHERE THE SWITCH "DOBERR" IS FALSE, THE
...
C   CASES WHERE THE SWITCH "DOFCST" IS FALSE, THE FORECAST VALUES FOR
C   ALL DATA IN THOSE MESSAGE TYPES FOR WHICH A GUESS VALUE IS NOT
C   NEEDED BY SUBSEQUENT QC PROGRAMS REMAINS MISSING.  IT SHOULD BE
C   NOTED THAT THE FILTERING OF DATA ASSOCIATED WITH THE "PREVENT"
C   EVENT PROCESSING IS NOT INVOKED IF ALL THREE ARE TRUE: DOBERR=
C   FALSE, THE FORECAST VALUES ARE MISSING (DOFCST=FALSE & MESSAGE
C   TYPE IS NOT "ADPUPA", "AIRCFT", "AIRCAR", "PROFLR", OR "VADWND"),
C   AND "VIRTMP" EVENT PROCESSING IS NOT INVOKED (MESSAGE TYPE IS NOT
C   "ADPSFC", "SFCSHP" OR "MSONET").
C
...
CC
CC
C    DOVTMP  - WRITE VIRTUAL TEMPERATURE EVENT FOR SURFACE LAND,
C               MARINE, MESONET AND RASS REPORTS?  FOR THE FIRST THREE
C               TYPES, THIS WILL ATTEMPT TO CALCULATE VIRTUAL
C               TEMPERATURE FROM SENSIBLE TEMPERATURE AND ENCODE IT AS
C               A STACKED EVENT IN THE PREPBUFR FILE.  FOR RASS
C               REPORTS THIS WILL JUST ENCODE THE REPORTED TEMPERATURE
C               AS A STACKED EVENT IN THE PREPBUFR FILE SINCE THE
C               REPORTED TEMPERATURE IS ALREADY VIRTUAL.
C              DOVTMP = .TRUE.  ---> YES                       (DEFAULT)
C              DOVTMP = .FALSE. ---> NO 
C      (NOTE1: FOR SURFACE LAND, MARINE AND MESONET REPORTS,
C              DOVTMP=FALSE WILL STILL RE-CALCULATE SPECIFIC HUMIDITY
C              AND ENCODE IT AS A STACKED EVENT IN THE PREPBUFR FILE
C              UNLESS DOANLS IS TRUE.)
C      (NOTE2: ONLY APPLIES TO SURFACE LAND, MARINE, MESONET AND RASS
C              DATA TYPES.)
...
C----------------------------------------------------------------------
     ...
     $                    NEWTYP)
      ...
      COMMON /GBEVAR/ SID,OBS(12,255),BAK(12,255),XOB,YOB,DHR,TYP
      COMMON /GBEVAI/ NLEV
      COMMON /GBEVBB/ PVCD,VTCD
      COMMON /GBEVCL/ DOVTMP,DOFCST,DOBERR,FCST,VIRT,SATMQC
      COMMON /GBEVCR/ QTOP_REJ
...
C -------------------------------
  ...
  700 FORMAT(/1X,100('#')/' =====> SUBROUTINE GBLEVENTS INVOKED FOR ',
     $ 'THE FIRST TIME - VERSION LAST UPDATED 2005-01-03'/)
...
ccccc$           SUBSET.EQ.'RASSDA  ')
         VIRT = (SUBSET.EQ.'ADPSFC  '.OR.SUBSET.EQ.'SFCSHP  '.OR.
     $           SUBSET.EQ.'MSONET  '.OR.SUBSET.EQ.'RASSDA  ')
...
C***********************************************************************
     ...
     $             PW4V(4,255),OBS,BAK,SID
      LOGICAL      DN2FAR,REJPS,REJT,REJQ,REJW,REJPW,REJPW1,REJPW2,
     $             REJPW3,REJPW4,SATEMP,SOLN60,SOLS60
      LOGICAL      DOVTMP,DOFCST,DOBERR,FCST,VIRT,DOANLS,SATMQC
...
C-----------------------------------------------------------------------
      ...
      COMMON /GBEVAR/ SID,OBS(12,255),BAK(12,255),XOB,YOB,DHR,TYP
      COMMON /GBEVAI/ NLEV
      COMMON /GBEVFI/IMAX,JMAX,KMAX,kmaxs,IDVC
      COMMON /GBEVFR/DLAT,DLON,SL(100),SI(101)
      COMMON /GBEVGI/ MAXPRS
      COMMON /GBEVGR/ SITERP(1200),SLTERP(1200)
      COMMON /GBEVEE/PSG01,ZSG01,TG01(100),UG01(100),VG01(100),QG01(100)
...
C  --------------------------------
      ...
      IF(IDVC.EQ.1) THEN
         PSIG  = PSG01*SL(1)
      ELSE IF(IDVC.EQ.2) THEN
         PSIG=((PINT(1)**KAP1-PINT(2)**KAP1)/
     $        (KAP1*(PINT(1)-PINT(2))))**KAPR
      END IF
...
C  ---------------------------------------------------------
      ...
      DO K=1,KMAX
         IF(IDVC.EQ.1) THEN
            PMTMP(K)=SL(K)*1000.
         ELSE IF(IDVC.EQ.2) THEN
            PMTMP(K)=((PITMP(K)**KAP1-PITMP(K+1)**KAP1)/
     $               (KAP1*(PITMP(K)-PITMP(K+1))))**KAPR
         END IF
      END DO
...
C   PRGMMR: D.A. KEYSER      ORG: NP22       DATE: 2004-09-10
C
...
C   REASON CODE 0).  FOR RASS DATA, SPECIFIC HUMIDITY IS MISSING
C   HOWEVER IF THE NAMELIST SWITCH DOVTMP IS TRUE, A SIMPLE COPY OF THE
C   REPORTED (VIRTUAL) TEMPERATURE IS ENCODED AS A STACKED EVENT TO BE
C   LATER WRITTEN INTO THE PREPBUFR FILE (UNDER PROGRAM "VIRTMP",
C   REASON CODE 3).  THIS SUBROUTINE IS CURRENTLY ONLY CALLED FOR
C   SURFACE LAND ("ADPSFC"), MARINE ("SFCSHP"), MESONET ("MSONET") OR
C   RASS ("RASSDA") DATA TYPES AND ONLY IN THE PREVENTS MODE.  THIS
C   ROUTINE IS CALLED ONCE FOR EACH VALID REPORT IN THE PREPBUFR FILE.
C
C PROGRAM HISTORY LOG:
