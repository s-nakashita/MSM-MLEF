...
{ extern char *optarg;
  DMS  dms;
  DMST keyset,dltkey;
  char *outfile=NULL,dmsfile[MAX_DMS_FNAME],*dmsdb=NULL,msgstr[MAX_DMS_STRL];
  char key[MAXBUFFER],temp[MAXKEYL],argument[8],*buffer,*kptr=NULL,*ptr;
  FILE *dirf=NULL;
  int  keylen,keyuse,keynum,keyfind;
  int  i,j,c,errflg=0,out,no=0,stat=0,buffsize=MAXBUFFER;
  ...
  { while( fgets(temp,MAXKEYL,dirf) != NULL )
    { temp[strlen(temp)-1]='\0';
      ...
      { printf("Error: Too many key (>=%d) in dirf(%s)\n",MAXBUFFER/(keylen+1),dirf);
        fclose(dirf);
        dms_exit(EDMS_FAIL);
      }
    }
