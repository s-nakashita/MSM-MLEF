...
CMIC$ DO ALL AUTOSCOPE
        DO K=1,KM
          DO N=1,NO
            LO(N,K)=LO(N,K).AND.N11(N).GT.0.AND.N21(N).GT.0.AND.
     &              N12(N).GT.0.AND.N22(N).GT.0.AND.(IBI(K).EQ.0.OR.
     &              (LI(N11(N),K).AND.LI(N21(N),K).AND.
     &               LI(N12(N),K).AND.LI(N22(N),K)))
            IF(LO(N,K)) THEN
              U11=C11(N)*UI(N11(N),K)-S11(N)*VI(N11(N),K)
     ...
     &                       +W12(N)*V12+W22(N)*V22
            ...
            ELSE
              ...
              VO(N,K)=0.
              W=0.
              IF(N11(N).GT.0.AND.LI(N11(N),K)) THEN
                U11=C11(N)*UI(N11(N),K)-S11(N)*VI(N11(N),K)
                V11=S11(N)*UI(N11(N),K)+C11(N)*VI(N11(N),K)
                UO(N,K)=UO(N,K)+W11(N)/WC(N)*U11
                VO(N,K)=VO(N,K)+W11(N)/WC(N)*V11
                W=W+W11(N)/WC(N)
              ENDIF
              IF(N21(N).GT.0.AND.LI(N21(N),K)) THEN
                U21=C21(N)*UI(N21(N),K)-S21(N)*VI(N21(N),K)
                V21=S21(N)*UI(N21(N),K)+C21(N)*VI(N21(N),K)
                UO(N,K)=UO(N,K)+W21(N)/WC(N)*U21
                VO(N,K)=VO(N,K)+W21(N)/WC(N)*V21
                W=W+W21(N)/WC(N)
              ENDIF
              IF(N12(N).GT.0.AND.LI(N12(N),K)) THEN
                U12=C12(N)*UI(N12(N),K)-S12(N)*VI(N12(N),K)
                V12=S12(N)*UI(N12(N),K)+C12(N)*VI(N12(N),K)
                UO(N,K)=UO(N,K)+W12(N)/WC(N)*U12
                VO(N,K)=VO(N,K)+W12(N)/WC(N)*V12
                W=W+W12(N)/WC(N)
              ENDIF
              IF(N22(N).GT.0.AND.LI(N22(N),K)) THEN
                U22=C22(N)*UI(N22(N),K)-S22(N)*VI(N22(N),K)
                V22=S22(N)*UI(N22(N),K)+C22(N)*VI(N22(N),K)
                UO(N,K)=UO(N,K)+W22(N)/WC(N)*U22
                VO(N,K)=VO(N,K)+W22(N)/WC(N)*V22
                W=W+W22(N)/WC(N)
              ENDIF
              LO(N,K)=W.GE.PMP
              IF(LO(N,K)) THEN
