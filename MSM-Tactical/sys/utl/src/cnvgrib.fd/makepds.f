...
C
        If (ipdstmpl(24).eq.2.and.ipdstmpl(1).eq.19.and.
...
C
C   Special case for WAFS (Max Wind height(ICAO), Max U/V)
C
        if (ipdstmpl(5).eq.96.AND.((ipdstmpl(1).eq.2.or.
     &     ipdstmpl(1).eq.3).AND.(ipdstmpl(2).eq.3.or.
     &     ipdstmpl(2).eq.2)).AND.(ipdstmpl(10).eq.6)) then
           kpds(16) = 10
        end if
C
