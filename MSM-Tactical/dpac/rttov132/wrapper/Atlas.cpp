...
void Atlas::dropAtlas() {
    ...
    if (err != 0) throw std::runtime_error ("Error deallocating atlas");
...
void Atlas::fillEmisBrdf(double* emisBrdf, rttov::Rttov * rttov, const std::vector<int>& channels) {

    if (! this->isAtlasLoaded()) throw std::logic_error( "Error: atlas data not loaded");
    if (! rttov->isCoeffsLoaded()) throw std::logic_error( "Error: coefficients not loaded in instrument");
    if (! rttov->isProfileSet()) throw std::logic_error( "Error: profiles not set for instrument");
    ...
    if (channels.size() == 0) {
        ...
        for (int i = 0; i < nchannels; i++) {
            if (channels[i] < 1 || channels[i] > nchannels) throw std::runtime_error( "Error: channel index out of range" );
    ...
    if (err != 0) throw std::runtime_error ("Error in rttov_get_emisbrdf");
}
...
///@param[in]     channels   vector containing the list of channels to simulate (indices with respect to channel list loaded from coef file), optional
void Atlas::fillEmisBrdf(double* emisBrdf, rttov::RttovSafe * rttov, const std::vector<int>& channels) {
...
///@param[in]     channels   vector containing the list of channels to simulate (indices with respect to channel list loaded from coef file), optional
void Atlas::fillEmisBrdf(double* emisBrdf, rttov::RttovScatt * rttov, const std::vector<int>& channels) {
...
///@param[in]     channels   vector containing the list of channels to simulate (indices with respect to channel list loaded from coef file), optional
void Atlas::fillEmisBrdf(double* emisBrdf, rttov::RttovScattSafe * rttov, const std::vector<int>& channels) {
