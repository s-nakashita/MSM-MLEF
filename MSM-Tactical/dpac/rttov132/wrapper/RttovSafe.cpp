...
const std::string& RttovSafe::getFileCoef() const {
...
void RttovSafe::setFileCoef(const std::string& fileCoef) {
    Rttov::setFileCoef(fileCoef);
}

///@brief Return the cloud coefficient filename
const std::string& RttovSafe::getFileSccld() const {
...
void RttovSafe::setFileSccld(const std::string& fileSccld) {
    Rttov::setFileSccld(fileSccld);
}

///@brief Return the aerosol coefficient filename
const std::string& RttovSafe::getFileScaer() const {
...
void RttovSafe::setFileScaer(const std::string& fileScaer) {
    Rttov::setFileScaer(fileScaer);
}

///@brief Return the MFASIS cloud LUT filename
const std::string& RttovSafe::getFileMfasisCld() const {
...
void RttovSafe::setFileMfasisCld(const std::string& fileMfasisCld) {
    Rttov::setFileMfasisCld(fileMfasisCld);
}

///@brief Return the MFASIS NN coef filename
const std::string& RttovSafe::getFileMfasisNN() const {
    return Rttov::getFileMfasisNN();
}

///@brief Set the MFASIS NN coef filename
///@param fileMfasisNN string containing full path to MFASIS NN coef file
void RttovSafe::setFileMfasisNN(const std::string& fileMfasisNN) {
    Rttov::setFileMfasisNN(fileMfasisNN);
}
...
void RttovSafe::loadInst(const std::vector<int>& channels) {
...
//     if (this->options.isVerboseWrapper()) std::cerr << "Number of profiles : " << nbProfiles << std::endl;
    if (nbProfiles == 0) throw std::logic_error("Error: no Profile instances in the vector of profiles");
...
//     if (this->options.isVerboseWrapper()) std::cerr << "Number of levels : " << nbLevels << std::endl;
    ...
    for (int i=1; i< nbProfiles;i++) {
        ...
        if (profiles[i].getItemContents() != Profilecontents )
            throw std::logic_error("Error: Profiles must all have the same gas/cloud/aerosol data defined");
        if ( ! profiles[i].check() )
        ...
        if (  profiles[i].isMmrCldAer() != mmr_cldaer )
            throw std::logic_error("Error: Profiles must all have the same mmr_cldaer");
    }
...
void RttovSafe::runDirect(const std::vector<int>& channels) {
...
///@param channels vector containing the list of channels to simulate
void RttovSafe::runK(const std::vector<int>& channels) {
    Rttov::runK(channels);
}
...
///@param profile index of the profile (0..nprofiles-1)
std::vector<double> RttovSafe::getTskinEffK(int profile) {
    return Rttov::getTskinEffK(profile);
}
