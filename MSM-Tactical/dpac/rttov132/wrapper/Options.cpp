...
Options::Options(): nprofs_per_call(1),
                    ...
                    pol_mode(empirical_pol){
    // General configuration options
    this->config["apply_reg_limits"]=false;
    this->config["opdep13_gas_clip"]=true;
    ...
    this->rt_all["use_tskin_eff"]=false;
    ...
    this->rt_all["transmittances_only"]=false;
    // MW-only RT options
    this->mw["clw_data"]=false;
    this->mw["fastem3_rwd_fix"]=true;
...
///@brief Return the opts\%config\%opdep13_gas_clip and opts_scatt\%config\%opdep13_gas_clip options
bool Options::isOpdep13GasClip() {
    return this->config["opdep13_gas_clip"] ;
}

///@brief Set the opts\%config\%opdep13_gas_clip and opts_scatt\%config\%opdep13_gas_clip options
void Options::setOpdep13GasClip(bool opdep13GasClip) {
    this->config["opdep13_gas_clip"] = opdep13GasClip;
}
...
///@brief Return the opts\%rt_all\%use_tskin_eff and opts_scatt\%use_tskin_eff options
bool Options::isUseTskinEff() {
    return this->rt_all["use_tskin_eff"];
}

///@brief Set the opts\%rt_all\%use_tskin_eff and opts_scatt\%use_tskin_eff options
void Options::setUseTskinEff(bool useTskinEff) {
    this->rt_all["use_tskin_eff"] = useTskinEff;
}
...
///@brief Return the opts\%rt_all\%transmittances_only option
bool Options::isTransmittancesOnly() {
    return this->rt_all["transmittances_only"];
}

///@brief Set the opts\%rt_all\%transmittances_only option
void Options::setTransmittancesOnly(bool transmittancesOnly) {
    this->rt_all["transmittances_only"] = transmittancesOnly;
}


// RTTOV opts%rt_mw%

///@brief Return the opts\%rt_mw\%fastem3_rwd_fix and opts_scatt\%fastem3_rwd_fix options
bool Options::isFastem3RwdFix() {
    return this->mw["fastem3_rwd_fix"];
}

///@brief Set the opts\%rt_mw\%fastem3_rwd_fix and opts_scatt\%fastem3_rwd_fix options
void Options::setFastem3RwdFix(bool fastem3RwdFix) {
    this->mw["fastem3_rwd_fix"] = fastem3RwdFix;
}
...
///@brief Return the opts\%rt_ir\%rayleigh_depol option
bool Options::isRayleighDepol() {
    return this->ir["rayleigh_depol"];
}

///@brief Set the opts\%rt_ir\%rayleigh_depol option
void Options::setRayleighDepol(bool rayleighDepol) {
    this->ir["rayleigh_depol"] = rayleighDepol;
}
...
///@brief Return the opts_scatt\%pol_mode option
int Options::getPolMode() const {
    return pol_mode;
}

///@brief Set the opts_scatt\%pol_mode option
void Options::setPolMode(int polMode) {
    pol_mode = polMode;
}
...
std::string Options::defineStrOptions() {
    ...
    for(auto config_item : this->config)
    {
        std::string strval;
        key = config_item.first;
        val = this->config[key];
    ...
    for(auto config_item : this->interp)
    {
        std::string strval;
        key = config_item.first;
        val = this->interp[key];
    ...
    for(auto config_item : this->rt_all)
    {
        std::string strval;
        key = config_item.first;
        val = this->rt_all[key];
    ...
    for(auto config_item : this->scatt)
    {
        std::string strval;
        key = config_item.first;
        val = this->scatt[key];
    ...
    oss << " opts_scatt%pol_mode "<< this->pol_mode;
