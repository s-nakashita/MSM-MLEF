...
Rttov::~Rttov() {
    ...
    if ( inst_id > 0 ) {
        ...
        inst_id = -1;
        if ( err!=0) {
            throw std::runtime_error("Error in rttov_drop_inst");
        }
    }
}
...
void Rttov::setFileMfasisCld(const std::string& fileMfasisCld) {
    file_mfasis_cld = fileMfasisCld;
}

///@brief Return the MFASIS NN coef filename
const std::string& Rttov::getFileMfasisNN() const {
    return file_mfasis_nn;
}

///@brief Set the MFASIS NN coef filename
///@param fileMfasisNN string containing full path to MFASIS NN coef file
void Rttov::setFileMfasisNN(const std::string& fileMfasisNN) {
    file_mfasis_nn = fileMfasisNN;
}
...
void Rttov::loadInst(const std::vector<int>& channels) {

    bool flagClouds=this->options.isAddClouds();
    bool flagAer=this->options.isAddAerosl();
    bool flagPC=this->options.isAddPC();
    std::string filesPaths;
    if (file_coef.empty()) {
        throw std::logic_error("Error: " + file_coef + " not set");
    }
    ...
    {
        throw std::logic_error("Error: " + file_coef + " does not exist or is not readable");
    }
    ...
    if (! file_sccld.empty() ) {
        if ( is_readable(file_sccld))  {
            ...
            throw std::logic_error("Error: " + file_sccld + " does not exist or is not readable");
        }
    }
    if (! file_scaer.empty() ) {
        if ( is_readable(file_scaer))  {
            ...
            throw std::logic_error("Error: " + file_scaer + " does not exist or is not readable");
        }
    }
    if (! file_mfasis_cld.empty() ) {
        if ( is_readable(file_mfasis_cld))  {
            ...
            throw std::logic_error("Error: " + file_mfasis_cld + " does not exist or is not readable");
        }
    }
    if (! file_mfasis_nn.empty() ) {
        if ( is_readable(file_mfasis_nn))  {
            filesPaths.append(" file_mfasis_nn ");
            filesPaths.append(file_mfasis_nn + " ");
        }
        else {
            throw std::logic_error("Error: " + file_mfasis_nn + " does not exist or is not readable");
        }
    }
    if (! file_pccoef.empty() ) {
        if ( is_readable(file_pccoef))  {
            ...
            throw std::logic_error("Error: " + file_pccoef + " does not exist or is not readable");
        }
    }

    this->coeffsLoaded = false;

    std::string optionsAndPaths;
    ...
    if (inst_id > 0) {
        ...
        if ( err != 0 ) throw std::runtime_error("Error getting number of channels");
        this->coeffsLoaded=true;
        this->options.setAddClouds(flagClouds);
        this->options.setAddAerosl(flagAer);
    }
    else {
        throw std::runtime_error("Error in rttov_load_inst");
    }
...
double* Rttov::getRefPressures() {
    if ( this->isCoeffsLoaded() ){
        ...
        if ( err != 0 ) throw std::runtime_error("Error getting pressures from coefficient");
        return refPressures;
    }
    else {
        return NULL;
...
int Rttov::getCoeffsNlevels() {
    if ( this->isCoeffsLoaded() ){
        ...
        if ( err != 0 ) throw std::runtime_error("Error getting number of levels");
        return coeffNlevels;
    }
    else {
        return 0;
...
double* Rttov::getWaveNumbers() {
    if ( this->isCoeffsLoaded() ){
        ...
        if ( err != 0 ) throw std::runtime_error("Error getting channel central wave numbers");
        return wavenumbers;
    }
    else {
        return NULL;
...
void Rttov::updateOptions() {
    if (! this-> coeffsLoaded  ) {
        throw std::logic_error( "Error: coefficients not loaded");
    }
    ...
    if ( err != 0 ) {
        throw std::runtime_error("Error in rttov_set_options");
    }
}
...
void Rttov::printOptions() {
    ...
    if ( err != 0 ) {
        throw std::runtime_error("Error in rttov_print_options");
    }
}


///@brief Set pointer to array containing input/output surface emissivity and reflectance values;
///this must be previously allocated a double array of dimensions [5][nprofiles][nchannels]; this
...
///@param surfemisrefl pointer to a previously allocated double array [5][nprofiles][nchannels]
...
void  Rttov::setProfiles(rttov::Profiles * profiles) {
    ...
    if (this->profiles->isDefaultPressureLevels())
        if ( this->coeffsLoaded ) {
            ...
            else throw std::logic_error("Error: number of levels differs to coefficient file, cannot use coefficient file pressure levels");
        }
        else throw std::logic_error("Error: instrument not loaded, cannot use coefficient file pressure levels");


    bool profileOK=this->profiles->check();
    if (! profileOK )  {
        this->profileSet=false;
        throw std::logic_error( "Error: some mandatory profile fields are missing");
    }
...
void Rttov::printGases() {
    ...
    if (this->surfemisrefl != NULL) {
        for  (int prof=0; prof < nprofiles; prof++){
            std::cout << "surfemisrefl prof " << prof << " " << this->surfemisrefl[0] << " " << this->surfemisrefl[1] << " " << this->surfemisrefl[2] << " " << this->surfemisrefl[3] << " " << this->surfemisrefl[4] << std::endl;
        }
    }
}
...
void Rttov::setOptParams(int nchannels) {
    ...
    if (this->aer_asb == NULL && doaer)
        throw std::logic_error("Absorption, scattering and bpr coefficients must be defined");
    if (this->aer_phangle == NULL && doaer && this->options.isAddSolar())
        throw std::logic_error("Phase function angles must be defined for solar simulations");
    if (this->aer_pha == NULL && doaer && this->options.isAddSolar())
        throw std::logic_error("Phase functions must be defined for solar simulations");
    if (this->aer_legcoef == NULL && doaer && (this->options.getIrScattModel() == dom_ir ||
        ...
        throw std::logic_error("Legendre coefficients must be defined for DOM simulations");
...
void Rttov::runDirect(const std::vector<int>& channels) {
    if (! this-> coeffsLoaded  ) {
        throw std::logic_error( "Error: coefficients not loaded");
    }
    if (! this->profileSet) {
        throw std::logic_error( "Error: profiles not set");
    }
    ...
    if (this->surfemisrefl == NULL) {
        ...
        this->surfemisrefl = new double [5*nprofiles*nchannels]();
        for (int i = 0; i < 5*nprofiles*nchannels; i++) this->surfemisrefl[i]=-1.;
    }
    ...
    if (do_opt_param) {
        rttov_visir_scatt_call_direct_(
                ...
                surfemisrefl,      // input/output surface emis, refl, specularity, tskin eff [5][nprofiles][nchannels]
        ...
        if ( err != 0 ) throw std::runtime_error ("Error in rttov_visir_scatt_call_direct");
    } else {
        rttov_call_direct_(
                &err,
                &inst_id,
                ...
                rads,              // output radiances                                        [nprofiles][nchannels]
                &nchannels, &ngases, &nlevels, &nprofiles);
        if ( err != 0 ) throw std::runtime_error ("Error in rttov_call_direct");
    }
...
///@param channels vector containing the list of channels to simulate
void Rttov::runK(const std::vector<int>& channels) {
    int err=0;
    ...
    if (surfemisrefl == NULL) {
        if (this->options.isVerboseWrapper())
            std::cerr << "No surface emissivity/reflectance supplied: setting calcemis and calcrefl to true" << std::endl;
        surfemisrefl = new double [5*nprofiles*nchannels]();
        for (int i=0; i < 5*nprofiles*nchannels; i++) surfemisrefl[i]=-1;
    ...
    surfemisrefl_k = new double [5*nprofiles*nchannels]();
    for (int i=0; i < 5*nprofiles*nchannels; i++) surfemisrefl_k[i]=0;
    ...
    if (do_opt_param) {
        rttov_visir_scatt_call_k_(
                ...
                cld_pha,           // cloud phase fns                                         [nprofiles,nchannels,nlayers,cld_nphangle]
                surfemisrefl,      // input/output surface emis, refl, specularity, tskin eff [5][nprofiles][nchannels]
                surfemisrefl_k,    // surface emis, refl, specularity, tskin eff K            [5][nprofiles][nchannels]
                btrefl,            // output BTs/refls (for thermal/solar chans)              [nprofiles][nchannels]
                rads,              // output radiances                                        [nprofiles][nchannels]
                ...
                &nchannels, &ngases, &nlevels, &nprofiles,
                &aer_nphangle, &aer_nmom, &cld_nphangle, &cld_nmom);
        if ( err != 0 ) throw std::runtime_error ("Error in rttov_visir_scatt_call_k");
    } else {
        rttov_call_k_(
                &err,
                &inst_id,
                ...
                rad_k,             // input radiance perturbations                            [nprofiles][nchannels]
                &nchannels, &ngases, &nlevels, &nprofiles);
        if ( err != 0 ) throw std::runtime_error ("Error in rttov_call_k");
    }
    this->nchannelsForLastRun=nchannels;
...
void Rttov::setAerPha(int nphangle, double* phangle, double* pha) {
    if (nphangle > 1) {
        ...
        throw std::logic_error("Error: nphangle must be greater than 1");
    }
}
...
void Rttov::setAerLegcoef(int nmom, double* legcoef) {
    if (nmom > 0) {
        ...
        throw std::logic_error("Error: nmom must be greater than 0");
    }
}
...
double Rttov::calcBpr(int nphangle, double* phangle, double* pha) {
    ...
    if ( err != 0 ) throw std::runtime_error("Error calculating bpr");
    return bpr;
}
...
void Rttov::calcLegcoef(int nphangle, double* phangle, double* pha, int nmom, double* legcoef, int ngauss) {
    ...
    if ( err != 0 ) throw std::runtime_error("Error calculating Legendre coefficients");
}


///@brief Return a pointer to an array of dimensions [5][nprofiles][nchannels]
...
std::vector<double> Rttov::convertPointer4D2Vector(double ptr[], int x, int y,
    ...
    if (ptr == NULL) throw std::logic_error("Error: data not present");
    if (x < 0 || x >= dim1) throw std::logic_error("Error: wrong call to convertPointer4D2Vector");
    if (y < 0 || y >= dim2) throw std::logic_error("Error: wrong call to convertPointer4D2Vector");
    if (z < 0 || z >= dim3) throw std::logic_error("Error: wrong call to convertPointer4D2Vector");
    if (ptr == NULL) throw std::logic_error("Error: wrong call to convertPointer4D2Vector");
...
std::vector<double> Rttov::convertPointer3D2Vector(double ptr[], const int x,
    ...
    std::vector <double>myvector{};
    if (ptr == NULL) throw std::logic_error("Error: data not present");
    if (x < 0 || x >= dim1) throw std::logic_error("Error: wrong call to convertPointer3D2Vector");
    if (y < 0 || y >= dim2) throw std::logic_error("Error: wrong call to convertPointer3D2Vector");
    if (ptr == NULL) throw std::logic_error("Error: wrong call to convertPointer3D2Vector");
...
std::vector<double> Rttov::convertPointer2D2Vector(double ptr[], int x, int dim1, int dim2) {
    if (ptr == NULL) throw std::logic_error("Error: data not present");
    if (x < 0 || x >= dim1) throw std::logic_error("Error: wrong call to convertPointer2D2Vector");
    if (ptr == NULL) throw std::logic_error("Error: wrong call to convertPointer2D2Vector");
    std::vector <double>myvector{};
...
std::vector<int> Rttov::convertIntPointer2D2Vector(int ptr[], int x, int dim1, int dim2) {
    if (ptr == NULL) throw std::logic_error("Error: data not present");
    if (x < 0 || x >= dim1) throw std::logic_error("Error: wrong call to convertIntPointer2D2Vector");
    if (ptr == NULL) throw std::logic_error("Error: wrong call to convertIntPointer2D2Vector");
...
///@brief Return computed surface Tskin eff Jacobians for a given profile
///@param profile index of the profile (0..nprofiles-1)
std::vector<double> Rttov::getTskinEffK(int profile) {
    return this->convertPointer3D2Vector(this->surfemisrefl_k,
            4,
            profile,
            5,
...
void Rttov::doStoreTrans() {
    ...
    if ( err != 0 ) throw std::runtime_error("Error getting tausunlevelspath1");

    delete [] tausuntotalpath2;
    tausuntotalpath2 = new double [nchanprof]();
    rttov_get_tausun_total_path2_(&err, &inst_id, tausuntotalpath2, &nchanprof);
    if ( err != 0 ) throw std::runtime_error("Error getting tausuntotalpath2");
