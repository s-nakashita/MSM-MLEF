...
int main(int argc, char **argv) {
    ...
    char * opts = " opts%interpolation%addinterp 1 opts%rt_all%co2_data 1 opts%rt_ir%addclouds 1 store_rad 1 verbose_wrapper 1 ";

    // Specify instrument and channel list and add coefficient files to the options string
    char * paths[2] = {" file_coef ../rtcoef_rttov13/rttov13pred54L/rtcoef_metop_1_hirs_o3co2.dat ",
                       " file_sccld ../rtcoef_rttov13/cldaer_visir/sccldcoef_metop_1_hirs.dat "};
    ...
    if (atlas_wrap_id < 1) fprintf(stderr, "Error initialising IR emissivity atlas: atlas will not be used\n");
    // =================================================================
    ...
    double surfemisrefl[5][nprofiles][nchannels];
    ...
    if (atlas_wrap_id > 0) {
        ...
        if (err != 0) {
            fprintf(stderr, "Error getting emissivities from atlas: atlas will not be used\n");
            for (int i = 0; i < nprofiles; i++) {
                for (int c = 0; c < nchannels; c++) {
                    surfemisrefl[0][i][c] = -1.;
                    surfemisrefl[1][i][c] = -1.;
    ...
    rttov_call_direct_(
        ...
        surfemisrefl,      // input/output surface emissivities/BRDFs                 [5][nprofiles][nchannels]
