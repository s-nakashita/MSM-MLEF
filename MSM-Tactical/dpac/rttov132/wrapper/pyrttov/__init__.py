...
class Rttov(_RttovCommons):
    ...
    FileMfasisNN = _FPathDescRWD('coefs_conf', 'file_mfasis_nn',
                                  doc="Path to the MFASIS NN coef-file.")
    ...
    def _pre_loadInst(self, channels):
        ...
        if self.FileMfasisNN is not None:
            OptStack.extend(['file_mfasis_nn', self.FileMfasisNN])
    ...
    def _setSurfEmisRefl(self, a):
        if (isinstance(a, np.ndarray) and a.shape[0] == 5 and
                len(a.shape) == 3):
    ...
    (direct BRDF and diffuse reflectance), specularity, and Tskin eff values.

    This must be a ``numpy.ndarray`` of shape [5, nprofiles, nchannels], the
    ...
    def printSurfEmisRefl(self, pfunction=print):
        ...
        fmt_e = 'Channel #{:5d} e={:6.3f} brdf={:6.3f} diff={:6.3f} ' + \
                'spec={:6.3f} tskin eff={:6.3f}'
        try:
    ...
    def _prerun_commons(self, channels):
        ...
        surfemisrefl_shape = (5, self.Profiles.Nprofiles, len(channels))
    ...
    def runDirect(self, channels=None):
        ...
        if do_opt_param:
            ...
            err = rtwrap.rttov_visir_scatt_call_direct(
                ...
                self._surfemisrefl.transpose(),         # input/output surface emis, refl, spec, tskin eff        [5][nprofiles][nchannels]
    ...
    def runK(self, channels=None):
        ...
        if do_opt_param:
            ...
            err = rtwrap.rttov_visir_scatt_call_k(
                ...
                self._surfemisrefl.transpose(),             # input/output surface emis, refl, spec, tskin eff        [5][nprofiles][nchannels]
                self._bBasics['surfemisrefl_k'].transpose(),# output surface emis, refl, spec, tskin eff K            [5][nprofiles][nchannels]
                self._bBasics['btrefl'].transpose(),        # output BTs/refls (for thermal/solar chans)              [nprofiles][nchannels]
                self._bBasics['rads'].transpose(),          # output radiances                                        [nprofiles][nchannels]
    ...
    @property_ro
    def TskinEffK(self):
        """Computed surface Tskin eff Jacobians from the previous run. ``numpy.ndarray`` of shape [nprofiles, nchannels]."""
        res = self._bBasics.get('surfemisrefl_k', None)
        if res is not None:
            res = res[4, ...]
        return res
...
class RttovScatt(_RttovCommons):
    ...
    FilePol = _FPathDescRWD('coefs_conf', 'file_pol',
                            doc="Path to the RTTOV-SCATT pol file.")
    ...
    def _pre_loadInst(self, channels):
        ...
        if self.FilePol is not None:
            OptStack.extend(['file_pol', self.FilePol])
    ...
    def _setSurfEmis(self, a):
        if (isinstance(a, np.ndarray)  and a.shape[0] == 2 and len(a.shape) == 3):
            if a.dtype is not np.dtype(wrapfloat) or np.isfortran(a):
                a = np.array(a, order='C', dtype=wrapfloat)
    ...
    dstring = """Array containing input/output surface emissivity and
    Tskin eff values.
    ...
    def printSurfEmis(self, pfunction=print):
        ...
        fmt_e = 'Channel #{:5d} e={:6.3f} tskin eff={:6.3f}'
        try:
            emis = self.SurfEmis
        except RttovInternalError:
            emis = None
    ...
    def _prerun_commons(self, channels):
        ...
        # deal with SurfEmis
        surfemis_shape = (2, self.Profiles.Nprofiles, len(channels))
    ...
    def runDirect(self, channels=None):
        ...
        err = rtwrap.rttov_scatt_call_direct(
            ...
            self._surfemis.transpose(),             # input/output surface emissivities, tskin eff            [2][nprofiles][nchannels]
    ...
    def runK(self, channels=None):
        ...
        err = rtwrap.rttov_scatt_call_k(
            ...
            self.CalcZef,                               # calc_zef flag
            self._surfemis.transpose(),                 # input/output surface emissivities, tskin eff            [2][nprofiles][nchannels]
            self._bBasics['surfemis_k'].transpose(),    # input/output surface emissivities, tskin eff K          [2][nprofiles][nchannels]
            self._bBasics['bt'].transpose(),            # output BTs                                              [nprofiles][nchannels]
    ...
    def SurfEmisK(self):
        """Computed surface emissivity Jacobians from the previous run. ``numpy.ndarray`` of shape [nprofiles, nchannels]."""
        res = self._bBasics.get('surfemis_k', None)
        if res is not None:
            res = res[0, ...]
        return res
