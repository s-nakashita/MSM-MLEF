...
const std::string& RttovScatt::getFileCoef() const {
    return file_coef;
}

///@brief Set the coefficient filename
///@param fileCoef string containing full path to "rtcoef" coefficient file
void RttovScatt::setFileCoef(const std::string& fileCoef) {
    file_coef = fileCoef;
}

///@brief Return the hydrotable filename
const std::string& RttovScatt::getFileHydrotable() const {
...
void RttovScatt::setFileHydrotable(const std::string& fileHydrotable) {
    file_hydrotable = fileHydrotable;
}

///@brief Return the pol filename
const std::string& RttovScatt::getFilePol() const {
    return file_pol;
}

///@brief Set the pol filename
///@param filePol string containing full path to pol file
void RttovScatt::setFilePol(const std::string& filePol) {
    file_pol = filePol;
}
...
void RttovScatt::loadInst() {
    ...
    if (file_coef.empty()) {
        throw std::logic_error("Error: call setFileCoef() method first");
    }
    ...
    if (file_hydrotable.empty()) {
        throw std::logic_error("Error: call setFileHydrotable() method first");
    }
    if (is_readable(file_hydrotable))  {
        filesPaths.append(" file_hydrotable ");
        filesPaths.append(file_hydrotable + " ");
    } else {
        throw std::logic_error("Error: " + file_hydrotable + " does not exist or is not readable");
    }

    if (is_readable(file_pol))  {
        filesPaths.append(" file_pol ");
        filesPaths.append(file_pol + " ");
    }
...
///@brief Set pointer to array containing input/output surface emissivity and Tskin eff values;
...
///@param surfemis pointer to a previously allocated double array [2][nprofiles][nchannels]
...
void RttovScatt::printGases() {
    ...
    if (this->surfemis != NULL) {
        for  (int prof=0; prof < nprofiles; prof++){
            std::cout << "surfemis prof " << prof << " " << this->surfemis[0] << " " << this->surfemis[1] << " " << std::endl;
        }
    }
}
...
void RttovScatt::runDirect(const std::vector<int>& channels) {
    if (! this-> coeffsLoaded  ) {
        throw std::logic_error( "Error: coefficients not loaded");
    }
    if (! this->profileSet) {
    ...
    if (this->surfemis == NULL) {
        ...
        this->surfemis = new double [2*nprofiles*nchannels]();
        for (int i = 0; i < 2 * nprofiles * nchannels; i++) this->surfemis[i] = -1.;
    }
    else if (allocatedSurfemis) {
        // Reallocate in case nprofiles or nchannels changed since the previous call
        delete [] this->surfemis;
        surfemis = new double [2*nprofiles*nchannels]();
        for (int i = 0; i < 2 * nprofiles * nchannels; i++) surfemis[i] = -1;
    }
    ...
    rttov_scatt_call_direct_(
            ...
            surfemis,          // input/output surface emissivities, tskin eff            [2][nprofiles][nchannels]
            bt,                // output BTs                                              [nprofiles][nchannels]
            &nchannels, &ngases, &nlevels, &nprofiles);
    if ( err != 0 ) throw std::runtime_error ("Error in rttov_scatt_call_direct");
...
///@param channels vector containing the list of channels to simulate
void RttovScatt::runK(const std::vector<int>& channels) {
    int err=0;
    ...
    surfemis_k = new double [2*nprofiles*nchannels]();
    for (int i = 0; i < 2 * nprofiles * nchannels; i++) surfemis_k[i]=0;
    ...
    rttov_scatt_call_k_(
            ...
            &calc_zef,         // enable/disable radar reflectivity calculations
            surfemis,          // input/output surface emissivities, tskin eff            [2][nprofiles][nchannels]
            surfemis_k,        // input/output surface emissivities, tskin eff K          [2][nprofiles][nchannels]
            bt,                // output BTs                                              [nprofiles][nchannels]
            bt_k,              // input BT perturbations                                  [nprofiles][nchannels]
            zef_k,             // input radar reflectivity perturbations                  [nprofiles][nchannels][nlevels]
            &nchannels, &ngases, &nlevels, &nprofiles);

    if ( err != 0 ) throw std::runtime_error ("Error in rttov_scatt_call_k");
    this->nchannelsForLastRun=nchannels;
...
/// containing output values of surface emissivity and Tskin eff;
...
std::vector<double> RttovScatt::getSurfEmisK(int profile) {
    return this->convertPointer3D2Vector(this->surfemis_k,
            0,
            profile,
            2,
            this->nprofiles,
...
///@brief Return computed Tskin eff Jacobians for a given profile
///@param profile index of the profile (0..nprofiles-1)
std::vector<double> RttovScatt::getTskinEffK(int profile) {
    return this->convertPointer3D2Vector(this->surfemis_k,
            1,
            profile,
            2,
...
void RttovScatt::doStoreZef() {
    ...
    if ( err != 0 ) throw std::runtime_error("Error getting zef");

    delete [] azef;
    azef = new double [nchanprof*nlevels]();
    rttov_get_azef_(&err, &inst_id, azef, &nchanprof, &nlevels);
    if ( err != 0 ) throw std::runtime_error("Error getting azef");
}
