#!/bin/sh
# Script to run the example_cld_mfasis_nn_fwd example program
#
...
CLD_COEF_FILENAME="sccldcoef_msg_4_seviri.dat"            # Location of this file is set below in $SCCOEF_DIR
MFASIS_COEF_FILENAME="rttov_mfasis_nn_msg_4_seviri.dat"   # Location of this file is set below in $MFASIS_DIR
PROF_FILENAME="prof_mfasis.dat"              # Input profile(s), usually found in $TEST_DIR set below
...
MFASIS_DIR=../../rtcoef_rttov13/mfasis_nn    # MFASIS coefficients directory
...
echo " "
echo " "
echo " Test forward with MFASIS-NN cloud simulation "
echo " "

echo  "Coef filename:           ${COEF_FILENAME}"
echo  "Cloud coef filename:     ${CLD_COEF_FILENAME}"
echo  "MFASIS-NN coef filename: ${MFASIS_COEF_FILENAME}"
...
CLD_COEF_FILENAME=$SCCOEF_DIR/$CLD_COEF_FILENAME
if [ ! -f $CLD_COEF_FILENAME ]; then
  echo "Coef file $CLD_COEF_FILENAME not found, aborting..."
  exit 1
fi

MFASIS_COEF_FILENAME=$MFASIS_DIR/$MFASIS_COEF_FILENAME
if [ ! -f $MFASIS_COEF_FILENAME ]; then
  echo "Coef file $MFASIS_COEF_FILENAME not found, aborting..."
  exit 1
fi

$BIN_DIR/example_cld_mfasis_nn_fwd.exe << EOF
"${COEF_FILENAME}"       , Coefficient filename
"${CLD_COEF_FILENAME}"   , Cloud coefficient filename
"${MFASIS_COEF_FILENAME}", MFASIS coef filename
...
if [ $CHECK_REF -ne 0 ]; then
  ...
  OUT_FILE=output_example_cld_mfasis_nn_fwd.dat
  DIFF_FILE=diff_example_cld_mfasis_nn_fwd.${ARCH}
