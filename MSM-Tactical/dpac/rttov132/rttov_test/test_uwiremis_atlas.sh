#!/bin/sh
#
# UW IR emissivity atlas test control script
#
# This test requires the UW IR emissivity atlas files in the directory $DATAATLAS
...
DATAATLAS=../../emis_data
EXEC=../../$BIN/rttov_uwiremis_atlas_test.exe
...
echo " "
echo " "
echo " Test UW IR emissivity atlas "
echo " "
...
EOF
  ...
  mv output_uwiremis_atlas.ascii  output_uwiremis_atlas.$TEST_NUMBER.$ARCH
  ...
  echo
  echo "Output is in the file ${TEST}/output_uwiremis_atlas.$TEST_NUMBER.$ARCH"

  DIFF_FILE=diff_uwiremis_atlas.$TEST_NUMBER.$ARCH

  if [ -f $REF_TEST/output_uwiremis_atlas.$TEST_NUMBER ] ; then
    diff -biw output_uwiremis_atlas.$TEST_NUMBER.$ARCH \
      $REF_TEST/output_uwiremis_atlas.$TEST_NUMBER > $DIFF_FILE
