#!/bin/sh
# Script to run the example_atlas_fwd example program
#
...
PROF_FILENAME="prof_atlas.dat"                # Input profile(s), usually found in $TEST_DIR set below
NPROF=1                                       # Number of profiles defined in prof.dat
NLEVELS=51                                    # Number of profile levels
MONTH=8                                       # Month (1-12) for emissivity/BRDF atlases
DO_SOLAR=1                                    # 0 = solar off / 1 = solar on

NCHAN=11                                      # Number of channels to simulate for each profile
...
echo " "
echo " "
echo " Test forward with emissivity/BRDF atlases "
echo " "
...
$BIN_DIR/example_atlas_fwd.exe << EOF
...
if [ $CHECK_REF -ne 0 ]; then
  ...
  OUT_FILE=output_example_atlas_fwd.dat
  DIFF_FILE=diff_example_atlas_fwd.${ARCH}
