#!/bin/sh
# Script to run the example_cld_mfasis_fwd example program
#
...
MFASIS_LUT_FILENAME="rttov_mfasis_cld_msg_4_seviri_deff.H5"    # Location of this file is set below in $MFASIS_DIR
...
MFASIS_DIR=../../rtcoef_rttov13/mfasis_lut   # Scattering coefficients directory
...
echo " "
echo " "
echo " Test forward with MFASIS cloud simulation "
echo " "

echo  "Coef filename:           ${COEF_FILENAME}"
echo  "Cloud coef filename:     ${CLD_COEF_FILENAME}"
echo  "MFASIS LUT filename:     ${MFASIS_LUT_FILENAME}"
...
MFASIS_LUT_FILENAME=$MFASIS_DIR/$MFASIS_LUT_FILENAME
if [ ! -f $MFASIS_LUT_FILENAME ]; then
  echo "Coef file $MFASIS_LUT_FILENAME not found, aborting..."
  exit 1
fi

$BIN_DIR/example_cld_mfasis_fwd.exe << EOF
"${COEF_FILENAME}"      , Coefficient filename
"${CLD_COEF_FILENAME}"  , Cloud coefficient filename
"${MFASIS_LUT_FILENAME}", MFASIS LUT filename
...
if [ $CHECK_REF -ne 0 ]; then
  ...
  OUT_FILE=output_example_cld_mfasis_fwd.dat
  DIFF_FILE=diff_example_cld_mfasis_fwd.${ARCH}
