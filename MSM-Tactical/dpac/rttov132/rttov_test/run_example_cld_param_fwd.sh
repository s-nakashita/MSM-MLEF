#!/bin/sh
# Script to run the example_cld_param_fwd example program
#
...
CLD_OPTP_FILENAME="cld_opt_param_avhrr.dat"
NPROF=1                                         # Number of profiles defined in prof.dat
NLEVELS=51                                      # Number of profile levels
DO_SOLAR=0                                      # 0 = solar off / 1 = solar on
NPHANGLE=208                                    # Number of angles over which cloud phase functions are defined
...
echo " "
echo " "
echo " Test forward with explicit cloud optical parameters "
echo " "

echo  "Coef filename:          ${COEF_FILENAME}"
echo  "Input profile file:     ${PROF_FILENAME}"
echo  "Optical param file:     ${CLD_OPTP_FILENAME}"
...
$BIN_DIR/example_cld_param_fwd.exe << EOF
"${COEF_FILENAME}"  , Coefficient filename
${PROF_FILENAME}    , Input profile filename
${CLD_OPTP_FILENAME}, Cloud optical parameter filename
...
if [ $CHECK_REF -ne 0 ]; then
  ...
  OUT_FILE=output_example_cld_param_fwd.dat
  DIFF_FILE=diff_example_cld_param_fwd.${ARCH}
