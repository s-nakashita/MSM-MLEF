#!/bin/sh
#
# CAMEL IR emissivity atlas test control script
#
# This test requires the CAMEL IR emissivity atlas files in the directory $DATAATLAS
...
DATAATLAS=../../emis_data
EXEC=../../$BIN/rttov_camel_atlas_test.exe
...
echo " "
echo " "
echo " Test CAMEL IR emissivity atlas "
echo " "
...
EOF
  ...
  mv output_camel_atlas.ascii  output_camel_atlas.$TEST_NUMBER.$ARCH
  ...
  echo
  echo "Output is in the file ${TEST}/output_camel_atlas.$TEST_NUMBER.$ARCH"

  DIFF_FILE=diff_camel_atlas.$TEST_NUMBER.$ARCH

  if [ -f $REF_TEST/output_camel_atlas.$TEST_NUMBER ] ; then
    diff -biw output_camel_atlas.$TEST_NUMBER.$ARCH \
      $REF_TEST/output_camel_atlas.$TEST_NUMBER > $DIFF_FILE
