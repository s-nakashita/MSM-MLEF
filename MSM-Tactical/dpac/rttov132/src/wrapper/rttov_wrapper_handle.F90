...
#include "rttov_nullify_scattcoeffs.interface"
  ...
  ! Ensure MFASIS LUT/NN is read if filename was specified
  ...
  ! MFASIS-NN overrides MFASIS-LUT if both files specified
  IF (rth%file_mfasis_nn .NE. '') &
    rth%opts%rt_ir%vis_scatt_model = vis_scatt_mfasis_nn
...
!                           file_mfasis_aer = rth%file_mfasis_aer, &
                          ...
                          file_mfasis_nn  = rth%file_mfasis_nn, &
