...
!------------------------------------------------------------------------------
  ...
  IF (rth%verbose) THEN
    ...
    IF (TRIM(rth%file_mfasis_nn) .NE. '') &
      INFO(rth%file_mfasis_nn)
...
!! @param[in]     surfemisrefl      input/output surface emis, refls, specularities, tskin eff (nchannels,nprofiles,5)
...
SUBROUTINE rttov_call_direct( &
    ...
    surfemisrefl,      &    ! surface emis, refls, specularities, tskin eff           (nchannels,nprofiles,5)
...
#include "throw.h"
  ...
  REAL(jprb),       INTENT(INOUT) :: surfemisrefl(nchannels,nprofiles,5)
...
!------------------------------------------------------------------------------
  ...
  DO i = 1, ncalls
    ...
    DO iprof = 1, thisnprof
      ...
      ENDWHERE
      IF (rth%opts%rt_all%use_tskin_eff) THEN
        emissivity(lochan:hichan)%tskin_eff = surfemisrefl(:,k,5)
      ENDIF
    ENDDO
...
!! @param[in]     surfemis          input/output surface emissivities, tskin eff (nchannels,nprofiles,2)
...
SUBROUTINE rttov_scatt_call_direct( &
    ...
    surfemis,          &    ! surface emissivities, tskin eff                         (nchannels,nprofiles,2)
...
#include "throw.h"
  ...
  REAL(jprb),       INTENT(INOUT) :: surfemis(nchannels,nprofiles,2)
...
!------------------------------------------------------------------------------
  ...
  DO i = 1, ncalls
    ...
    DO iprof = 1, thisnprof
      ...
      lochan = (iprof - 1) * nchannels + 1
      hichan = iprof * nchannels
      WHERE (surfemis(:,k,1) >= 0._jprb)
        emissivity(lochan:hichan)%emis_in = surfemis(:,k,1)
        calcemis(lochan:hichan) = .FALSE.
      ENDWHERE
      IF (rth%opts%rt_all%use_tskin_eff) THEN
        emissivity(lochan:hichan)%tskin_eff = surfemis(:,k,2)
      ENDIF
    ENDDO
    ...
    DO iprof = 1, thisnprof
      ...
      surfemis(:,k,1) = emissivity(lochan:hichan)%emis_out
...
!! @param[in]     surfemisrefl      input/output surface emis, refls, specularities, tskin eff  (nchannels,nprofiles,5)
!! @param[in,out] surfemisrefl_k    emis/refl/specularity/tskin eff Jacobians (nchannels,nprofiles,5)
!! @param[in,out] btrefl            returned BTs/reflectances (nchannels,nprofiles)
!! @param[in,out] rads              returned radiances (nchannels,nprofiles)
...
SUBROUTINE rttov_call_k( &
    ...
    surfemisrefl,      &    ! surface emis, refls, specularities, tskin eff           (nchannels,nprofiles,5)
    surfemisrefl_k,    &    ! output surface emis, refl, spec, tskin eff K            (nchannels,nprofiles,5)
    btrefl,            &    ! output BTs/refls (for thermal/solar chans)              (nchannels,nprofiles)
    rads,              &    ! output radiances                                        (nchannels,nprofiles)
...
#include "throw.h"
  ...
  REAL(jprb),       INTENT(INOUT) :: surfemisrefl(nchannels,nprofiles,5)
  REAL(jprb),       INTENT(INOUT) :: surfemisrefl_k(nchannels,nprofiles,5)
...
!------------------------------------------------------------------------------
  ...
  DO i = 1, ncalls
    ...
    DO iprof = 1, thisnprof
      ...
      surfemisrefl_k(:,k,5) = emissivity_k(lochan:hichan)%tskin_eff
    ENDDO
...
!! @param[in]     cld_pha           cloud phase fns (cld_nphangle,nlayers,nchannels,nprofiles)
!! @param[in]     surfemisrefl      input/output surface emis, refls, specularities, tskin eff (nchannels,nprofiles,5)
!! @param[in,out] surfemisrefl_k    emissivity/reflectance/specularity/tskin eff Jacobians (nchannels,nprofiles,5)
!! @param[in,out] btrefl            returned BTs/reflectances (nchannels,nprofiles)
!! @param[in,out] rads              returned radiances (nchannels,nprofiles)
!! @param[in]     bt_k              input BT perturbations (nchannels,nprofiles)
!! @param[in]     rads_k            input radiance perturbations (nchannels,nprofiles)
...
!! @param[in]     calc_zef          enable/disable the radar reflectivity calculations
!! @param[in]     surfemis          input/output surface emissivities, tskin eff (nchannels,nprofiles,2)
!! @param[in,out] surfemis_k        emissivity, tskin eff Jacobians (nchannels,nprofiles,2)
!! @param[in,out] bt                returned BTs (nchannels,nprofiles)
!! @param[in]     bt_k              input BT perturbations (nchannels,nprofiles)
...
SUBROUTINE rttov_scatt_call_k( &
    ...
    calc_zef,          &    ! enable/disable the radar reflectivity calculations
    surfemis,          &    ! surface emissivities, tskin eff                         (nchannels,nprofiles,2)
    surfemis_k,        &    ! output surface emissivities, tskin eff K                (nchannels,nprofiles,2)
    bt,                &    ! output BTs                                              (nchannels,nprofiles)
...
#include "throw.h"
  ...
  REAL(jprb),       INTENT(INOUT) :: surfemis(nchannels,nprofiles,2)
  REAL(jprb),       INTENT(INOUT) :: surfemis_k(nchannels,nprofiles,2)
...
!------------------------------------------------------------------------------
  ...
  DO i = 1, ncalls
    ...
    DO iprof = 1, thisnprof
      ...
      bt(:,k) = radiance%bt(lochan:hichan)
      surfemis(:,k,1) = emissivity(lochan:hichan)%emis_out
      surfemis_k(:,k,1) = emissivity_k(lochan:hichan)%emis_in
      surfemis_k(:,k,2) = emissivity_k(lochan:hichan)%tskin_eff
    ENDDO
...
!------------------------------------------------------------------------------
  ...
  CALL getoption('opts%config%opdep13_gas_clip', ival, exists = exists)
  IF (exists) THEN
    rth%opts%config%opdep13_gas_clip = (ival .NE. 0)
    rth%opts_scatt%config%opdep13_gas_clip = (ival .NE. 0)
  ENDIF

  CALL getoption('opts_scatt%config%opdep13_gas_clip', ival, exists = exists)
  IF (exists) THEN
    rth%opts%config%opdep13_gas_clip = (ival .NE. 0)
    rth%opts_scatt%config%opdep13_gas_clip = (ival .NE. 0)
  ENDIF
  ...
  CALL getoption('opts%rt_all%use_tskin_eff', ival, exists = exists)
  IF (exists) THEN
    rth%opts%rt_all%use_tskin_eff = (ival .NE. 0)
    rth%opts_scatt%use_tskin_eff = (ival .NE. 0)
  ENDIF

  CALL getoption('opts_scatt%use_tskin_eff', ival, exists = exists)
  IF (exists) THEN
    rth%opts%rt_all%use_tskin_eff = (ival .NE. 0)
    rth%opts_scatt%use_tskin_eff = (ival .NE. 0)
  ENDIF
  ...
  CALL getoption('opts%rt_all%transmittances_only', ival, exists = exists)
  IF (exists) THEN
    rth%opts%rt_all%transmittances_only = (ival .NE. 0)
  ENDIF
  ...
  CALL getoption('opts%rt_mw%fastem3_rwd_fix', ival, exists = exists)
  IF (exists) THEN
    rth%opts%rt_mw%fastem3_rwd_fix = (ival .NE. 0)
    rth%opts_scatt%fastem3_rwd_fix = (ival .NE. 0)
  ENDIF

  CALL getoption('opts_scatt%fastem3_rwd_fix', ival, exists = exists)
  IF (exists) THEN
    rth%opts%rt_mw%fastem3_rwd_fix = (ival .NE. 0)
    rth%opts_scatt%fastem3_rwd_fix = (ival .NE. 0)
  ENDIF
  ...
  CALL getoption('opts_scatt%pol_mode', ival, exists = exists)
  IF (exists) rth%opts_scatt%pol_mode = ival
  ...
  CALL getoption('opts%rt_ir%rayleigh_depol', ival, exists = exists)
  IF (exists) rth%opts%rt_ir%rayleigh_depol = (ival .NE. 0)
  ...
  CALL getoption('file_mfasis_nn', strval, exists = exists)
  IF (exists) rth%file_mfasis_nn = TRIM(strval)
...
!   IF (exists) rth%file_pccoef = TRIM(strval)
  ...
  CALL getoption('file_pol', strval, exists = exists)
  IF (exists) rth%file_pol = TRIM(strval)
...
!------------------------------------------------------------------------------
  ...
  IF (rth%file_mfasis_nn .NE. '') &
    WRITE(error_unit,'(2x,a,a)')      "MFASIS NN file       ", TRIM(rth%file_mfasis_nn)
  ...
  IF (rth%file_hydrotable .NE. '') &
    ...
    IF (rth%file_pol .NE. '') &
      WRITE(error_unit,'(2x,a,a)')      "RTTOV-SCATT pol file   ", rth%file_pol
