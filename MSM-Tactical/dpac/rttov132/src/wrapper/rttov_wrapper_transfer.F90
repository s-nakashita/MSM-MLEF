  ...
  DO i = 1, nprofiles
    ...
    DO g = 1, ngases
      ...
      ELSEIF (gas_id(g) == gas_id_scatt_rain) THEN
        cld_profiles(i)%hydro(:,hydro_index_rain) = gases(:,j,g)
      ELSEIF (gas_id(g) == gas_id_scatt_snow) THEN
        cld_profiles(i)%hydro(:,hydro_index_snow) = gases(:,j,g)
      ELSEIF (gas_id(g) == gas_id_scatt_graupel) THEN
        cld_profiles(i)%hydro(:,hydro_index_graupel) = gases(:,j,g)
      ELSEIF (gas_id(g) == gas_id_scatt_clw) THEN
        cld_profiles(i)%hydro(:,hydro_index_clw) = gases(:,j,g)
      ELSEIF (gas_id(g) == gas_id_scatt_ciw) THEN
        cld_profiles(i)%hydro(:,hydro_index_ciw) = gases(:,j,g)
      ENDIF
  ...
  DO i = 1, nprofiles
    ...
    DO j = 1, nchannels
      ...
      DO g = 1, ngases
        ...
        ELSEIF (gas_id(g) == gas_id_scatt_rain) THEN
          gases_k(:,j,iprof,g) = cld_profiles_k(k)%hydro(:,hydro_index_rain)
        ELSEIF (gas_id(g) == gas_id_scatt_snow) THEN
          gases_k(:,j,iprof,g) = cld_profiles_k(k)%hydro(:,hydro_index_snow)
        ELSEIF (gas_id(g) == gas_id_scatt_graupel) THEN
          gases_k(:,j,iprof,g) = cld_profiles_k(k)%hydro(:,hydro_index_graupel)
        ELSEIF (gas_id(g) == gas_id_scatt_clw) THEN
          gases_k(:,j,iprof,g) = cld_profiles_k(k)%hydro(:,hydro_index_clw)
        ELSEIF (gas_id(g) == gas_id_scatt_ciw) THEN
          gases_k(:,j,iprof,g) = cld_profiles_k(k)%hydro(:,hydro_index_ciw)
        ENDIF
