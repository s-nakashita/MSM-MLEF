...
#include "rttov_alloc_phfn_int.interface"
      ...
      PUBLIC :: RTTOV_HDF_MFASISNNCOEF_WH, RTTOV_HDF_MFASISNNCOEF_RH
...
!! param[in]  force_double  if true all real values are stored as H5T_NATIVE_DOUBLE, optional
      SUBROUTINE RTTOV_HDF_MFASISCOEF_WH(X, FILE_TYPE, LUN,ERR,COMPRESS,FORCE_DOUBLE)
        ...
        IF (X%VERSION > 1) THEN
          SNAME = 'CHANNEL_DEFF_MIXING'
          CALL write_array_hdf(LUN,SNAME,&
          'List of switches for enabling special treatment for particular channels',&
          ERR,I1=X%CHANNEL_DEFF_MIXING)
          THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))
        ENDIF
...
!> Write MFASIS NN structure to HDF5 file
!! param[in]  x             MFASIS NN structure
!! param[in]  lun           file ID of HDF5 file
!! param[out] err           return status
!! param[in]  compress      if true will apply internal HDF5 compression, optional
!! param[in]  force_double  if true all real values are stored as H5T_NATIVE_DOUBLE, optional
      SUBROUTINE RTTOV_HDF_MFASISNNCOEF_WH(X, LUN,ERR,COMPRESS,FORCE_DOUBLE)

        TYPE(RTTOV_COEF_MFASIS_NN), INTENT(IN)    :: X
        INTEGER(HID_T),             INTENT(IN)    :: LUN
        INTEGER(KIND=JPIM),         INTENT(OUT)   :: ERR
        LOGICAL,INTENT(IN),OPTIONAL    ::COMPRESS
        LOGICAL,INTENT(IN),OPTIONAL    ::FORCE_DOUBLE

        CHARACTER(LEN=LENSH) :: GNAME, SNAME, SGNAME, SSGNAME
        INTEGER(KIND=JPIM)   :: I, J
        INTEGER(KIND=HID_T)  :: G_ID_SUB
        ...
        SNAME = 'CHANNEL_LIST'
        CALL write_array_hdf(LUN,SNAME,&
        'List of channels for which NNs are stored',&
        ERR,I1=X%CHANNEL_LIST)
        THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

        SNAME = 'CHANNEL_NN_INDEX'
        CALL write_array_hdf(LUN,SNAME,&
        'Index into channel_list for each channel in rtcoef file',&
        ERR,I1=X%CHANNEL_NN_INDEX)
        THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

        SNAME = 'NPARTICLES'
        CALL write_array_hdf(LUN,SNAME,&
        'Number of particle types included in NNs',&
        ERR,I0=X%NPARTICLES)
        THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

        ! Write NN data: assume one NN per channel
        DO I = 1, X%NCHANNELS
          WRITE(GNAME, '(A,I6.6)') 'NN', I

          CALL MKPAR( LUN, GNAME, G_ID_SUB, ERR )
          THROWM(ERR.NE.0,"CANNOT CREATE GROUP "//GNAME)

          ! Write group NETWORK
          SGNAME = TRIM(GNAME)//'/NETWORK'
          CALL MKPAR( LUN, SGNAME, G_ID_SUB, ERR )
          THROWM(ERR.NE.0,"CANNOT CREATE GROUP "//SGNAME)

          SNAME = TRIM(SGNAME)//'/N_INPUT'
          CALL write_array_hdf(LUN,SNAME,&
          'Number of input nodes',&
          ERR,I0=X%NN(I)%N_INPUT)
          THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

          SNAME = TRIM(SGNAME)//'/N_OUTPUT'
          CALL write_array_hdf(LUN,SNAME,&
          'Number of output nodes',&
          ERR,I0=X%NN(I)%N_OUTPUT)
          THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

          SNAME = TRIM(SGNAME)//'/N_HIDDEN'
          CALL write_array_hdf(LUN,SNAME,&
          'Number of hidden layers',&
          ERR,I0=X%NN(I)%N_HIDDEN)
          THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

          SNAME = TRIM(SGNAME)//'/N_NODES_MAX'
          CALL write_array_hdf(LUN,SNAME,&
          'Maximum number of nodes in layers',&
          ERR,I0=X%NN(I)%N_NODES_MAX)
          THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

          SNAME = TRIM(SGNAME)//'/NN_VERSION'
          CALL write_array_hdf(LUN,SNAME,&
          'NN version',&
          ERR,I0=X%NN(I)%NN_VERSION)
          THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

          SNAME = TRIM(SGNAME)//'/ACTIVATION_HIDDEN_LAYERS'
          CALL write_array_hdf(LUN,SNAME,&
          'Activation function used in all hidden layers',&
          ERR,I0=X%NN(I)%ACTFUNC_HL)
          THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

          SNAME = TRIM(SGNAME)//'/ACTIVATION_OUTPUT_LAYER'
          CALL write_array_hdf(LUN,SNAME,&
          'Activation function used in output layer',&
          ERR,I0=X%NN(I)%ACTFUNC_OL)
          THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

          SNAME = TRIM(SGNAME)//'/BIAS_I'
          CALL write_array_hdf(LUN,SNAME,&
          'Biases for input layer',&
          ERR,R1=X%NN(I)%BIAS_I, COMPRESS=COMPRESS, FORCE_DOUBLE=FORCE_DOUBLE)
          THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

          SNAME = TRIM(SGNAME)//'/BIAS_H'
          CALL write_array_hdf(LUN,SNAME,&
          'Biases for hidden layers',&
          ERR,R2=X%NN(I)%BIAS_H, COMPRESS=COMPRESS, FORCE_DOUBLE=FORCE_DOUBLE)
          THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

          SNAME = TRIM(SGNAME)//'/BIAS_O'
          CALL write_array_hdf(LUN,SNAME,&
          'Biases for output layer',&
          ERR,R1=X%NN(I)%BIAS_O, COMPRESS=COMPRESS, FORCE_DOUBLE=FORCE_DOUBLE)
          THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

          SNAME = TRIM(SGNAME)//'/WEIGHT_I'
          CALL write_array_hdf(LUN,SNAME,&
          'Weights for input layer',&
          ERR,R2=X%NN(I)%WEIGHT_I, COMPRESS=COMPRESS, FORCE_DOUBLE=FORCE_DOUBLE)
          THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

          SNAME = TRIM(SGNAME)//'/WEIGHT_H'
          CALL write_array_hdf(LUN,SNAME,&
          'Weights for hidden layers',&
          ERR,R3=X%NN(I)%WEIGHT_H, COMPRESS=COMPRESS, FORCE_DOUBLE=FORCE_DOUBLE)
          THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

          SNAME = TRIM(SGNAME)//'/WEIGHT_O'
          CALL write_array_hdf(LUN,SNAME,&
          'Weights for output layer',&
          ERR,R2=X%NN(I)%WEIGHT_O, COMPRESS=COMPRESS, FORCE_DOUBLE=FORCE_DOUBLE)
          THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

          CALL H5GCLOSE_F( G_ID_SUB, ERR )
          THROWM(ERR.NE.0,"CANNOT CLOSE GROUP "//TRIM(GNAME))

          ! Write group INPUT_PARAMS
          SGNAME = TRIM(GNAME)//'/INPUT_PARAMS'
          CALL MKPAR( LUN, SGNAME, G_ID_SUB, ERR )
          THROWM(ERR.NE.0,"CANNOT CREATE GROUP "//SGNAME)

          DO J = 1, X%NN(I)%N_INPUT
            WRITE(SSGNAME, '(A,I3.3)') TRIM(SGNAME)//'/INDIM', J

            CALL MKPAR( LUN, SSGNAME, G_ID_SUB, ERR )
            THROWM(ERR.NE.0,"CANNOT CREATE GROUP "//SSGNAME)

            SNAME = TRIM(SSGNAME)//'/NAME'
            CALL write_array_hdf(LUN,SNAME,&
            'Parameter name',&
            ERR,C0=X%NN(I)%IN(J)%NAME)
            THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

            SNAME = TRIM(SSGNAME)//'/TRANSFORM'
            CALL write_array_hdf(LUN,SNAME,&
            'Transform name',&
            ERR,C0=X%NN(I)%IN(J)%TRANSFORM)
            THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

            SNAME = TRIM(SSGNAME)//'/MIN'
            CALL write_array_hdf(LUN,SNAME,&
            'Parameter minimum',&
            ERR,R0=X%NN(I)%IN(J)%MIN)
            THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

            SNAME = TRIM(SSGNAME)//'/MAX'
            CALL write_array_hdf(LUN,SNAME,&
            'Parameter maximum',&
            ERR,R0=X%NN(I)%IN(J)%MAX)
            THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

            SNAME = TRIM(SSGNAME)//'/NAUXPARAMS'
            CALL write_array_hdf(LUN,SNAME,&
            'Prec-processing parameters',&
            ERR,I0=X%NN(I)%IN(J)%NAUXPARAMS)
            THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

            IF (X%NN(I)%IN(J)%NAUXPARAMS > 0) THEN
              SNAME = TRIM(SSGNAME)//'/AUXPARAMS'
              CALL write_array_hdf(LUN,SNAME,&
              'Prec-processing parameters',&
              ERR,R1=X%NN(I)%IN(J)%AUXPARAMS)
              THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))
            ENDIF
          ...
          ! Write group OUTPUT_PARAMS
          SGNAME = TRIM(GNAME)//'/OUTPUT_PARAMS'
          CALL MKPAR( LUN, SGNAME, G_ID_SUB, ERR )
          THROWM(ERR.NE.0,"CANNOT CREATE GROUP "//SGNAME)

          DO J = 1, X%NN(I)%N_OUTPUT
            WRITE(SSGNAME, '(A,I3.3)') TRIM(SGNAME)//'/OUTDIM', J
            ...
            CALL write_array_hdf(LUN,SNAME,&
            'Parameter name',&
            ERR,C0=X%NN(I)%OUT(J)%NAME)
            THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

            SNAME = TRIM(SSGNAME)//'/TRANSFORM'
            CALL write_array_hdf(LUN,SNAME,&
            'Transform name',&
            ERR,C0=X%NN(I)%OUT(J)%TRANSFORM)
            THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

            SNAME = TRIM(SSGNAME)//'/MIN'
            CALL write_array_hdf(LUN,SNAME,&
            'Parameter minimum',&
            ERR,R0=X%NN(I)%OUT(J)%MIN)
            THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

            SNAME = TRIM(SSGNAME)//'/MAX'
            CALL write_array_hdf(LUN,SNAME,&
            'Parameter maximum',&
            ERR,R0=X%NN(I)%OUT(J)%MAX)
            THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

            SNAME = TRIM(SSGNAME)//'/NAUXPARAMS'
            CALL write_array_hdf(LUN,SNAME,&
            'Prec-processing parameters',&
            ERR,I0=X%NN(I)%OUT(J)%NAUXPARAMS)
            THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))

            IF (X%NN(I)%OUT(J)%NAUXPARAMS > 0) THEN
              SNAME = TRIM(SSGNAME)//'/AUXPARAMS'
              CALL write_array_hdf(LUN,SNAME,&
              'Prec-processing parameters',&
              ERR,R1=X%NN(I)%OUT(J)%AUXPARAMS)
              THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))
            ENDIF
        ...
        CATCH
      END SUBROUTINE RTTOV_HDF_MFASISNNCOEF_WH
...
!! param[out]    err           return status
      SUBROUTINE RTTOV_HDF_MFASISCOEF_RH(X, FILE_TYPE, LUN, ERR)
        ...
        IF (X%VERSION > 1) THEN
          CALL read_array_hdf(LUN,'CHANNEL_DEFF_MIXING',ERR,PI1=X%CHANNEL_DEFF_MIXING)
          THROWM(ERR.NE.0,"Failure reading MFASIS LUT")
        ELSE
          ALLOCATE(X%CHANNEL_DEFF_MIXING(X%NCHANNELS),STAT=ERR)
          THROWM(ERR.NE.0,"Failure allocating MFASIS LUT")
          X%CHANNEL_DEFF_MIXING(:) = 0
        ENDIF
...
!> Read MFASIS NN structure from HDF5 file
!! param[inout]  x             MFASIS NN structure
!! param[in]     lun           file ID of HDF5 file
!! param[out]    err           return status
      SUBROUTINE RTTOV_HDF_MFASISNNCOEF_RH(X, LUN, ERR)
        USE RTTOV_CONST, ONLY : MFASIS_NN_VERSION_COMPATIBLE_MIN, &
                                MFASIS_NN_VERSION_COMPATIBLE_MAX

        TYPE(RTTOV_COEF_MFASIS_NN), INTENT(INOUT) :: X
        INTEGER(HID_T),             INTENT(IN)    :: LUN
        INTEGER(KIND=JPIM),         INTENT(OUT)   :: ERR

        CHARACTER(LEN=LENSH) :: GNAME, SGNAME
        INTEGER(KIND=JPIM)   :: I, J

        TRY

        CALL read_array_hdf(LUN,'VERSION',ERR,I0=X%VERSION)
        THROWM(ERR.NE.0,"Failure reading MFASIS NN version")

        IF (x%version < mfasis_nn_version_compatible_min .OR. &
            x%version > mfasis_nn_version_compatible_max) THEN
          err = errorstatus_fatal
          THROWM(err.NE.0, "Version of MFASIS NN file is incompatible with RTTOV library")
        ENDIF

        CALL read_array_hdf(LUN,'FILE_TYPE',ERR,I0=X%FILE_TYPE)
        THROWM(ERR.NE.0,"Failure reading MFASIS NN file (FILE_TYPE)")

        CALL read_array_hdf(LUN,'README_NN',ERR,C1=X%README_NN)
        THROWM(ERR.NE.0,"Failure reading MFASIS NN file (README_NN)")

        CALL read_array_hdf(LUN,'CREATION_DATE',ERR,I1=X%CREATION_DATE)
        THROWM(ERR.NE.0,"Failure reading MFASIS NN file (CREATION_DATE)")

        CALL read_array_hdf(LUN,'NCHANNELS_COEF',ERR,I0=X%NCHANNELS_COEF)
        THROWM(ERR.NE.0,"Failure reading MFASIS NN file (NCHANNELS_COEF)")

        CALL read_array_hdf(LUN,'NCHANNELS',ERR,I0=X%NCHANNELS)
        THROWM(ERR.NE.0,"Failure reading MFASIS NN file (NCHANNELS)")

        CALL read_array_hdf(LUN,'CHANNEL_LIST',ERR,PI1=X%CHANNEL_LIST)
        THROWM(ERR.NE.0,"Failure reading MFASIS NN file (CHANNEL_LIST)")

        CALL read_array_hdf(LUN,'CHANNEL_NN_INDEX',ERR,PI1=X%CHANNEL_NN_INDEX)
        THROWM(ERR.NE.0,"Failure reading MFASIS NN file (CHANNEL_NN_INDEX)")

        CALL read_array_hdf(LUN,'NPARTICLES',ERR,I0=X%NPARTICLES)
        THROWM(ERR.NE.0,"Failure reading MFASIS NN file (NPARTICLES)")

        ! Read NN input: assume one NN per channel
        ALLOCATE(X%NN(X%NCHANNELS), STAT=ERR)
        THROWM(ERR.NE.0,"Failure allocating MFASIS NN")

        DO I = 1, X%NCHANNELS
          WRITE(GNAME, '(A,I6.6)') 'NN', I

          ! Read group NETWORK
          CALL read_array_hdf(LUN,TRIM(GNAME)//'/NETWORK/N_INPUT',ERR,I0=X%NN(I)%N_INPUT)
          THROWM(ERR.NE.0,"Failure reading MFASIS NN%N_INPUT")

          CALL read_array_hdf(LUN,TRIM(GNAME)//'/NETWORK/N_OUTPUT',ERR,I0=X%NN(I)%N_OUTPUT)
          THROWM(ERR.NE.0,"Failure reading MFASIS NN%N_OUTPUT")

          CALL read_array_hdf(LUN,TRIM(GNAME)//'/NETWORK/N_HIDDEN',ERR,I0=X%NN(I)%N_HIDDEN)
          THROWM(ERR.NE.0,"Failure reading MFASIS NN%N_HIDDEN")

          CALL read_array_hdf(LUN,TRIM(GNAME)//'/NETWORK/N_NODES_MAX',ERR,I0=X%NN(I)%N_NODES_MAX)
          THROWM(ERR.NE.0,"Failure reading MFASIS NN%N_NODES_MAX")

          CALL read_array_hdf(LUN,TRIM(GNAME)//'/NETWORK/NN_VERSION',ERR,I0=X%NN(I)%NN_VERSION)
          THROWM(ERR.NE.0,"Failure reading MFASIS NN%NN_VERSION")

          CALL read_array_hdf(LUN,TRIM(GNAME)//'/NETWORK/ACTIVATION_HIDDEN_LAYERS',ERR,I0=X%NN(I)%ACTFUNC_HL)
          THROWM(ERR.NE.0,"Failure reading MFASIS NN%ACTFUNC_HL")

          CALL read_array_hdf(LUN,TRIM(GNAME)//'/NETWORK/ACTIVATION_OUTPUT_LAYER',ERR,I0=X%NN(I)%ACTFUNC_OL)
          THROWM(ERR.NE.0,"Failure reading MFASIS NN%ACTFUNC_OL")

          CALL read_array_hdf(LUN,TRIM(GNAME)//'/NETWORK/BIAS_I',ERR,PR1=X%NN(I)%BIAS_I)
          THROWM(ERR.NE.0,"Failure reading MFASIS NN%BIAS_I")

          CALL read_array_hdf(LUN,TRIM(GNAME)//'/NETWORK/BIAS_H',ERR,PR2=X%NN(I)%BIAS_H)
          THROWM(ERR.NE.0,"Failure reading MFASIS NN%BIAS_H")

          CALL read_array_hdf(LUN,TRIM(GNAME)//'/NETWORK/BIAS_O',ERR,PR1=X%NN(I)%BIAS_O)
          THROWM(ERR.NE.0,"Failure reading MFASIS NN%BIAS_O")

          CALL read_array_hdf(LUN,TRIM(GNAME)//'/NETWORK/WEIGHT_I',ERR,PR2=X%NN(I)%WEIGHT_I)
          THROWM(ERR.NE.0,"Failure reading MFASIS NN%WEIGHT_I")

          CALL read_array_hdf(LUN,TRIM(GNAME)//'/NETWORK/WEIGHT_H',ERR,PR3=X%NN(I)%WEIGHT_H)
          THROWM(ERR.NE.0,"Failure reading MFASIS NN%WEIGHT_H")

          CALL read_array_hdf(LUN,TRIM(GNAME)//'/NETWORK/WEIGHT_O',ERR,PR2=X%NN(I)%WEIGHT_O)
          THROWM(ERR.NE.0,"Failure reading MFASIS NN%WEIGHT_O")

          ! Read group INPUT_PARAMS
          ALLOCATE(X%NN(I)%IN(X%NN(I)%N_INPUT), STAT=ERR)
          THROWM(ERR.NE.0,"Failure allocating MFASIS NN/IN")

          DO J = 1, X%NN(I)%N_INPUT
            WRITE(SGNAME, '(A,I3.3)') 'INDIM', J

            CALL read_array_hdf(LUN,TRIM(GNAME)//'/INPUT_PARAMS/'//TRIM(SGNAME)//'/NAME',ERR,C0=X%NN(I)%IN(J)%NAME)
            THROWM(ERR.NE.0,"Failure reading MFASIS NN%IN%NAME")

            CALL read_array_hdf(LUN,TRIM(GNAME)//'/INPUT_PARAMS/'//TRIM(SGNAME)//'/TRANSFORM',ERR,C0=X%NN(I)%IN(J)%TRANSFORM)
            THROWM(ERR.NE.0,"Failure reading MFASIS NN%IN%TRANSFORM")

            CALL read_array_hdf(LUN,TRIM(GNAME)//'/INPUT_PARAMS/'//TRIM(SGNAME)//'/MIN',ERR,R0=X%NN(I)%IN(J)%MIN)
            THROWM(ERR.NE.0,"Failure reading MFASIS NN%IN%MIN")

            CALL read_array_hdf(LUN,TRIM(GNAME)//'/INPUT_PARAMS/'//TRIM(SGNAME)//'/MAX',ERR,R0=X%NN(I)%IN(J)%MAX)
            THROWM(ERR.NE.0,"Failure reading MFASIS NN%IN%MAX")

            CALL read_array_hdf(LUN,TRIM(GNAME)//'/INPUT_PARAMS/'//TRIM(SGNAME)//'/NAUXPARAMS',ERR,I0=X%NN(I)%IN(J)%NAUXPARAMS)
            THROWM(ERR.NE.0,"Failure reading MFASIS NN%IN%AUXPARAMS")

            IF (X%NN(I)%IN(J)%NAUXPARAMS > 0) THEN
              CALL read_array_hdf(LUN,TRIM(GNAME)//'/INPUT_PARAMS/'//TRIM(SGNAME)//'/AUXPARAMS',ERR,PR1=X%NN(I)%IN(J)%AUXPARAMS)
              THROWM(ERR.NE.0,"Failure reading MFASIS NN%IN%AUXPARAMS")
            ELSE
              ALLOCATE(X%NN(I)%IN(J)%AUXPARAMS(X%NN(I)%IN(J)%NAUXPARAMS))
            ENDIF
          ENDDO

          ! Read group OUT_PARAMS
          ALLOCATE(X%NN(I)%OUT(X%NN(I)%N_OUTPUT), STAT=ERR)
          THROWM(ERR.NE.0,"Failure allocating MFASIS NN/OUT")

          DO J = 1, X%NN(I)%N_OUTPUT
            WRITE(SGNAME, '(A,I3.3)') 'OUTDIM', J

            CALL read_array_hdf(LUN,TRIM(GNAME)//'/OUTPUT_PARAMS/'//TRIM(SGNAME)//'/NAME',ERR,C0=X%NN(I)%OUT(J)%NAME)
            THROWM(ERR.NE.0,"Failure reading MFASIS NN%OUT%NAME")

            CALL read_array_hdf(LUN,TRIM(GNAME)//'/OUTPUT_PARAMS/'//TRIM(SGNAME)//'/TRANSFORM',ERR,C0=X%NN(I)%OUT(J)%TRANSFORM)
            THROWM(ERR.NE.0,"Failure reading MFASIS NN%OUT%TRANSFORM")

            CALL read_array_hdf(LUN,TRIM(GNAME)//'/OUTPUT_PARAMS/'//TRIM(SGNAME)//'/MIN',ERR,R0=X%NN(I)%OUT(J)%MIN)
            THROWM(ERR.NE.0,"Failure reading MFASIS NN%OUT%MIN")

            CALL read_array_hdf(LUN,TRIM(GNAME)//'/OUTPUT_PARAMS/'//TRIM(SGNAME)//'/MAX',ERR,R0=X%NN(I)%OUT(J)%MAX)
            THROWM(ERR.NE.0,"Failure reading MFASIS NN%OUT%MAX")

            CALL read_array_hdf(LUN,TRIM(GNAME)//'/OUTPUT_PARAMS/'//TRIM(SGNAME)//'/NAUXPARAMS',ERR,I0=X%NN(I)%OUT(J)%NAUXPARAMS)
            THROWM(ERR.NE.0,"Failure reading MFASIS NN%OUT%AUXPARAMS")

            IF (X%NN(I)%OUT(J)%NAUXPARAMS > 0) THEN
              CALL read_array_hdf(LUN,TRIM(GNAME)//'/OUTPUT_PARAMS/'//TRIM(SGNAME)//'/AUXPARAMS',ERR,PR1=X%NN(I)%OUT(J)%AUXPARAMS)
              THROWM(ERR.NE.0,"Failure reading MFASIS NN%OUT%AUXPARAMS")
            ELSE
              ALLOCATE(X%NN(I)%OUT(J)%AUXPARAMS(X%NN(I)%OUT(J)%NAUXPARAMS))
            ENDIF
          ENDDO
        ENDDO

        CATCH
      END SUBROUTINE RTTOV_HDF_MFASISNNCOEF_RH
