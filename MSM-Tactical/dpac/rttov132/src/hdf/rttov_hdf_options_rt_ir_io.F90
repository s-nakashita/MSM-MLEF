...
sname='RAYLEIGH_DEPOL'
call read_array_hdf(lun,sname,err,l0=x%RAYLEIGH_DEPOL)
THROWM(err.ne.0,"CANNOT READ "//trim(sname))
...
sname='VIS_SCATT_MODEL'
call write_array_hdf(lun,sname,&
  & 'Visible/near-IR scattering model: 1=>DOM, 2=>single-scattering, 3=>MFASIS, 4=>MFASIS-NN',&
...
sname='RAYLEIGH_DEPOL'
call write_array_hdf(lun,sname,&
  & 'Include depolarisation impact on phase fn in Rayleigh scattering',&
  & err,l0=x%RAYLEIGH_DEPOL   )
THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))
