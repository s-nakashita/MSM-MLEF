...
!! @param[in]     mfasisnn      MFASIS NN structure, optional
...
!
      SUBROUTINE RTTOV_HDF_SAVE( ERR, F, PATH, CREATE, PROFILE, PROFILES, KMATRIX, OPTIONS, CHANPROF, &
         ...
         &  MFASISLUTCLD, MFASISLUTAER, MFASISNN, PCCOEF, &
...
!
      ...
      TYPE(RTTOV_COEF_MFASIS_NN), INTENT(IN), OPTIONAL :: MFASISNN
...
TRY
      ...
      IF( PRESENT(MFASISNN) ) THEN

        CALL RTTOV_HDF_MFASISNNCOEF_WH(MFASISNN, G_ID,ERR, COMPRESS=COMPRESS, FORCE_DOUBLE=FORCE_DOUBLE)
        THROWM(ERR.NE.0,"CANNOT WRITE MFASIS NN IN FILE "//TRIM(F))
