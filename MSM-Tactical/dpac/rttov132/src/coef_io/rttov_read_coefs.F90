...
!!
!!   The MFASIS cloud LUT or NN file will only be read if the addclouds option
...
!! @param[in]     form_mfasis_nn     format of MFASIS NN file, optional
...
!! @param[in]     file_mfasis_nn     file name of MFASIS NN file, optional
...
!! @param[in]     file_id_mfasis_nn  logical unit of MFASIS NN file, optional
...
!INTF_OFF
  ...
  USE rttov_const, ONLY : inst_name, vis_scatt_mfasis, vis_scatt_mfasis_nn
...
!   CHARACTER(LEN=*),   INTENT(IN), OPTIONAL :: form_mfasis_aer
  ...
  CHARACTER(LEN=*),   INTENT(IN), OPTIONAL :: form_mfasis_nn
...
!   CHARACTER(LEN=*),   INTENT(IN), OPTIONAL :: file_mfasis_aer
  ...
  CHARACTER(LEN=*),   INTENT(IN), OPTIONAL :: file_mfasis_nn
...
!   INTEGER(KIND=jpim), INTENT(IN), OPTIONAL :: file_id_mfasis_aer
  ...
  INTEGER(KIND=jpim), INTENT(IN), OPTIONAL :: file_id_mfasis_nn
...
#include "rttov_read_ascii_mfasis_nn_file.interface"
...
#include "rttov_read_binary_mfasis_nn_file.interface"
...
#include "rttov_channel_extract_mfasis_nn.interface"
...
#include "rttov_dealloc_coef_mfasis_nn.interface"
...
!   INTEGER(KIND=jpim) :: file_id_mfasis_aer1=0_jpim
  ...
  INTEGER(KIND=jpim) :: file_id_mfasis_nn1=0_jpim
...
!   CHARACTER(LEN=256) :: file_mfasis_aer1
  ...
  CHARACTER(LEN=256) :: file_mfasis_nn1
...
!   CHARACTER(LEN=32) :: form_mfasis_aer1=""
  ...
  CHARACTER(LEN=32) :: form_mfasis_nn1=""
...
!     ENDIF ! MFASIS
  ...
  IF (opts%rt_ir%addclouds .AND. .NOT. opts%rt_ir%user_cld_opt_param) THEN
    ...
    ! MFASIS-NN file
    !--------------------------
    IF (opts%rt_ir%vis_scatt_model == vis_scatt_mfasis_nn .AND. &
        (PRESENT(instrument) .OR. PRESENT(file_mfasis_nn) .OR. PRESENT(file_id_mfasis_nn))) THEN

      CALL getlun(err, file_id_mfasis_nn1, file_mfasis_nn1, form_mfasis_nn1, .FALSE._jplm, "rttov_mfasis_nn", &
                       file_id_mfasis_nn, file_mfasis_nn, form_mfasis_nn, instrument, path)
      THROW(err.NE.0)

      IF (rttov_cmpuc(form_mfasis_nn1,"unformatted")) THEN

        CALL rttov_read_binary_mfasis_nn_file(err, coefs%coef, coefs%coef_mfasis_nn, &
                                              file_id_mfasis_nn1, channels, file_type=1_jpim)
        THROW(err.NE.0)
        CALL closelun(err, file_id_mfasis_nn1, file_id_mfasis_nn)
        THROW(err.NE.0)

      ELSE IF (rttov_cmpuc(form_mfasis_nn1,"formatted")) THEN

        CALL rttov_read_ascii_mfasis_nn_file(err, coefs%coef, coefs%coef_mfasis_nn, &
                                             file_id_mfasis_nn1, channels, file_type=1_jpim)
        THROW(err.NE.0)
        CALL closelun(err, file_id_mfasis_nn1, file_id_mfasis_nn)
        THROW(err.NE.0)

      ELSE IF (rttov_cmpuc(form_mfasis_nn1,"hdf5")) THEN
#ifndef _RTTOV_HDF
        err = errorstatus_fatal
        THROWM(err.NE.0,"This program is not compiled with HDF5 capability; use RTTOV_HDF=1 with Makefile.PL")
#else
        IF (PRESENT(channels)) THEN
          CALL rttov_hdf_load(err, file_mfasis_nn1, "/MFASIS_NN", &
                      mfasisnn=coefs_all_chan%coef_mfasis_nn)
          THROW(err.NE.0)

          CALL rttov_channel_extract_mfasis_nn(err, coefs_all_chan%coef_mfasis_nn, &
                                               coefs%coef_mfasis_nn, channels)
          THROW(err.NE.0)

          CALL rttov_dealloc_coef_mfasis_nn(err, coefs_all_chan%coef_mfasis_nn)
          THROW(err.NE.0)
        ELSE
          CALL rttov_hdf_load(err, file_mfasis_nn1, "/MFASIS_NN", &
                      mfasisnn=coefs%coef_mfasis_nn)
          THROW(err.NE.0)
        ENDIF
#endif
      ELSE
        err = errorstatus_fatal
        THROWM(err.NE.0,"Unknown format "//TRIM(form_mfasis_nn1))
      ENDIF
      file_id_mfasis_nn1 = 0_jpim
    ENDIF ! MFASIS-NN
...
!     CHARACTER(LEN=*),   INTENT(IN)           :: form_mfasis_aer1
    ...
    CHARACTER(LEN=*),   INTENT(IN)           :: form_mfasis_nn1
...
!     INTEGER(KIND=jpim), INTENT(IN)           :: file_id_mfasis_aer1
    ...
    INTEGER(KIND=jpim), INTENT(IN)           :: file_id_mfasis_nn1
...
!     ENDIF
    ...
    IF (file_id_mfasis_nn1 > 0) THEN
      IF (.NOT. rttov_cmpuc(form_mfasis_nn1, "hdf5")) THEN
        CALL closelun(err, file_id_mfasis_nn1, file_id_mfasis_nn)
        THROW(err.NE.0)
      ENDIF
    ENDIF
