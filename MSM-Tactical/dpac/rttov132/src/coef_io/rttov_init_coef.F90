...
! If there is no SOLAR_SPECTRUM section then all channels are thermal (useful for v7/v8pred coefs)
  ...
  ELSE
! Interpolate Rayleigh depolarisation coefficient for each solar channel
    ALLOCATE(coef%rayleigh_depol_gamma(coef%fmv_chn), STAT = err)
    THROWM(err.NE.0, "allocation of Solar section")
    DO i = 1, coef%fmv_chn
      IF (coef%ss_val_chn(i) > 0) THEN
        wvl = 10000._jprb / coef%ff_cwn(i)
        ! Extrapolate at constant value beyond bounds of gamma table
        IF (wvl <= ray_depol_wvl(1)) THEN
          coef%rayleigh_depol_gamma(i) = ray_depol_gamma(1)
        ELSEIF (wvl >= ray_depol_wvl(nray_depol)) THEN
          coef%rayleigh_depol_gamma(i) = ray_depol_gamma(nray_depol)
        ELSE
          DO iwvl = 2, nray_depol
            IF (wvl < ray_depol_wvl(iwvl)) THEN
              coef%rayleigh_depol_gamma(i) = &
                ((wvl - ray_depol_wvl(iwvl-1)) * ray_depol_gamma(iwvl) +  &
                 (ray_depol_wvl(iwvl) - wvl) * ray_depol_gamma(iwvl-1)) / &
                (ray_depol_wvl(iwvl) - ray_depol_wvl(iwvl-1))
              EXIT
            ENDIF
          ENDDO
        ENDIF
      ELSE
        coef%rayleigh_depol_gamma(i) = 0
      ENDIF
    ENDDO
  ENDIF
...
! -----------------------------------------------
  ...
  IF (coef%pmc_shift) THEN
    ...
    coef%pmc_ppmc = coef%pmc_pnominal
  ELSE
