...
#include "rttov_channel_extract_sublist.interface"
  ...
  INTEGER(KIND=jpim), ALLOCATABLE :: channel_deff_mixing_mfasis(:) ! List of switches for radius mixing (nchannels_mfasis)
...
!- End of header --------------------------------------------------------
  ...
  ALLOCATE(coef_mfasis%channel_deff_mixing(coef_mfasis%nchannels),stat=err)
  THROWM(err.NE.0,'Error allocating coef_mfasis%channel_deff_mixing')
  IF (coef_mfasis%version > 1) THEN
    ALLOCATE(channel_deff_mixing_mfasis(nchannels_mfasis))
    READ (file_id, iostat=err)channel_deff_mixing_mfasis(:)
    THROWM(err.NE.0, 'reading MFASIS switch list for radius mixing')
    DO j = 1, coef_mfasis%nchannels
      coef_mfasis%channel_deff_mixing(j) = channel_deff_mixing_mfasis(mfasis_ext_index(j))
    ENDDO
  ELSE
    coef_mfasis%channel_deff_mixing(:) = 0
  ENDIF
  ...
  IF (ALLOCATED(channel_deff_mixing_mfasis)) DEALLOCATE(channel_deff_mixing_mfasis)
