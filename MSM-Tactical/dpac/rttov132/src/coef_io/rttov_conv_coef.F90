...
!   CALL getoption( "--mfasis_aer-in", f_mfasis_aer_in, USE="input MFASIS aerosol file name")
  ...
  CALL getoption( "--mfasis_nn-in",  f_mfasis_nn_in,  USE="input MFASIS NN file name")
...
!   CALL getoption( "--mfasis_aer-out", f_mfasis_aer_out, USE="output MFASIS aerosol file name")
  ...
  CALL getoption( "--mfasis_nn-out",  f_mfasis_nn_out,  USE="output MFASIS NN file name")
...
!     IF (f_mfasis_aer_out .EQ. "" .AND. f_mfasis_aer_in .NE. "") f_mfasis_aer_out = TRIM(f_mfasis_aer_in)//'.H5'
    ...
    IF (f_mfasis_nn_out  .EQ. "" .AND. f_mfasis_nn_in  .NE. "") f_mfasis_nn_out = TRIM(f_mfasis_nn_in)//'.H5'
...
!       IF (f_mfasis_aer_in .NE. "") f_mfasis_aer_out = f_coef_out
      ...
      IF (f_mfasis_nn_in  .NE. "") f_mfasis_nn_out = f_coef_out
...
!     IF (f_mfasis_aer_out .EQ. "" .AND. f_mfasis_aer_in .NE. "") f_mfasis_aer_out = TRIM(f_mfasis_aer_in)//'.bin'
    ...
    IF (f_mfasis_nn_out  .EQ. "" .AND. f_mfasis_nn_in  .NE. "") f_mfasis_nn_out = TRIM(f_mfasis_nn_in)//'.bin'
  ...
  IF (f_mfasis_cld_out .NE. "" .AND. f_mfasis_nn_out  .NE. "") THEN
    err = errorstatus_fatal
    THROWM(err.NE.0, "Specify only one of --mfasis_cld-in or --mfasis_nn-in")
  ENDIF
  IF (f_mfasis_cld_out .NE. "") opts%rt_ir%vis_scatt_model = vis_scatt_mfasis
  IF (f_mfasis_nn_out  .NE. "") opts%rt_ir%vis_scatt_model = vis_scatt_mfasis_nn
  IF ((f_mfasis_cld_out .NE. "" .OR. f_mfasis_nn_out  .NE. "") .AND. &
      .NOT. opts%rt_ir%addclouds) THEN
    err = errorstatus_fatal
    THROWM(err.NE.0, "Specify --sccld-in in addition to MFASIS file")
  ENDIF
...
!                           form_mfasis_aer = TRIM(coef_format_in), &
                          ...
                          form_mfasis_nn  = TRIM(coef_format_in), &
...
!                          form_mfasis_aer = TRIM(coef_format_out), &
                         ...
                         form_mfasis_nn  = TRIM(coef_format_out), &
...
!                          file_mfasis_aer = f_mfasis_aer_out,      &
                         ...
                         file_mfasis_nn  = f_mfasis_nn_out,       &
