...
!!
!!   The MFASIS cloud LUT or NN file will only be written if the addclouds option
...
!! @param[in]     file_mfasis_nn     file name of MFASIS NN LUT file, optional
...
!! @param[in]     file_id_mfasis_nn  logiucal unit of MFASIS NN LUT file, optional
...
!INTF_OFF
  ...
  USE rttov_const, ONLY : vis_scatt_mfasis, vis_scatt_mfasis_nn
...
#include "rttov_write_ascii_mfasis_nn_file.interface"
...
#include "rttov_write_binary_mfasis_nn_file.interface"
...
!   INTEGER(KIND=jpim) :: file_id_mfasis_aer1
  ...
  INTEGER(KIND=jpim) :: file_id_mfasis_nn1
...
!     ENDIF
  ...
  IF (opts%rt_ir%addclouds .AND. .NOT. opts%rt_ir%user_cld_opt_param) THEN
    ...
    IF (opts%rt_ir%vis_scatt_model == vis_scatt_mfasis_nn .AND. &
      ...
      CALL getlun(err, file_id_mfasis_nn1, file_mfasis_nn1, form_mfasis_nn1, .TRUE._jplm, "rttov_mfasis_nn", &
                       file_id_mfasis_nn, file_mfasis_nn, form_mfasis_nn, instrument)
      THROW(err.NE.0)
      IF (rttov_cmpuc(form_mfasis_nn1,"unformatted")) THEN
        CALL rttov_write_binary_mfasis_nn_file(err, coefs%coef_mfasis_nn, &
                                               file_id_mfasis_nn1, opts%config%verbose)
        THROW(err.NE.0)
        CALL closelun(err, file_id_mfasis_nn1, file_id_mfasis_nn)
        THROW(err.NE.0)
      ELSE IF (rttov_cmpuc(form_mfasis_nn1,"formatted")) THEN
        CALL rttov_write_ascii_mfasis_nn_file(err, coefs%coef, coefs%coef_mfasis_nn, &
                                              file_id_mfasis_nn1, opts%config%verbose)
        THROW(err.NE.0)
        CALL closelun(err, file_id_mfasis_nn1, file_id_mfasis_nn)
        THROW(err.NE.0)
      ELSE IF (rttov_cmpuc(form_mfasis_nn1,"hdf5")) THEN
        ...
        THROWM(err.NE.0,"This program is not compiled with HDF5 capability; use RTTOV_HDF=1 with Makefile.PL")
#else
        create = .NOT. (TRIM(file_coef) .EQ. TRIM(file_mfasis_nn) .AND. TRIM(file_coef) .NE. "")
        CALL rttov_hdf_save(err, file_mfasis_nn, "/MFASIS_NN", create=create, &
