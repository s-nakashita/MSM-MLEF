...
! ----------------------------------------------------------------------------
  ...
  ALLOCATE(coef_mfasis2%channel_deff_mixing(coef_mfasis2%nchannels),stat=err)
  THROWM(err.NE.0, 'allocation of lut')
  DO j = 1, coef_mfasis2%nchannels
    coef_mfasis2%channel_deff_mixing(j) = coef_mfasis1%channel_deff_mixing(mfasis_ext_index(j))
