! Description:
!> @file
!!   Example calling direct model MFASIS-NN visible cloud simulation.
!
!> @brief
!!   Example calling direct model MFASIS-NN visible cloud simulation.
!!
!! @details
!!   This example is most easily run using the run_example_cld_mfasis_nn_fwd.sh
...
!!     the RTTOV MFASIS-NN coef file
!!
!!   The output is written to a file called example_cld_mfasis_nn_fwd_output.dat
!!
...
!
PROGRAM example_cld_mfasis_nn_fwd
...
#include "rttov_skipcommentline.interface"
  ...
  CHARACTER(LEN=25)  :: NameOfRoutine = 'example_cld_mfasis_nn_fwd'

  ! variables for input
  !====================
  CHARACTER(LEN=256) :: coef_filename
  CHARACTER(LEN=256) :: cld_coef_filename
  CHARACTER(LEN=256) :: mfasis_coef_filename
  ...
  WRITE(0,*) 'enter path of MFASIS-NN coef file'
  READ(*,*) mfasis_coef_filename
  ...
  opts % rt_ir % vis_scatt_model     = &       ! Scattering model for solar source term must be set
                          vis_scatt_mfasis_nn  !   to MFASIS-NN before reading coefficient files
  ...
  ! (unless simulating thermal channels in the same call)
  ...
  CALL rttov_read_coefs(errorstatus, coefs, opts, &
                        ...
                        file_mfasis_nn=mfasis_coef_filename)
  ...
  ! MFASIS-NN coef files are trained for all CLW optical properties, so you can set clw_scheme
  ! to 1 or 2 as desired.
  !
  ! Currently MFASIS-NN is only compatible with the Baum ice properties, so you must set
  ! ice_scheme = 1.

  ! Set the cloud liquid and ice schemes
  profiles(:) % clw_scheme = 2 ! Can be 1 (OPAC) or 2 ("CLW Deff")
  profiles(:) % ice_scheme = 1 ! Must be 1
...
END PROGRAM example_cld_mfasis_nn_fwd
