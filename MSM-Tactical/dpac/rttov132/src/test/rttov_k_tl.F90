...
!
  ...
  emissivity_tl%tskin_eff        = 0._jprb
  ...
  IF (opts%rt_all%use_tskin_eff) THEN
    ...
    ! Per-channel effective Tskin
    ! All channels can be perturbed together with just one call to RTTOV
    ! which is much more efficient than doing each one individually.

    emissivity_tl%emis_in          = 0._jprb
    emissivity_tl%specularity      = 0._jprb
    emissivity_tl%tskin_eff        = 1._jprb
    reflectance_tl%refl_in         = 0._jprb
    reflectance_tl%diffuse_refl_in = 0._jprb
    ...
    DO ichan = 1, nchannels
      IF (do_rttovscatt .OR. &
          ...
          coefs%coef%ss_val_chn(chanprof(ichan)%chan) < 2)) THEN
          IF (do_radar) THEN
            emissivity_k(ichan)%tskin_eff = reflectivity_tl%azef(radar_k_lev,ichan)
          ELSE
            emissivity_k(ichan)%tskin_eff = radiance_tl%bt(ichan)
          ENDIF
      ELSE
        emissivity_k(ichan)%tskin_eff = radiance_tl%total(ichan)
      ENDIF
    ENDDO
