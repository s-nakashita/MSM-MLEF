...
#include "rttov_errorreport.interface"
  ...
  REAL(KIND=jprb), PARAMETER :: dtskin = 0.01_jprb
  ...
  IF (.NOT. opts%htfrtc_opts%htfrtc) THEN
    ...
    emissivity_bf%tskin_eff = emissivity%tskin_eff
    DO ichan = 1, nchannels
  ...
  ! ---------------------------------------------------------------------------
  ! Perturb per-channel effective Tskin
  ! ---------------------------------------------------------------------------
  ! All channels can be perturbed together with just one call to RTTOV
  ! which is much more efficient than doing each one individually.
  ...
  IF (opts%rt_all%use_tskin_eff .AND. .NOT. opts%htfrtc_opts%htfrtc) THEN

    emissivity_bf%emis_in = emisout
    emissivity_bf%tskin_eff = emissivity%tskin_eff + dtskin
    ...
    DO ichan = 1, nchannels
      IF (do_rttovscatt .OR. &
          (opts%rt_all%switchrad .AND. .NOT. do_pc .AND. &
           coefs%coef%ss_val_chn(chanprof(ichan)%chan) < 2)) THEN
        IF (do_radar) THEN
          emissivity_k(ichan)%tskin_eff = (reflectivity_bf%azef(radar_k_lev,ichan) - &
                                           reflectivity%azef(radar_k_lev,ichan)) / dspec
        ELSE
          emissivity_k(ichan)%tskin_eff = (radiance_bf%bt(ichan) - radiance%bt(ichan)) / dspec
        ENDIF
      ELSE
        emissivity_k(ichan)%tskin_eff = (radiance_bf%total(ichan) - radiance%total(ichan)) / dspec
      ENDIF
    ENDDO

  ENDIF ! use_tskin_eff and not HTFRTC
