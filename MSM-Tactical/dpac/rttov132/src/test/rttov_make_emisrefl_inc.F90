...
!! @param[in,out] tskin_eff_inc            computed effective Tskin increments
!
! Copyright:
...
!INTF_ON
  ...
  REAL(jprb),             INTENT(INOUT) :: tskin_eff_inc(SIZE(chanprof))
!INTF_END
  ...
  DO i = 1, nchanprof
    ...
    IF (opts%rt_all%use_tskin_eff) THEN
      tskin_eff_inc(i) = 0.01_jprb * (MODULO(i, 3_jpim) + 1_jpim)
    ELSE
      tskin_eff_inc(i) = 0._jprb
    ENDIF
  ENDDO
