...
#include "throw.h"
  ...
  USE rttov_unix_env, ONLY: rttov_exit, rttov_dirname
  USE rttov_test_mod
...
! coefficients files
    ...
    CHARACTER(LEN=256) :: f_rttovscatt = "", f_mfasis_cld = "", f_mfasis_aer = "", f_mfasis_nn = ""
...
! input files
    ...
    CHARACTER(LEN=256) :: f_pol_filename = "ScalingFactorForBulkProperties.rssp"
...
! what we will do
    ...
    LOGICAL(jplm) :: tskin_eff_from_prof = .FALSE.
    ...
    IF (ts%defn%do_tl) THEN
      ...
      IF (ts%defn%do_print) THEN
        IF (ts%defn%scale_out /= 1.0_jprb) THEN
          ...
          ts%data%tl%emissivity%tskin_eff         = ts%data%tl%emissivity%tskin_eff * ts%defn%scale_out
    ...
    IF (ts%defn%do_ad) THEN
      ...
      IF (ts%defn%do_print) THEN
        IF (ts%defn%scale_out /= 1.0_jprb) THEN
          ...
          ts%data%ad%emissivity%tskin_eff = ts%data%ad%emissivity%tskin_eff * ts%defn%scale_out
...
!             file_mfasis_aer = TRIM(ts%defn%coef_prefix)//'/'//TRIM(ts%defn%f_mfasis_aer), &
            ...
            file_mfasis_nn  = TRIM(ts%defn%coef_prefix)//'/'//TRIM(ts%defn%f_mfasis_nn),  &
        ...
        IF (ts%defn%do_rttovscatt) THEN
          CALL rttov_read_scattcoeffs(err, ts%defn%opts_scatt, ts%data%coefs, ts%data%coefs_scatt, &
              ...
              file_pol  = TRIM(ts%defn%coef_prefix)//'/'// &
                          TRIM(rttov_dirname(TRIM(ts%defn%f_rttovscatt)))//'/'// &
                          TRIM(ts%defn%f_pol_filename))
          THROW(err.NE.0)
...
!
    DO imult = 2, ts%defn%mult
      ...
      ts%data%direct%emissivity(k1:k2)%tskin_eff        = ts%data%direct%emissivity(1:nchannels1)%tskin_eff
    ...
    IF (ts%defn%opts%rt_all%use_tskin_eff) THEN
      IF (ts%defn%tskin_eff_from_prof) THEN
        ts%data%direct%emissivity(:)%tskin_eff = &
            ts%data%direct%profiles(ts%data%chanprof(:)%prof)%skin%t
      ELSE
        DO i = 1, SIZE(ts%data%chanprof)
          ts%data%direct%emissivity(i)%tskin_eff = &
              ts%data%direct%profiles(ts%data%chanprof(i)%prof)%skin%t + &
              0.5 * (MODULO(i, 10_jpim) + 1)
        ENDDO
      ENDIF
    ENDIF
...
!
    ...
    IF (ts%defn%do_tl) THEN
      ...
      CALL rttov_make_emisrefl_inc(ts%defn%opts, ts%data%coefs, ts%data%direct%profiles, &
            ...
            ts%data%tl%emissivity%tskin_eff)
      ...
      IF (ts%defn%scale_inc /= 1.0_jprb) THEN
        ...
        ts%data%tl%emissivity(:)%tskin_eff = ts%data%tl%emissivity(:)%tskin_eff * ts%defn%scale_inc
...
#include "rttov_hdf_save.interface"
  ...
  SUBROUTINE printoutput(err, opts, path, suffix, adk, calcrad2, do_rttovscatt, radar, calcemisterms, data, data_k)
    ...
    IF (opts%rt_all%use_tskin_eff) THEN
      CALL printsurfvar(err, data%emissivity%tskin_eff, "TSKIN_EFF"//TRIM(suffixuc), &
              TRIM(path)//"/tskin_eff"//TRIM(suffixlc)//".txt")
      THROW(err.NE.0)
    ENDIF
