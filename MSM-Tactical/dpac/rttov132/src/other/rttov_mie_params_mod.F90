...
!!   - lognorm_logd: log-normal size distribution function in terms of log10(d)
...
#include "throw.h"
  ...
  PUBLIC :: read_opac_ref_index, read_segelstein_ref_index, &
            ...
            lognorm, lognorm_logd, gammadist, &
            make_radius_grid, calc_mass, &
...
SUBROUTINE lognorm_logd(ntot, rarr, asigma, armod, sqrt2pi, n)
  ...
  DO i = 1, ntot
    r = rarr(i)
    n(i) = (1._jprb / (sqrt2pi * LOG10(asigma))) * &
           EXP(-0.5_jprb * (LOG10(r / armod) / LOG10(asigma))**2._jprb)
  ENDDO

  RETURN
END SUBROUTINE lognorm_logd
