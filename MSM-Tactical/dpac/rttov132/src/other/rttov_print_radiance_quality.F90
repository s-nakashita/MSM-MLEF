...
!INTF_END
  ...
  ELSE
    ...
    IF (BTEST(quality, qflag_mfasis_geometry_bounds)) THEN
      WRITE(iu,'(a)') 'MFASIS warning: scattering angle out of training bounds'
    ENDIF
    IF (BTEST(quality, qflag_mfasis_opdpedia_bounds)) THEN
      WRITE(iu,'(a)') 'MFASIS warning: some optical depth/effective diamater value out of training bounds'
    ENDIF
  ENDIF
