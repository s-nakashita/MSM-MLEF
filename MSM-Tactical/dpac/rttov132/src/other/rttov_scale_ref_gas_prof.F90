...
!! @param[in]     log_p_interp   do vertical interpolation in log(pressure), default false, optional
!
! Copyright:
...
!INTF_ON
  ...
  LOGICAL(jplm),       INTENT(IN), OPTIONAL :: log_p_interp
!INTF_END

#include "rttov_errorreport.interface"

  INTEGER(jpim) :: prof, nlev
  LOGICAL(jplm) :: htfrtc, log_p_int
  ...
  log_p_int = .FALSE.
  IF (PRESENT(log_p_interp)) log_p_int = log_p_interp
  ...
  DO prof = 1, SIZE(profiles)
    ...
    IF ((PRESENT(o3_col_int) .OR. PRESENT(o3_col_int_du) .OR. PRESENT(o3_max_ppmv))) THEN
      ...
      CALL process_gas(err, gas_id_ozone, pref, gasref, &
                       ...
                       profiles(prof)%o3, log_p_int, o3_max_ppmv, o3_col_int, o3_col_int_du)
      THROW(err.NE.0)
    ENDIF
    ...
    IF ((PRESENT(co2_col_int) .OR. PRESENT(co2_max_ppmv))) THEN
      ...
      CALL process_gas(err, gas_id_co2, pref, gasref, &
                       profiles(prof)%gas_units, profiles(prof)%p, profiles(prof)%q, &
                       profiles(prof)%co2, log_p_int, co2_max_ppmv, co2_col_int)
      THROW(err.NE.0)
    ENDIF
    ...
    IF ((PRESENT(n2o_col_int) .OR. PRESENT(n2o_max_ppmv))) THEN
      ...
      CALL process_gas(err, gas_id_n2o, pref, gasref, &
                       profiles(prof)%gas_units, profiles(prof)%p, profiles(prof)%q, &
                       profiles(prof)%n2o, log_p_int, n2o_max_ppmv, n2o_col_int)
      THROW(err.NE.0)
    ENDIF
    ...
    IF ((PRESENT(co_col_int) .OR. PRESENT(co_max_ppmv))) THEN
      ...
      CALL process_gas(err, gas_id_co, pref, gasref, &
                       profiles(prof)%gas_units, profiles(prof)%p, profiles(prof)%q, &
                       profiles(prof)%co, log_p_int, co_max_ppmv, co_col_int)
      THROW(err.NE.0)
    ENDIF
    ...
    IF ((PRESENT(ch4_col_int) .OR. PRESENT(ch4_max_ppmv))) THEN
      ...
      CALL process_gas(err, gas_id_ch4, pref, gasref, &
                       profiles(prof)%gas_units, profiles(prof)%p, profiles(prof)%q, &
                       profiles(prof)%ch4, log_p_int, ch4_max_ppmv, ch4_col_int)
      THROW(err.NE.0)
    ENDIF
    ...
    IF ((PRESENT(so2_col_int) .OR. PRESENT(so2_max_ppmv))) THEN
      ...
      CALL process_gas(err, gas_id_so2, pref, gasref, &
                       profiles(prof)%gas_units, profiles(prof)%p, profiles(prof)%q, &
                       profiles(prof)%so2, log_p_int, so2_max_ppmv, so2_col_int)
      THROW(err.NE.0)
    ENDIF
...
CONTAINS
  ...
  SUBROUTINE interp(p_ref_input, p_out_input, log_p_int, gas_ref, gas_out)

    REAL(jprb),       INTENT(IN)           :: p_ref_input(:)
    REAL(jprb),       INTENT(IN)           :: p_out_input(:)
    LOGICAL(jplm),    INTENT(IN)           :: log_p_int
    REAL(jprb),       INTENT(IN)           :: gas_ref(:)
    REAL(jprb),       INTENT(INOUT)        :: gas_out(:)

    INTEGER(jpim) :: j, k, nlevels_coef, nlevels
    REAL(jprb)    :: p_ref(SIZE(p_ref_input))
    REAL(jprb)    :: p_out(SIZE(p_out_input))

    nlevels = profiles(1)%nlevels
    nlevels_coef = SIZE(p_ref)

    IF (log_p_int) THEN
      p_ref(:) = LOG(p_ref_input(:))
      p_out(:) = LOG(p_out_input(:))
    ELSE
      p_ref(:) = p_ref_input(:)
      p_out(:) = p_out_input(:)
    ENDIF
  ...
  SUBROUTINE process_gas(err, gas_id, p_ref, gas_ref, gas_units, &
                         p_out, q_out, gas_out, log_p_int, &
                         max_ppmv, col_int, col_int_du)
    ...
    IF (PRESENT(col_int) .OR. PRESENT(col_int_du)) THEN
      ...
      CALL interp(p_ref, p_out, log_p_int, gas_ref, gas_out)
