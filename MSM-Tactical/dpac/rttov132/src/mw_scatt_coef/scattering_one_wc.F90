subroutine scattering_one_wc ( wavelength, f_ghz, i_type, permwat, permice, ll_melt, dia_froz, &
  & d_dia_froz, nd, q_ext, q_sct, q_asm, q_bsct, by_dia_density, temp, ext, sct, asm, bsct, diag)
...
!INTF_OFF
use mod_scattering,  only: i_rain, i_snow, i_graupel !, is_frozen
!INTF_ON
implicit none

! Interface
real (kind=jprb),    intent (in   ) :: wavelength, f_ghz, temp
...
real (kind=jprb),    intent (in   ) :: by_dia_density(n_dia)  
...
if (ll_melt .and. (i_type == i_snow .or. i_type == i_graupel)) then
  ...
  call melting_layer (i_type, wavelength, nd, by_dia_density, temp, dia_froz, perm_wat, perm_froz, ext, sct, asm, bsct)
