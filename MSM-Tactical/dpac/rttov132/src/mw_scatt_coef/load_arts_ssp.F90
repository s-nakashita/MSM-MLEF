...
!           31/01/2019  fixed bug that prevented use of multiple ARTS habits (A. Geer)

use parkind1, only: jprb, jpim
use mod_arts, only: nf_max_arts, nT_max_arts, nD_max_arts
!INTF_OFF
use mod_scattering, only:  i_arts, lmax
...
real (kind=jprb), intent (inout) :: f_arts(0:nf_max_arts,m_type)
real (kind=jprb), intent (inout) :: T_arts(0:nT_max_arts,m_type)
real (kind=jprb), intent (inout) :: D_arts(0:nD_max_arts,m_type)
...
do i_type=1,m_type
  ! only in case ARTS-SSP data is used for this hydrometeor, we read the ARTS-SSP data from an rssp habit file
  if (i_scat(i_type) == i_arts) then
    ...
    D_arts(0,i_type) = REAL(nD)
    read(fid, *, iostat=ios) line ! comment line
    read(fid, *, iostat=ios) f_arts(1:nf,i_type)
    read(fid, *, iostat=ios) line ! comment line
    read(fid, *, iostat=ios) T_arts(1:nT,i_type)
    read(fid, *, iostat=ios) line ! comment line
    read(fid, *, iostat=ios) D_arts(1:nD,i_type)
    read(fid, *, iostat=ios) line ! comment line
