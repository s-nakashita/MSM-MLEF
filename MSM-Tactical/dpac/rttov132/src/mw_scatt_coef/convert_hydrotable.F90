subroutine convert_hydrotable(input_file, cversion, cconfig, nconfig, ext_tab, ssa_tab, asm_tab, zef_tab)
...
use mod_scattering, only: lmax, nmaxconfig, lversion
!INTF_OFF
use mod_scattering, only: n_temp, n_lwc, &
 & wc_slope, wc_offset, temp_offset, temp_offset_130, ctype, is_frozen
use rttov_const, only: scattering_coef_filestem, version, len_platform, len_instrument
!INTF_ON
implicit none

character(len=*), intent(in) :: input_file
character (len=lversion), intent(in) :: cversion
...
integer (kind=jpim) :: ifreq, jfreq, itype, itemp, isensor, nside, ii, jj, i_plat, i_inst, iostat

! v13.0 output specifiers
character(len=14) :: cout_130 = '(5(1x,e23.16))'
character(len=18) :: cdim_130 = '(i3,i2,i3,i4,l2,a)'

! v13.2 output specifiers
character(len=13) :: cout_132 = '(5(1x,e15.8))'
character(len=18) :: cdim_132 = '(i3,i2,i4,i4,l2,a)'

character(len=14) :: cfmt_out
character(len=18) :: cfmt_dim
...
! Dealing with variable format expressions is surprisingly hard in Fortran
if (cversion=='13.0') then
  cfmt_out = cout_130
  cfmt_dim = cdim_130
else
  cfmt_out = cout_132
...
do isensor = 1, n_sensor
  ...
  write (20,'(a,a)') cversion,'     ! hydrotable generator format version (but note that all v13 formats are interoperable)'
  call write_divider(20)
  ...
  write (20,'(a)') ' !'
  if(cversion=='13.0') then
    write (20,cfmt_dim) m_freq, m_type, n_temp, n_lwc, l_reflectivity, '        ! frequencies hydrometeor-types temperatures water-contents reflectivity(T/F)'
  else
    write (20,cfmt_dim) m_freq, m_type, n_temp, n_lwc, l_reflectivity, '        ! frequencies hydrometeor-types temperatures water-contents reflectivity(T/F)'
  endif
  call write_divider(20)
  ...
  if(cversion=='13.0') then
    ...
    write (20,'(30I4)') (int(temp_offset_130(id_type(jj))), jj = 1, m_type)
  else
    write (20,'(a)') ' ! offset          ! T [K] = index [1..ntemp] + offset  '
  ...
  do ifreq = 1, m_freq 
    do itype = 1, m_type
      do itemp = 1, n_temp
        write (20,cfmt_out) ext_tab (id_freq (ifreq),itype,itemp,:)
      end do
    end do
  end do
  ...
  do ifreq = 1, m_freq 
    do itype = 1, m_type
      do itemp = 1, n_temp
        write (20,cfmt_out) ssa_tab (id_freq (ifreq),itype,itemp,:)
      end do
    end do
  end do
  ...
  do ifreq = 1, m_freq 
    do itype = 1, m_type
      do itemp = 1, n_temp
        write (20,cfmt_out) asm_tab (id_freq (ifreq),itype,itemp,:)
      end do
    end do
  end do
  ...
  if(l_reflectivity) then
    ...
    do ifreq = 1, m_freq
      do itype = 1, m_type
        do itemp = 1, n_temp
          write (20,cfmt_out) zef_tab (id_freq (ifreq),itype,itemp,:)
        end do
      end do
    end do
  endif
