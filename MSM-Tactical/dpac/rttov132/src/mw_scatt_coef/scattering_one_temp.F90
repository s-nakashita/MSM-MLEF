...
!INTF_OFF
use mod_scattering, only: n_dia, get_lwc, ctype, psd_field_2007, t_fl, t_ml
!INTF_ON
...
do i_lwc = 1, n_lwc
  ...
  call scattering_one_wc( wavelength, f_ghz, i_type, permwat, permice, &
    & (ll_melt .and. nint(temp) >= t_fl .and. nint(temp) <= t_ml), dia_froz, d_dia_froz, nd, &
    & q_ext, q_sct, q_asm, q_bsct, by_dia_density, temp, ext, sct, asm, bsct, diag=diag)
