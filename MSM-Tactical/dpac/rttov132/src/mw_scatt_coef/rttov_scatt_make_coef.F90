...
use mod_scattering, only: n_temp, n_lwc, n_temp_130, n_temp_132, n_lwc_130, n_lwc_132, pi, i_liu, i_arts, &
                   temp_offset, temp_offset_130, lmax, i_rain, modified_gamma_data, diag_data, &
                   nmaxconfig, permwat_rosenkranz_15, lversion, wc_offset, wc_offset_130, wc_offset_132
...
character(len=lversion) :: cversion
...
if(cversion=='13.0') then
  n_lwc  = n_lwc_130
  n_temp = n_temp_130
  wc_offset = wc_offset_130
else
  n_lwc  = n_lwc_132
  n_temp = n_temp_132
  wc_offset = wc_offset_132
endif

read (10,*) ! text after header
...
if(ll_melt .and. cversion == '13.0') then
  write(*,*) 'Melting layer is now only compatible with 13.2 version hydrotables (needs temperatures to 277K)'
  stop
endif
...
print '(a,a)',  ' Hydrotable version = ', cversion
print '(a,i0)', ' Number of temperatures = ', n_temp
print '(a,i0)', ' Number of water contents = ', n_lwc
...
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(i_temp, temp)
      ...
      do i_temp = 1, n_temp
        ...
        if(cversion=='13.0') then
          temp = temp_offset_130(i_type) + i_temp
        else
        ...
        call scattering_one_temp (temp, wavelength, f_ghz, i_type, i_scat(i_mtype), &
          & dens(i_mtype), psd(i_mtype), regime(i_mtype), wperm(i_mtype), iperm(i_mtype), &
...
call convert_hydrotable(input_file, cversion, cconfig, nconfig, ext_tab, ssa_tab, asm_tab, zef_tab)
