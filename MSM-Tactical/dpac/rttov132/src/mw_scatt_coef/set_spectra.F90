...
!           21/09/2022  Include Abel&Boutle and Illingworth&Blackmann PSDs (Rohit Mangla/Alan Geer)
...
use mod_scattering,  only: i_aggregate, &
               ...
               & i_clw, i_ciw, modified_gamma_fixed, psd_modified_gamma, psd_abel_boutle, &
               & psd_illingworth_blackman, psd_heymsfield_2013, psd_mh97, t_0c, x1, x2, &
               & density_wat, pi
!INTF_ON
...
elseif (psd == psd_abel_boutle) then

  mu = 0.0_jprb
  lambda = (pi * density_wat * x1 * gamma(4+mu)/(6 * lwc)) ** (1.0_jprb/(4+mu-x2) )
  n0 = x1 * (lambda ** x2)

  call modified_gamma(n_dia, dia_froz, lwc, modified_gamma_fixed, n0, mu, lambda, 1.0_jprb, a, b, nd)

elseif (psd == psd_illingworth_blackman) then
 
  mu = 5.0_jprb
  n0_ib = 8e6_jprb
  d0_ib = ((175.55_jprb * lwc) / (pi * density_wat * n0_ib) ) ** (0.25_jprb)
  lambda = (3.67_jprb + mu)/ d0_ib
  n0 = (0.03_jprb * n0_ib * (d0_ib**4)* (lambda**(mu+4)))/ gamma(4+mu)
