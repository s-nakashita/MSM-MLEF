...
#include "rttov_hdf_load.interface"
    ...
    ! NB The final (numwave) snow PC scores in the current eigenvector file
    !    are not correct so replace them with the values from (numwave-1):
    atlas%pcu_snow(:,numwave) = atlas%pcu_snow(:,numwave-1)
    ...
    !------------------------------------------------------------
      ...
      IF (instr_brdf_flag > 0) THEN
        ...
        IF (atlas%single_inst) THEN
          ...
          ! Allow extrapolation at constant value to UV
          WHERE (instr_wavenum < hsr_wavenum(1))
            instr_brdf = hkod
          ENDWHERE
  ...
  SUBROUTINE rttov_brdf_select_wavenum( &
    ...
    IF (lcpu_brdf) THEN
      ...
      DO j = 1, nchs
        ...
        IF (instr_wavenum(j) > hsr_wavenum(numwave)) THEN
          ! Extrapolate at constant value for UV

          instr_brdf(j) = hsrbrdf(numwave)

        ELSEIF (instr_wavenum(j) >= hsr_wavenum(1)) THEN
