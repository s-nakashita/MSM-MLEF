...
Subroutine rttov_mieproc (&        
  ...
  ! - Moreau, E., P. Bauer and F. Chevallier, 2002: Microwave radiative transfer modeling
  !     in clouds and precipitation. Part II: Model evaluation.
  ...
  !  1.12      06/2022   Improved polarised scattering (V. Barlakas)
  !  1.13      08/2022   Add backscatter and coef_scatt source of polarised scattering (A. Geer)
  !
  ! Code Description:
  !   Language:           Fortran 90.
          ...
          ntype_loop2: do itype = 1, coef_scatt % nhydro
            ...
            if (iwc(itype) >= 1) then
              ...
              if (opts_scatt%zero_hydro_tlad .or. wc(itype) > 1E-10_JPRB) then
                ap = s_a * wc(itype)  
                gp = s_g * wc(itype) 
                if (lbackscatter) zp = s_z * wc(itype)
              else
                ap = 0.0_JPRB
                gp = 0.0_JPRB
                zp = 0.0_JPRB 
