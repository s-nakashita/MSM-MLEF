...
!! @param[in]     adk_switch       explicitly activate AD (0) or K (1) behaviour, optional
!
! Copyright:
...
!* Subroutine arguments:
  ...
  Integer (Kind=jpim), optional, Intent (in) :: adk_switch ! 0 => AD, 1 => K, if omitted determine from size(profiles_ad)
!INTF_END
...
!* Local variables:
  ...
  if (present(adk_switch)) then
    adk = adk_switch
    if (.not. (adk == adk_adjoint .or. adk == adk_k)) then
      errorstatus = errorstatus_fatal
      Write( errMessage, '( "invalid adk_switch value")' )
    ...
    if (.not. (adk == adk_adjoint .and. nprofilesad == nprofiles .or. &
               adk == adk_k .and. nprofilesad == nchannels)) then
      errorstatus = errorstatus_fatal
  ...
  if( opts_scatt%pol_mode == pol_mode_aro_scaled .and. .not. &
    & (associated(coef_scatt%coef_pol_v) .and. associated(coef_scatt%coef_pol_h) .and. &
    &  associated(coef_scatt%z_table)) ) then

    errorstatus = errorstatus_fatal
    write( errMessage, '( "ARO polarised scattering mode requires an additional coef file")' )
    call rttov_errorreport (errorstatus_fatal, errMessage, NameOfRoutine)
    IF (LHOOK) CALL DR_HOOK('RTTOV_SCATT',1_jpim,ZHOOK_HANDLE)
    return
  ...
  do ichan = 1, nchannels
    ...
    lthermal(ichan) = coef_rttov%coef%ff_val_chn(ichan_act) /= 0
  enddo
...
!*         1.   Gas absorption
  ...
  opts%rt_mw%fastem3_rwd_fix          = opts_scatt%fastem3_rwd_fix
  ...
  opts%rt_all%use_tskin_eff           = opts_scatt%use_tskin_eff
