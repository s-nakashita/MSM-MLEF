...
#include "rttov_errorreport.interface"
  ...
  THROW(err .NE. 0)
  IF (ASSOCIATED(coef_scatt%coef_pol_v)) DEALLOCATE(coef_scatt%coef_pol_v, STAT = err)
  THROW(err .NE. 0)
  IF (ASSOCIATED(coef_scatt%coef_pol_h)) DEALLOCATE(coef_scatt%coef_pol_h, STAT = err)
  THROW(err .NE. 0)
  IF (ASSOCIATED(coef_scatt%z_table)) DEALLOCATE(coef_scatt%z_table, STAT = err)
  THROW(err .NE. 0)
