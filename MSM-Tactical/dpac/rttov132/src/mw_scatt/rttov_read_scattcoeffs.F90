...
!!
!!   If the pol_mode option is set to pol_mode_aro_scaled then this
!!   routine will additionally read the ARO scaling table which by default is
!!   named "ScalingFactorForBulkProperties.rssp". The full path for this file
!!   may be specified using the file_pol argument. If file_pol is omitted, the
!!   code will assume the default file name and will look in the directory
!!   specified in path (if supplied) or otherwise in the current directory.
!!   Note that the file format of the table cannot be changed.
!!
...
!! @param[in]      file_pol     file name and path of ARO scaling table, optional
!
! Copyright:
...
Subroutine rttov_read_scattcoeffs  (&
  ...
  !   1.9    08/2022      Add polarisation scaling table (A Geer)
!INTF_OFF
#include "throw.h"
!INTF_ON
...
#include "rttov_read_polcoeffs.interface"
    ...
    read_ascii_file: Do
        ...
        Do i = 1, coef_scatt%mfreqm
          Do j = 1, coef_scatt%mtype
            Do k = 1, coef_scatt%mtemp
              Read(file_lu,*) coef_scatt % ext(i,j,k,:)
            Enddo
          Enddo
        Enddo
      ...
      Case( 'ALBEDO')
        ...
        Do i = 1, coef_scatt%mfreqm
          Do j = 1, coef_scatt%mtype
            Do k = 1, coef_scatt%mtemp
              Read(file_lu,*) coef_scatt % ssa(i,j,k,:)
            Enddo
          Enddo
        Enddo
      ...
      Case( 'ASYMMETRY')
        ...
        Do i = 1, coef_scatt%mfreqm
          Do j = 1, coef_scatt%mtype
            Do k = 1, coef_scatt%mtemp
              Read(file_lu,*) coef_scatt % asp(i,j,k,:)
            Enddo
          Enddo
        Enddo
      ...
      Case( 'REFLECTIVITY')
        ...
        Do i = 1, coef_scatt%mfreqm
          Do j = 1, coef_scatt%mtype
            Do k = 1, coef_scatt%mtemp
              Read(file_lu,*) coef_scatt % zef(i,j,k,:)
            Enddo
          Enddo
        Enddo
  ...
  ! Load scaling table for polarised scattering if required
  if (opts_scatt%pol_mode == pol_mode_aro_scaled) then
    call rttov_read_polcoeffs(err, opts_scatt, coef_scatt, path, file_pol)
    THROWM(err.NE.0, "Error reading polarisation LUT file")
  endif
