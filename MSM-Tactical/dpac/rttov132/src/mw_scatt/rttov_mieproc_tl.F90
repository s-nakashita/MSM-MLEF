...
Subroutine rttov_mieproc_tl (&        
  ...
  !  1.11      08/2022   ARO scaled polarised scattering TL/AD (A. Geer)
  !
  ! Code Description:
  !   Language:           Fortran 90.
...
!INTF_OFF
  ...
  use rttov_const, only : min_reflectivity, pol_mode_no_pol, &
            ...
            if (ladd_polarisation .and. coef_scatt%is_frozen(itype)) then
              ...
              elseif (opts_scatt % pol_mode == pol_mode_aro_scaled) then
                ...
                if (coef_rttov%fastem_polar(ichanid) == 1) then ! Nominal V at nadir rotating with view angle QV
                  ...
                  kp_tl = kp_tl*(p_extv*angles(iprof)%cosview_sq + p_exth*angles(iprof)%sinview_sq)
                  ap_tl = ap_tl*(p_ssav*angles(iprof)%cosview_sq + p_ssah*angles(iprof)%sinview_sq)   
                  gp_tl = gp_tl*(p_asyv*angles(iprof)%cosview_sq + p_asyh*angles(iprof)%sinview_sq)   
                  if (lbackscatter) then
                    zp = zp*(p_bscv*angles(iprof)%cosview_sq + p_bsch*angles(iprof)%sinview_sq)
                    zp_tl = zp_tl*(p_bscv*angles(iprof)%cosview_sq + p_bsch*angles(iprof)%sinview_sq)
                  endif
                elseif (coef_rttov%fastem_polar(ichanid) == 2) then ! Nominal H at nadir rotating with view angle QH
                  ...
                  kp_tl = kp_tl*(p_extv*angles(iprof)%sinview_sq + p_exth*angles(iprof)%cosview_sq)   
                  ap_tl = ap_tl*(p_ssav*angles(iprof)%sinview_sq + p_ssah*angles(iprof)%cosview_sq)   
                  gp_tl = gp_tl*(p_asyv*angles(iprof)%sinview_sq + p_asyh*angles(iprof)%cosview_sq)  
                  if (lbackscatter) then
                    zp = zp*(p_bscv*angles(iprof)%sinview_sq + p_bsch*angles(iprof)%cosview_sq)
                    zp_tl = zp_tl*(p_bscv*angles(iprof)%sinview_sq + p_bsch*angles(iprof)%cosview_sq)
                  endif 
