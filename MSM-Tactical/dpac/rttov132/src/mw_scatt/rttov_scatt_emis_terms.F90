...
!INTF_END
  ...
  do ichan = 1, nchannels
    ! Clear sky terms
    emis_retrieval_terms%tau_clr(ichan)  = transmission%tau_total(ichan)

    ! AJGDB scatt_aux%emis_cld used (wrongly?) at ECMWF
    emis_retrieval_terms%up_clr(ichan)   = radiance%upclear(ichan) - &
      & scatt_aux%bsfc(ichan) * transmission%tau_total(ichan) * emissivity(ichan)%emis_out

    emis_retrieval_terms%down_clr(ichan) = radiance%dnclear(ichan)
 
    ! Surface black body radiance (possibly, as with other radiances, incorporating band correction)
    emis_retrieval_terms%bsfc(ichan) = scatt_aux%bsfc(ichan)
