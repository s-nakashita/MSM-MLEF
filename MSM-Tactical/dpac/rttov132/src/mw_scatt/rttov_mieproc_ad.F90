...
Subroutine rttov_mieproc_ad (&        
  ...
  !  1.14   08/2022      ARO-scaled polarised scattering TL/AD (A. Geer)
  !
  ! Code Description:
  !   Language:           Fortran 90.
...
!INTF_OFF
  ...
  use rttov_const, only : adk_adjoint, adk_k, min_reflectivity, pol_mode_no_pol, &
    & pol_mode_empirical, pol_mode_aro_scaled
!INTF_ON
  Implicit None
...
!* Subroutine arguments:
  ...
  Type (rttov_geometry),          Intent (in)    :: angles (nprofiles)   ! Zenithal angles
...
!* Local variables:
  ...
  Logical (Kind=jplm) :: ladd_polarisation, lpolactive, lbackscatter
  ...
  Real(Kind=jprb)     :: zen                                       ! lwc per hydrometeor, zenithal angle
  Real(kind=jprb)     :: p_extv,p_ssav,p_asyv,p_exth,p_ssah,p_asyh ! Polarised bulk properties
  Real(kind=jprb)     :: p_bscv,p_bsch                             ! Polarised bulk properties
  Integer(Kind=jpim)  :: zi                                        ! LUT index for zenith angle
  !- End of header --------------------------------------------------------
  ...
  lbackscatter = associated(coef_scatt % zef)

  ladd_polarisation = opts_scatt % pol_mode /= pol_mode_no_pol
  lpolactive = opts_scatt % ice_polarisation > 0.0_jprb
  if (lpolactive) then
      ...
      if( scatt_aux % cfrac(iprof) > opts_scatt % cc_threshold ) then
        ...
        if(iprof /= iprof_last) then
          ...
          ntype_loop1: do itype = 1, coef_scatt % nhydro
            ...
            if (itemp(itype) > coef_scatt % mtemp - 1) itemp(itype) = coef_scatt % mtemp
        ...
        if(ifreq /= ifreq_last .or. iprof /= iprof_last .or. (ladd_polarisation .and. (ichanid /= ichanid_last))) then
          ...
          ntype_loop2: do itype = 1, coef_scatt % nhydro
            ...
            if (ladd_polarisation .and. coef_scatt%is_frozen(itype)) then
              if (opts_scatt % pol_mode == pol_mode_empirical .and. lpolactive) then
              ...
              elseif (opts_scatt % pol_mode == pol_mode_aro_scaled) then

                if (iwc(itype) >= 1) then

                  ! Retrieve scaling of bulk properties from the LUT
                  zi = minloc(abs(coef_scatt%z_table - zen), 1)
                  p_extv = coef_scatt%coef_pol_v(ifreq, itemp(itype), iwc(itype), zi, 1)
                  p_asyv = coef_scatt%coef_pol_v(ifreq, itemp(itype), iwc(itype), zi, 2)
                  p_ssav = coef_scatt%coef_pol_v(ifreq, itemp(itype), iwc(itype), zi, 3)
                  p_bscv = coef_scatt%coef_pol_v(ifreq, itemp(itype), iwc(itype), zi, 4)
                  p_exth = coef_scatt%coef_pol_h(ifreq, itemp(itype), iwc(itype), zi, 1)
                  p_asyh = coef_scatt%coef_pol_h(ifreq, itemp(itype), iwc(itype), zi, 2)
                  p_ssah = coef_scatt%coef_pol_h(ifreq, itemp(itype), iwc(itype), zi, 3)
                  p_bsch = coef_scatt%coef_pol_h(ifreq, itemp(itype), iwc(itype), zi, 4)
                ...
                ! Polarised bulk properties are obtained by scaling
                if (coef_rttov%fastem_polar(ichanid) == 1) then ! Nominal V at nadir rotating with view angle QV
                  kp = kp*(p_extv*angles(iprof)%cosview_sq + p_exth*angles(iprof)%sinview_sq)
                  ap = ap*(p_ssav*angles(iprof)%cosview_sq + p_ssah*angles(iprof)%sinview_sq)   
                  gp = gp*(p_asyv*angles(iprof)%cosview_sq + p_asyh*angles(iprof)%sinview_sq)  
                  if (lbackscatter) zp = zp*(p_bscv*angles(iprof)%cosview_sq + p_bsch*angles(iprof)%sinview_sq)
                elseif (coef_rttov%fastem_polar(ichanid) == 2) then ! Nominal H at nadir rotating with view angle QH
                  kp = kp*(p_extv*angles(iprof)%sinview_sq + p_exth*angles(iprof)%cosview_sq)   
                  ap = ap*(p_ssav*angles(iprof)%sinview_sq + p_ssah*angles(iprof)%cosview_sq)   
                  gp = gp*(p_asyv*angles(iprof)%sinview_sq + p_asyh*angles(iprof)%cosview_sq) 
                  if (lbackscatter) zp = zp*(p_bscv*angles(iprof)%sinview_sq + p_bsch*angles(iprof)%cosview_sq) 
                elseif (coef_rttov%fastem_polar(ichanid) == 3) then ! Pure V 
                  kp = kp*p_extv
                  ap = ap*p_ssav
                  gp = gp*p_asyv
                  if (lbackscatter) zp = zp*p_bscv
                elseif (coef_rttov%fastem_polar(ichanid) == 4) then ! Pure H
                  kp = kp*p_exth
                  ap = ap*p_ssah
                  gp = gp*p_asyh
                  if (lbackscatter) zp = zp*p_bsch
                endif
        ...
        ntype_loop4: do itype = coef_scatt % nhydro, 1, -1
          ...
          if (ladd_polarisation .and. coef_scatt%is_frozen(itype)) then
            ...
            elseif (opts_scatt % pol_mode == pol_mode_aro_scaled) then
              ...
              if (iwc(itype) >= 1) then
                ...
                ! Retrieve scaling of bulk properties from the LUT
                zi = minloc(abs(coef_scatt%z_table - profiles(iprof)%zenangle), 1)
                p_extv = coef_scatt%coef_pol_v(ifreq, itemp(itype), iwc(itype), zi, 1)
                p_asyv = coef_scatt%coef_pol_v(ifreq, itemp(itype), iwc(itype), zi, 2)
                p_ssav = coef_scatt%coef_pol_v(ifreq, itemp(itype), iwc(itype), zi, 3)
                p_bscv = coef_scatt%coef_pol_v(ifreq, itemp(itype), iwc(itype), zi, 4)
          ...
          if (ladd_polarisation .and. coef_scatt%is_frozen(itype)) then
            ...
            elseif (opts_scatt % pol_mode == pol_mode_aro_scaled) then
              ...
              ! Polarised bulk properties are obtained by scaling
              if (coef_rttov%fastem_polar(ichanid) == 1) then ! Nominal V at nadir rotating with view angle QV
                kp_ad = kp_ad*(p_extv*angles(iprof)%cosview_sq + p_exth*angles(iprof)%sinview_sq)
                ap_ad = ap_ad*(p_ssav*angles(iprof)%cosview_sq + p_ssah*angles(iprof)%sinview_sq)   
                gp_ad = gp_ad*(p_asyv*angles(iprof)%cosview_sq + p_asyh*angles(iprof)%sinview_sq)  
                if (lbackscatter) zp_ad = zp_ad*(p_bscv*angles(iprof)%cosview_sq + p_bsch*angles(iprof)%sinview_sq) 
              elseif (coef_rttov%fastem_polar(ichanid) == 2) then ! Nominal H at nadir rotating with view angle QH
                kp_ad = kp_ad*(p_extv*angles(iprof)%sinview_sq + p_exth*angles(iprof)%cosview_sq)   
                ap_ad = ap_ad*(p_ssav*angles(iprof)%sinview_sq + p_ssah*angles(iprof)%cosview_sq)   
                gp_ad = gp_ad*(p_asyv*angles(iprof)%sinview_sq + p_asyh*angles(iprof)%cosview_sq) 
                if (lbackscatter) zp_ad = zp_ad*(p_bscv*angles(iprof)%sinview_sq + p_bsch*angles(iprof)%cosview_sq)
              elseif (coef_rttov%fastem_polar(ichanid) == 3) then ! Pure V 
                kp_ad = kp_ad*p_extv
                ap_ad = ap_ad*p_ssav
                gp_ad = gp_ad*p_asyv
                if (lbackscatter) zp_ad = zp_ad*p_bscv
              elseif (coef_rttov%fastem_polar(ichanid) == 4) then ! Pure H
                kp_ad = kp_ad*p_exth
                ap_ad = ap_ad*p_ssah
                gp_ad = gp_ad*p_asyh
                if (lbackscatter) zp_ad = zp_ad*p_bsch
              endif
          ...
          if (iwc(itype) >= 1) then
            ...
            if (opts_scatt%zero_hydro_tlad) then
              ...
              if( kp_store > 1E-10_JPRB) wc_ad = wc_ad + s_k * kp_ad
            endif
            kp_ad = 0.0_JPRB
