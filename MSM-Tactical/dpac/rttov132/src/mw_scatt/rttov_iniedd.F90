Subroutine rttov_iniedd (&        
  ...
  !            12/2010   Allow tau computation even if no cloud (A. Geer)
...
!* Local variables
  ...
  if (lhook) call dr_hook('RTTOV_INIEDD',0_jpim,zhook_handle)
      ...
      if (scatt_aux % cfrac(iprof) > ccthres) then
        ...
        if (ilayer >= scatt_aux % mclayer (ichan)) then
          ...
          scatt_aux % h      (ichan,ilayer) = 1.5_JPRB * scatt_aux % ext (ichan,ilayer) &
                                    & * (1.0_JPRB - scatt_aux % ssa (ichan,ilayer) * scatt_aux % asm (ichan,ilayer))
          if (scatt_aux % h (ichan,ilayer) <= 0.00001_JPRB) scatt_aux % h  (ichan,ilayer) = 0.00001_JPRB    

        endif

      endif

      ! Make sure to compute Tau even if cfrac < ccthres, i.e. no cloud signal
  ...
  if (lhook) call dr_hook('RTTOV_INIEDD',1_jpim,zhook_handle)
