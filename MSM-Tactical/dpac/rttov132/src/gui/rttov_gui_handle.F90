...
SUBROUTINE RTTOV_GUI_HANDLELOAD( RTH, channels, file_coef, file_scaer, file_sccld, file_mfasis_cld, file_mfasis_nn, &
  file_pccoef, ERR )
  ...
  Character(len=*),   Intent(in)  :: file_mfasis_nn 
...
TRY
  ...
  RTH%file_mfasis_nn = file_mfasis_nn
  ...
  ENDIF 
  ! if both mfasis_nn and Mfasis_cld prefers nn
  IF (file_mfasis_nn .NE. "") THEN
     RTH%opts%rt_ir%vis_scatt_model = 4
  ENDIF
