! Description:
!> @file
!!   Clip profile variables to regression limits.
!
!> @brief
!!   Clip profile variables to regression limits.
!!
...
!! @param[in,out] prof           profiles structure on coefficient levels
...
!
SUBROUTINE rttov_apply_reg_limits( &
        opts,        &
        prof_user,   &
        prof,        &
        coef,        &
...
!INTF_ON
  ...
  TYPE(rttov_profile),     INTENT(INOUT) :: prof(:)      ! Profiles on coef levels (ppmv dry)
...
!- End of header --------------------------------------------------------
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_APPLY_REG_LIMITS',0_jpim,ZHOOK_HANDLE)
  ...
  DO iprof = 1, nprofiles
      ...
      IF (opts%rt_all%so2_data .AND. coef%nso2 > 0) THEN
        ...
        DO ilev = 1, firstlevel
          IF (prof(iprof)%so2(ilev) > coef%lim_prfl_gmax(ilev, ig)) THEN
            prof(iprof)%so2(ilev) = coef%lim_prfl_gmax(ilev, ig)
          ELSE IF (prof(iprof)%so2(ilev) < coef%lim_prfl_gmin(ilev, ig)) THEN
            prof(iprof)%so2(ilev) = coef%lim_prfl_gmin(ilev, ig)
          ENDIF
        ENDDO
      ENDIF
    ...
    ELSE ! addpc
        ...
        IF (opts%rt_all%co2_data .AND. coef%nco2 > 0) THEN
          DO ilev = 1, firstlevel
            IF (prof(iprof)%co2(ilev) > coef_pccomp%co2_pc_max(ilev)) THEN
              prof(iprof)%co2(ilev) = coef_pccomp%co2_pc_max(ilev)
            ELSE IF (prof(iprof)%co2(ilev) < coef_pccomp%co2_pc_min(ilev)) THEN
              prof(iprof)%co2(ilev) = coef_pccomp%co2_pc_min(ilev)
            ENDIF
          ENDDO
        ENDIF
        ...
        IF (opts%rt_all%n2o_data .AND. coef%nn2o > 0) THEN
          DO ilev = 1, firstlevel
            IF (prof(iprof)%n2o(ilev) > coef_pccomp%n2o_pc_max(ilev)) THEN
              prof(iprof)%n2o(ilev) = coef_pccomp%n2o_pc_max(ilev)
            ELSE IF (prof(iprof)%n2o(ilev) < coef_pccomp%n2o_pc_min(ilev)) THEN
              prof(iprof)%n2o(ilev) = coef_pccomp%n2o_pc_min(ilev)
            ENDIF
          ENDDO
        ENDIF
        ...
        IF (opts%rt_all%co_data .AND. coef%nco > 0) THEN
          DO ilev = 1, firstlevel
            IF (prof(iprof)%co(ilev) > coef_pccomp%co_pc_max(ilev)) THEN
              prof(iprof)%co(ilev) = coef_pccomp%co_pc_max(ilev)
            ELSE IF (prof(iprof)%co(ilev) < coef_pccomp%co_pc_min(ilev)) THEN
              prof(iprof)%co(ilev) = coef_pccomp%co_pc_min(ilev)
            ENDIF
          ENDDO
        ENDIF
        ...
        IF (opts%rt_all%ch4_data .AND. coef%nch4 > 0) THEN
          DO ilev = 1, firstlevel
            IF (prof(iprof)%ch4(ilev) > coef_pccomp%ch4_pc_max(ilev)) THEN
              prof(iprof)%ch4(ilev) = coef_pccomp%ch4_pc_max(ilev)
            ELSE IF (prof(iprof)%ch4(ilev) < coef_pccomp%ch4_pc_min(ilev)) THEN
              prof(iprof)%ch4(ilev) = coef_pccomp%ch4_pc_min(ilev)
            ENDIF
          ENDDO
        ENDIF
      ...
      IF (wind < coef_pccomp%lim_pc_prfl_wsmin) THEN
        IF (wind > 0._jprb) THEN
          prof(iprof)%s2m%u = prof(iprof)%s2m%u * coef_pccomp%lim_pc_prfl_wsmin / wind
          prof(iprof)%s2m%v = prof(iprof)%s2m%v * coef_pccomp%lim_pc_prfl_wsmin / wind
        ELSE
          prof(iprof)%s2m%u = SQRT(0.5_jprb * coef_pccomp%lim_pc_prfl_wsmin**2)
          prof(iprof)%s2m%v = SQRT(0.5_jprb * coef_pccomp%lim_pc_prfl_wsmin**2)
        ENDIF
      ELSE IF (wind > coef_pccomp%lim_pc_prfl_wsmax) THEN
        prof(iprof)%s2m%u = prof(iprof)%s2m%u * coef_pccomp%lim_pc_prfl_wsmax / wind
        prof(iprof)%s2m%v = prof(iprof)%s2m%v * coef_pccomp%lim_pc_prfl_wsmax / wind
      ENDIF
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_APPLY_REG_LIMITS',1_jpim,ZHOOK_HANDLE)

END SUBROUTINE rttov_apply_reg_limits
