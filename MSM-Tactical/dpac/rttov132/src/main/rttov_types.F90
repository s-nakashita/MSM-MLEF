...
MODULE rttov_types
  ...
  TYPE rttov_opts_config
    ...
    LOGICAL(jplm) :: opdep13_gas_clip = .TRUE.   !< Switch for v13pred individual gas -ve optical depth clipping
  ...
  TYPE rttov_opts_rt_all
    ...
    LOGICAL(jplm) :: transmittances_only    = .FALSE.  !< Switch to only compute transmittances, no radiances
    LOGICAL(jplm) :: use_tskin_eff          = .FALSE.  !< Switch to enable per-channel effective Tskin inputs
  ...
  TYPE rttov_opts_rt_ir
    ...
    LOGICAL(jplm) :: rayleigh_depol          = .FALSE.   !< Rayleigh scattering includes depolarisation impact in phase fn
  END TYPE

  !> MW-only radiative transfer options
  TYPE rttov_opts_rt_mw
    INTEGER(jpim) :: fastem_version        = 6         !< FASTEM version (0-7); 0 => TESSEM2, 7 => SURFEM-Ocean
    LOGICAL(jplm) :: fastem3_rwd_fix       = .TRUE.    !< Switch to enable the FASTEM-3 relative wind direction bug fix
  ...
  TYPE rttov_options_scatt
    ...
    INTEGER(jpim) :: pol_mode = pol_mode_empirical         !< Polarisation mode (0-2, see rttov_const)
    REAL(jprb)    :: ice_polarisation      = 1.40_jprb     !< Polarised scattering factor for ice hydrometeors for empirical
                                                           !!   pol_mode; <0 = no polarisation
    LOGICAL(jplm) :: ozone_data            = .FALSE.       !< Switch to enable input of O3 profile
    LOGICAL(jplm) :: use_t2m_opdep         = .TRUE.        !< Switch to enable use of 2m T variable in optical depth calculation
    LOGICAL(jplm) :: use_q2m               = .TRUE.        !< Switch to enable use of 2m q variable
    LOGICAL(jplm) :: addrefrac             = .TRUE.        !< Switch to enable atmospheric refraction
  ...
  TYPE rttov_coef
    ...
    REAL(jprb),    POINTER :: rayleigh_depol_gamma(:) => NULL() ! Rayleigh depolarisation coefficient gamma
  ...
  TYPE rttov_scatt_coef
    ...
    REAL(jprb), POINTER :: coef_pol_v(:,:,:,:,:) => NULL() ! Scaling factor for polarised ARO scattering - v channels
    REAL(jprb), POINTER :: coef_pol_h(:,:,:,:,:) => NULL() ! Scaling factor for polarised ARO scattering - h channels
    REAL(jprb), POINTER :: z_table(:) => NULL() ! Zenith angle dimension of the polarisation coefs
  ...
  TYPE rttov_coef_mfasis
    ...
    INTEGER(jpim), POINTER :: channel_deff_mixing(:)=> NULL() ! switch for special treatment of NIR channels
    TYPE(rttov_mfasis_lut), POINTER :: lut(:)       => NULL() ! LUT(s) for each supported channel (nchannels)
  END TYPE rttov_coef_mfasis

  TYPE rttov_mfasis_nn_params
    REAL(jprb)          :: max
    REAL(jprb)          :: min
    CHARACTER(LEN=32)   :: name
    CHARACTER(LEN=32)   :: transform
    INTEGER(jpim)       :: nauxparams
    REAL(jprb), POINTER :: auxparams(:)  => NULL() ! container for preprocessing parameters, e.g. 2layer parameterization
                                                   ! currently fac_tlo, fac_thi, fac_zen1, fac_zen2
  END TYPE rttov_mfasis_nn_params

  TYPE rttov_mfasis_nn
    TYPE(rttov_mfasis_nn_params), POINTER :: in(:)  => NULL()
    TYPE(rttov_mfasis_nn_params), POINTER :: out(:) => NULL()
    INTEGER(jpim)       :: actfunc_hl  ! activation function used in all hidden layers
    INTEGER(jpim)       :: actfunc_ol  ! activation function used in output layer
    REAL(jprb), POINTER :: bias_i(:)       => NULL()  ! ( n_nodes_max )
    REAL(jprb), POINTER :: bias_h(:,:)     => NULL()  ! ( n_nodes_max, n_hidden-1 )
    REAL(jprb), POINTER :: bias_o(:)       => NULL()  ! ( n_output )
    REAL(jprb), POINTER :: weight_i(:,:)   => NULL()  ! (  n_input, n_nodes_max)
    REAL(jprb), POINTER :: weight_h(:,:,:) => NULL()  ! ( n_nodes_max, n_nodes_max, n_hidden-1 )
    REAL(jprb), POINTER :: weight_o(:,:)   => NULL()  ! ( n_nodes_max, n_output )
    INTEGER(jpim)       :: n_input     ! number of input nodes
    INTEGER(jpim)       :: n_output    ! number of output nodes
    INTEGER(jpim)       :: n_hidden    ! number of hidden layers
    INTEGER(jpim)       :: n_nodes_max ! maximum number of nodes in layers
    INTEGER(jpim)       :: nn_version
  END TYPE rttov_mfasis_nn

  !> @internal MFASIS-NN structure
  TYPE rttov_coef_mfasis_nn
    INTEGER(jpim)          :: version                       ! version number, may be useful for handling
                                                            !   updates to NN format later and checking
                                                            !   compatibility with code
    INTEGER(jpim)          :: file_type                     ! 1=>clouds, 2=>aerosols (in future: 3=>clouds+aerosols)
    INTEGER(jpim)          :: creation_date(3)              ! year, month, day
    CHARACTER(LEN=132)     :: readme_nn(100)                ! description of NN creation, etc
    ...
    INTEGER(jpim), POINTER :: channel_list(:)     => NULL() ! supported channel numbers (nchannels)
    INTEGER(jpim)          :: nchannels_coef                ! number of channels in the associated rtcoef file
    INTEGER(jpim), POINTER :: channel_nn_index(:) => NULL() ! index of each supported channel in nn(:)  (nchannels_coef)
    TYPE(rttov_mfasis_nn), POINTER :: nn(:)       => NULL() ! NN(s) for each supported channel (nchannels)
  END TYPE rttov_coef_mfasis_nn
  ...
  TYPE rttov_coefs
    ...
    TYPE(rttov_coef_mfasis_nn) :: coef_mfasis_nn
...
!
    ...
    LOGICAL(jplm)          :: do_solar_opdep_calc  ! flag to indicate RTTOV gas optical depth calculation required
                                                   !   for solar channels
    LOGICAL(jplm)          :: do_mfasis_lut        ! flag to indicate MFASIS-LUT is being used
    LOGICAL(jplm)          :: do_mfasis_nn         ! flag to indicate MFASIS-NN is being used
    LOGICAL(jplm)          :: do_mfasis            ! flag to indicate MFASIS-LUT/NN is being used
