! Description:
!> @file
!!   Clip aerosol profiles to PC-RTTOV regression limits AD.
!
!> @brief
!!   Clip aerosol profiles to PC-RTTOV regression limits AD.
!!
...
!! @param[in,out] profiles_int_ad   profiles in RTTOV internal units
...
!- End of header --------------------------------------------------------
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_APPLY_PC_AER_REG_LIMS_AD',0_jpim,ZHOOK_HANDLE)
    ...
    DO lay = 1, nlayers
      ...
      DO t = 1, naer_coef
        ...
        IF (t > coef_pccomp%fmv_pc_naer_types) THEN
          ...
          profiles_int_ad(prof)%aerosols(t,lay) = 0._jprb
        ...
        ELSE
          ...
          ELSE IF (aer_ref(t,lay,prof) < aer_min(t,lay,prof)) THEN
            ...
            IF (opts%config%apply_reg_limits) profiles_int_ad(prof)%aerosols(t,lay) = 0._jprb
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_APPLY_PC_AER_REG_LIMS_AD',1_jpim,ZHOOK_HANDLE)

END SUBROUTINE rttov_apply_pc_aer_reg_lims_ad
