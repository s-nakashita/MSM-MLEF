...
#define od_singlelayer transmission_aux%thermal_path1%Od_singlelayer(coli,lay,i)
  ...
  !----------------------------
  IF (dothermal) CALL rttov_calcrad_tl(chanprof, profiles, profiles_tl, coef, &
                                       thermal, emissivity_tl, auxrad, auxrad_tl)
  ...
  !cdir nodep
    DO i = 1, nchanprof
      IF (thermal(i) .AND. .NOT. dom_ir) THEN
        ...
        ENDDO
      ELSEIF (solar(i) .AND. .NOT. do_mfasis .AND. .NOT. dom_vis .AND. .NOT. opts%rt_ir%addaerosl) THEN
    ...
    DO i = 1, nchanprof
      ...
      DO lev = 2, nlevels
        ...
        IF (opts%rt_ir%rayleigh_depol) THEN
          ! Account for depolarisation
          gamma = coef%rayleigh_depol_gamma(chan)
          ray_phase_tl = 2._jprb * (1._jprb - gamma) * cosscata_tl * cosscata * &
                         0.75_jprb / (1._jprb + 2._jprb * gamma)
        ELSE
          ! Simple phase fn, no depolarisation
