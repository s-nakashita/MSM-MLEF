...
#include "rttov_mfasis_nn.interface"
...
!- End of header ------------------------------------------------------
  ...
  IF (opts%htfrtc_opts%htfrtc) THEN
    ...
    ! Nothing more to do for HTFRTC so return
    RETURN
  ENDIF
...
!-----------------------------------------------------------------------
! Populate Tskin array used within RTTOV
!-----------------------------------------------------------------------
  IF (traj0_sta%dothermal .AND. .NOT. opts%rt_all%use_tskin_eff) THEN
    emissivity(:)%tskin_eff = profiles(chanprof(:)%prof)%skin%t
  ENDIF
...
!-----------------------------------------------------------------------
  IF (opts%config%do_checkinput) THEN
    CALL rttov_check_profiles(err, opts, coefs, traj0_sta%dothermal, traj0_sta%do_mfasis, &
                              traj0_sta%do_mfasis_lut, chanprof, profiles, traj0%profiles_int, &
                              radiance, emissivity, aer_opt_param, cld_opt_param)
    THROW(err.NE.0)
  ENDIF
...
!------------------------------------------------------------------------
  ...
  ! then deallocate data and return. In particular this can happen when using
  ! the parallel interface. Do this after we have populated the geometric_height
  ! output to ensure consistency in outputs via the parallel interface.
  IF (.NOT. (traj0_sta%dothermal .OR. traj0_sta%dosolar)) THEN
    CALL cleanup()
    RETURN
  ENDIF
...
!-------------------------------------------------
  ...
  ! If radiances are not required, deallocate data and return - direct only
...
!---------------------------------------------------------
  IF (traj0_sta%do_mfasis) THEN
    IF (traj0_sta%do_mfasis_lut) THEN
      CALL rttov_mfasis( &
              err,                           &
              chanprof,                      &
              traj0_sta%solar,               &
      ...
      THROW(err.NE.0)
    ELSE IF (traj0_sta%do_mfasis_nn) THEN
      CALL rttov_mfasis_nn( &
              err,                           &
              chanprof,                      &
              traj0_sta%solar,               &
              opts,                          &
