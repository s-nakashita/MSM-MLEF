...
#define chan chanprof(i)%Chan
  ...
  !-------------------------------------------------------------------------------
  IF (dothermal) CALL rttov_calcrad_k(chanprof, profiles, profiles_k, coef, &
                                      thermal, emissivity_k, auxrad, auxrad_k)
  ...
  SUBROUTINE solar_rayleigh_k(raytracing, raytracing_k, ircld, profiles, profiles_k, profiles_dry, &
    ...
    DO i = 1, nchanprof
      ...
      IF (profiles(prof)%s2m%p >= opts%rt_ir%rayleigh_min_pressure) THEN
        ...
        ! First use of cossacata_k:
        IF (opts%rt_ir%rayleigh_depol) THEN
          ! Account for depolarisation
          gamma = coef%rayleigh_depol_gamma(chan)
          cosscata_k = 2._jprb * (1._jprb - gamma) * ray_phase_k * cosscata * &
                       0.75_jprb / (1._jprb + 2._jprb * gamma)
        ELSE
          ! Simple phase fn, no depolarisation
          cosscata_k = 2._jprb * 0.75_jprb * ray_phase_k * cosscata
        ENDIF
