...
!!   AD/K of optical depth calculation (v13 predictors).
!!
!! @param[in]     opdep13_gas_clip    switch for individual gas -ve opdep clipping
...
!INTF_OFF
  USE rttov_const, ONLY : adk_adjoint
  USE yomhook, ONLY : lhook, dr_hook, jphook
!INTF_ON
  IMPLICIT NONE

  LOGICAL(jplm),             INTENT(IN)    :: opdep13_gas_clip
...
!-----------
    IF (coef%nso2 > 0) THEN
      IF (ASSOCIATED(fast_coef(chan)%so2)) THEN
        ...
        od_ad = odtot_ad
        IF (opdep13_gas_clip) WHERE(opdp_ref%od_so2_ref(:,j) < 0._jprb) od_ad = 0._jprb
...
!-----------
    IF (coef%nch4 > 0) THEN
      IF (ASSOCIATED(fast_coef(chan)%ch4)) THEN
        ...
        od_ad = odtot_ad
        IF (opdep13_gas_clip) WHERE(opdp_ref%od_ch4_ref(:,j) < 0._jprb) od_ad = 0._jprb
...
!-----------
    IF (coef%nco > 0) THEN
      IF (ASSOCIATED(fast_coef(chan)%co)) THEN
        ...
        od_ad = odtot_ad
        IF (opdep13_gas_clip) WHERE(opdp_ref%od_co_ref(:,j) < 0._jprb) od_ad = 0._jprb
...
!-----------
    IF (coef%nn2o > 0) THEN
      IF (ASSOCIATED(fast_coef(chan)%n2o)) THEN
        ...
        od_ad = odtot_ad
        IF (opdep13_gas_clip) WHERE(opdp_ref%od_n2o_ref(:,j) < 0._jprb) od_ad = 0._jprb
...
!-----------
    IF (coef%nco2 > 0) THEN
      IF (ASSOCIATED(fast_coef(chan)%co2)) THEN
        ...
        od_ad = odtot_ad
        IF (opdep13_gas_clip) WHERE(opdp_ref%od_co2_ref(:,j) < 0._jprb) od_ad = 0._jprb
...
!------------------------------
    IF (coef%nwvcont > 0) THEN
      IF (ASSOCIATED(fast_coef(chan)%wvcont)) THEN
        ...
        od_ad = odtot_ad
        IF (opdep13_gas_clip) WHERE(opdp_ref%od_wvcont_ref(:,j) < 0._jprb) od_ad = 0._jprb
...
!-------------
    IF (coef%nozone > 0) THEN
      IF (ASSOCIATED(fast_coef(chan)%ozone)) THEN
        ...
        od_ad = odtot_ad
        IF (opdep13_gas_clip) WHERE(opdp_ref%od_o3_ref(:,j) < 0._jprb) od_ad = 0._jprb
...
!--------------------
    IF (ASSOCIATED(fast_coef(chan)%watervapour)) THEN
      ...
      od_ad = odtot_ad
      IF (opdep13_gas_clip) WHERE(opdp_ref%od_wv_ref(:,j) < 0._jprb) od_ad = 0._jprb
...
!--------------------------
    IF (ASSOCIATED(fast_coef(chan)%mixedgas)) THEN
      ...
      od_ad = odtot_ad
      IF (opdep13_gas_clip) WHERE(opdp_ref%od_mg_ref(:,j) < 0._jprb) od_ad = 0._jprb
