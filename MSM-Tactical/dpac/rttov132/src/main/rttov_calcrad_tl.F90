...
!! @param[in]     emissivity_tl    emissivity perturbations
...
!INTF_ON
  ...
  TYPE(rttov_emissivity),   INTENT(IN)    :: emissivity_tl(:)
...
!- End of header --------------------------------------------------------
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_CALCRAD_TL', 0_jpim, ZHOOK_HANDLE)
  ...
  DO ichan = 1, nchanprof
    ...
    IF (coef%ff_val_bc) THEN
       t_effective_skin_tl = coef%ff_bcs(chan) * emissivity_tl(ichan)%tskin_eff
       t_effective_s2m_tl = coef%ff_bcs(chan) * profiles_tl(prof)%s2m%t
       t_effective_air_tl = coef%ff_bcs(chan) * profiles_tl(prof)%t(:)
    ELSE
       t_effective_skin_tl = emissivity_tl(ichan)%tskin_eff
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_CALCRAD_TL', 1_jpim, ZHOOK_HANDLE)
