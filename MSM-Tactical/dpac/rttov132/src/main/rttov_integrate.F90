...
#define surf_fac transmission_aux%Surf_fac(col,i)
  ...
  !-------------------------------------------------------------------------------
  IF (dothermal) CALL rttov_calcrad(addcosmic, chanprof, profiles, coef, &
                                    thermal, emissivity, auxrad)
  ...
  ! only for non-DOM and non-PC simulations if the rad2 parameter is present

  IF (PRESENT(rad2) .AND. dothermal .AND. .NOT. dom_ir .AND. .NOT. keyradonly) THEN
    !cdir nodep
    DO i = 1, nchanprof
      IF (thermal(i)) THEN
        col = 0
