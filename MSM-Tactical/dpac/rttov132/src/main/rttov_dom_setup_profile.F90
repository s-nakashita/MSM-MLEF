...
!INTF_END
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_DOM_SETUP_PROFILE', 0_jpim, ZHOOK_HANDLE)
...
!   %laymap(nlayers)              # index of user layer for each DOM layer containing scattering material
  ...
  DO i = 1, nchanprof
    ...
    IF (opts%rt_ir%addclouds) THEN
      ...
      ENDWHERE
    ENDIF
    WHERE (ssa(:,:,i) >= 1._jprb - 1.E-6_jprb) ssa(:,:,i) = 1._jprb - 1.E-6_jprb
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_DOM_SETUP_PROFILE', 1_jpim, ZHOOK_HANDLE)
