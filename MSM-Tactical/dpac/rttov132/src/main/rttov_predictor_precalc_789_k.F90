! Description:
!> @file
!!   K of v7, v8 and v9 predictor pre-calculations.
!
!> @brief
!!   K of v7, v8 and v9 predictor pre-calculations.
!!
...
!
SUBROUTINE rttov_predictor_precalc_789_k( &
              opts,           &
              dosolar,        &
              plane_parallel, &
              chanprof,       &
...
!- End of header --------------------------------------------------------
  IF (LHOOK) CALL DR_HOOK('RTTOV_PREDICTOR_PRECALC_789_K', 0_jpim, ZHOOK_HANDLE)
...
!-----------------------------------------------------------------------------------------
  ...
  IF (coef%pmc_shift) CALL calc_ssu_co2_thickness_k()  

  ! t_layer shouldn't be initialised yet but the zeeman code for SSMIS does do this so don't overwrite
  IF (.NOT. (coef%id_inst == inst_id_ssmis .AND. coef%inczeeman)) aux_k%t_layer = 0._jprb
  ...
  IF (coef%nco > 0 .AND. opts%rt_all%co_data) THEN
    DO i = 1, nchannels
      ...
      aux_k%cor(:,i) = &!aux_k%cor + & 
                              aux%cow_r4rt(:, prof) *                   aux_k%corw_r4rt(:,i) + &
                              aux%cow_rsqrt(:, prof) * (aux%cow_rsqrt(:, prof) * aux_k%corw_r(:,i) + &
                                                               aux_k%corw_rsqrt(:,i))

      aux_k%cow_r4rt(:,i) = &!aux_k%cow_r4rt + &
                      aux%cor(:, prof) * aux_k%corw_r4rt(:,i)

      aux_k%cow_rsqrt(:,i) = &!aux_k%cow_rsqrt(:,i) + &
                                          aux%cor(:, prof) * (2.0_jprb * aux%cow_rsqrt(:, prof) * aux_k%corw_r(:,i) + &
                                                                                aux_k%corw_rsqrt(:,i))
    ENDDO
    CALL sqrt_k(aux%cow_r4rt, aux_k%cow_rsqrt, aux_k%cow_r4rt, acc = .TRUE._jplm, map = map, prof_stat = prof_stat)
  ...
  IF (coef%fmv_model_ver <= 8) THEN
    CALL sqrt_k(aux%tw_4rt, aux_k%tw_sqrt, aux_k%tw_4rt, acc = .FALSE._jplm, map = map, prof_stat = prof_stat)  
    CALL sqrt_k(aux%tw_sqrt, aux_k%tw, aux_k%tw_sqrt, acc = .TRUE._jplm, map = map, prof_stat = prof_stat)  
  ENDIF

  IF (coef%fmv_model_ver >= 8) &
  ...
  IF (coef%fmv_model_ver >= 8) THEN
    ...
    IF (coef%fmv_model_ver == 9) THEN
      DO i = 1, nchannels
        ...
        sum1_k   = 0._jprb
        DO lay = nlayers, 2, -1
          sum1_k = sum1_k + coef%tstar_uwsum_r(lay) * aux_k%tuw(lay, i)
          aux_k%t_layer(lay, i) = aux_k%t_layer(lay, i) + sum1_k         
        ENDDO
        
        aux_k%t_layer(1, i) = aux_k%t_layer(1, i) + &
                              coef%tstar_r(1) * aux_k%tuw(1, i) + sum1_k
      ENDDO
    ENDIF
    ...
    DO i = 1, nchannels
      ...
      DO lay = nlayers, 2, -1
        IF (coef%fmv_model_ver == 8) THEN
          sum2_k = sum2_k + aux_k%twr(lay, i) * coef%tstarmod_wsum_r(lay)
          aux_k%t_layer(lay-1, i) = aux_k%t_layer(lay-1, i) + coef%dpp(lay - 1) * sum2_k
        ELSE
          sum2_k = sum2_k + coef%tstar_wsum_r(lay) * aux_k%twr(lay, i)
          aux_k%t_layer(lay, i) = aux_k%t_layer(lay, i) + coef%dpp(lay - 1) * sum2_k
        ENDIF
      ENDDO
      
      IF (coef%fmv_model_ver == 8) THEN
        aux_k%twr(1, i) = 0.0_jprb
  ...
  DO i = 1, nchannels
    ...
    aux_k%tr(:, i) = aux_k%tr(:, i) + 2.0_jprb * aux%tr(:, prof) * aux_k%tr2(:, i)
  ...
  aux_k%dt(:,:) = aux_k%dt(:,:) + 2.0_jprb * ABS(aux%dt(:, map(:,1))) * aux_k%dtabsdt(:, :)

  IF (coef%fmv_model_ver == 9) THEN
    DO i = 1, nchannels
      aux_k%t_layer(:, i) = aux_k%t_layer(:, i) + aux_k%dt(:, i) + &
                                                  aux_k%tro(:, i) * coef%to3star_r(:) + &
                                                  aux_k%dto(:, i)
    ENDDO
  ELSEIF (coef%fmv_model_ver < 9) THEN
    IF (coef%nozone > 0) THEN
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_PREDICTOR_PRECALC_789_K', 1_jpim, ZHOOK_HANDLE)
...
CONTAINS
  ...
  SUBROUTINE calc_ssu_co2_thickness_k
    ...
    REAL(jprb) :: pstd
    REAL(jprb) :: tstd
    REAL(jprb) :: pa(profiles(1)%nlayers),    pa_k(profiles(1)%nlayers)
    REAL(jprb) :: dpa(profiles(1)%nlayers),   dpa_k(profiles(1)%nlayers)
    REAL(jprb) :: rho(profiles(1)%nlayers),   rho_k(profiles(1)%nlayers)
    REAL(jprb) :: temp(profiles(1)%nlayers),  temp_k(profiles(1)%nlayers)
    REAL(jprb) :: co2(profiles(1)%nlayers),   co2_k(profiles(1)%nlayers)
    REAL(jprb) :: dz, dz_k
    REAL(jprb) :: dzstp_k
    INTEGER(jpim) :: lay, prof, i
...
! fwd
        ...
        rho(lay)  = (raytracing%dmair(lay,prof) + raytracing%dmair(lay+1,prof)) / 2._jprb
        temp(lay) = (profiles(prof)%t(lay) + profiles(prof)%t(lay+1)) / 2._jprb
        co2(lay)  = (profiles(prof)%co2(lay) + profiles(prof)%co2(lay+1)) / 2._jprb
! ad
        pa_k(lay)   = 0._jprb
        dpa_k(lay)  = 0._jprb
        rho_k(lay)  = 0._jprb
...
! ad
        dzstp_k = 0._jprb
        dzstp_k = dzstp_k + aux_k%co2_cm(i)
        dz_k = 0._jprb
        dz_k =  dz_k + &
          dzstp_k * pa(lay)/pstd * tstd/temp(lay)* co2(lay)*1.E-6_jprb*100._jprb

        IF (opts%interpolation%lgradp) THEN
          pa_k(lay) = pa_k(lay) + &
            dzstp_k * dz / pstd * tstd/temp(lay)* co2(lay)*1.E-6_jprb*100._jprb
!        ELSE
!          pa_k(lay) = 0._jprb
        ENDIF
        ...
        co2_k(lay) = co2_k(lay) + &
          dzstp_k * dz * pa(lay)/pstd * tstd/temp(lay)*1.E-6_jprb*100._jprb
!        dzstp_k = 0._jprb
...
!        dz_k = 0._jprb
        ...
        profiles_k(i)%co2(lay) = profiles_k(i)%co2(lay) + &
          co2_k(lay)/2._jprb
        profiles_k(i)%co2(lay+1) = profiles_k(i)%co2(lay+1) + &
          co2_k(lay)/2._jprb
!        co2_k(lay) = 0._jprb
...
!        temp_k(lay) = 0._jprb
        ...
        raytracing_k%dmair(lay,i) = raytracing_k%dmair(lay,i) + rho_k(lay) / 2._jprb
        raytracing_k%dmair(lay+1,i) = raytracing_k%dmair(lay+1,i) + rho_k(lay) / 2._jprb
...
END SUBROUTINE rttov_predictor_precalc_789_k
