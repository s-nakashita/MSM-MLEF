...
!! @param[in]     emissivity       RTTOV emissivity structure
...
!- End of header ------------------------------------------------------
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_CALCRAD', 0_jpim, ZHOOK_HANDLE)
...
! Calculate effective temperatures, store in auxrad
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_CALCRAD', 1_jpim, ZHOOK_HANDLE)
