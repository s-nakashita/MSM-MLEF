...
!! @param[in]     do_mfasis                   flag to indicate MFASIS-LUT/NN simulation
...
SUBROUTINE rttov_integrate_ad(opts, maxncolumns, chanprof, emissivity, emissivity_ad, &
                              ...
                              do_lambertian, do_mfasis, thermal, dothermal, solar, &
                              dosolar, do_rayleigh_ss, solar_spectrum, &
...
#define chan chanprof(i)%Chan
  ...
  keyradonly = opts%rt_ir%pc%addpc .OR. (dom_ir .AND. dom_vis)
    ...
    DO i = 1, nchanprof
      IF (thermal(i) .AND. .NOT. dom_ir) THEN
        DO lay = 1, nlayers
          ...
          ENDIF
        ENDDO
      ELSE IF (solar(i) .AND. .NOT. do_mfasis .AND. .NOT. dom_vis .AND. .NOT. opts%rt_ir%addaerosl) THEN
  ...
  !-------------------------------------------------------------------------------
  IF (dothermal) CALL rttov_calcrad_ad(chanprof, profiles, profiles_ad, coef, &
                                       thermal, emissivity_ad, auxrad, auxrad_ad)
...
#define B1_2 rad_air_diff(lay) * dtau(lay) * transmission_aux%thermal_path1%od_singlelayer_r(coli,lay,i)
  ...
  SUBROUTINE solar_rayleigh_ad(raytracing, raytracing_ad, ircld, profiles, profiles_ad, profiles_dry, &
    ...
    REAL(jprb) :: gamma, ray_phase, solar_src, solar_src_updn
    ...
    DO i = 1, nchanprof
      ...
      IF (profiles(prof)%s2m%p >= opts%rt_ir%rayleigh_min_pressure) THEN
        ...
        IF (opts%rt_ir%rayleigh_depol) THEN
          ...
          ray_phase = (1._jprb + 3._jprb * gamma + (1._jprb - gamma) * cosscata**2) * &
        ...
        ! First use of cossacata_ad:
        IF (opts%rt_ir%rayleigh_depol) THEN
          ! Account for depolarisation
          gamma = coef%rayleigh_depol_gamma(chan)
          cosscata_ad = 2._jprb * (1._jprb - gamma) * ray_phase_ad * cosscata * &
                        0.75_jprb / (1._jprb + 2._jprb * gamma)
        ELSE
          ! Simple phase fn, no depolarisation
          cosscata_ad = 2._jprb * 0.75_jprb * ray_phase_ad * cosscata
        ENDIF
