...
!! @param[in,out] emissivity_ad    emissivity increments
...
!- End of header --------------------------------------------------------
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_CALCRAD_AD', 0_jpim, ZHOOK_HANDLE)
  ...
  DO ichan = 1, nchanprof
    ...
    IF (coef%ff_val_bc) THEN
      emissivity_ad(ichan)%tskin_eff = emissivity_ad(ichan)%tskin_eff + &
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_CALCRAD_AD', 1_jpim, ZHOOK_HANDLE)
