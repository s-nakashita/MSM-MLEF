...
!! @param[in,out] foam_fraction_k       user foam fraction increment
!
! Copyright:
...
!INTF_ON
    ...
    REAL(fp),      INTENT(INOUT) :: Foam_Fraction_k
!INTF_END
...
!    REAL(fp) ::
  ...
  ! ad part
    ...
    REAL(jprb)    :: u19, u19_k, freq_ghz, dfreq
    ...
    REAL(jprb)    :: tbfixed_k(4,4,3)
    REAL(jprb)    :: efixed_k(4,4,3)
    REAL(jprb)    :: einterpolated_k(4,3)
    REAL(jprb)    :: a1e_k, a2e_k, a3e_k
...
! freq.
  ...
  !  ****** start of adjoint code ******
    ...
    IF (fastem_version == 6) THEN
      zreflmod_v_k = -0.5_jprb * Reflectivity_k(3) * Emissivity(3) - &
                      0.5_jprb * Reflectivity_k(4) * Emissivity(4)
      zreflmod_h_k = zreflmod_v_k
      Emissivity_k(3) = Emissivity_k(3) - &
        0.5_jprb * (zreflmod_v + zreflmod_h) * Reflectivity_k(3)
      Emissivity_k(4) = Emissivity_k(4) - &
        0.5_jprb * (zreflmod_v + zreflmod_h) * Reflectivity_k(4)
    ELSE
      ! FASTEM-4/5 set Stokes 3/4 refl to zero: this is not recommended
      Reflectivity_k(3:4) = ZERO
    ENDIF
  ...
  IF (fastem_version == 6) THEN  !M.Kazumori azimuth model function
    ...
    DO istokes = 3, 4
      DO M = 1, 3
        einterpolated_k(istokes, M) =      &
          & Azimuth_Emi_k(istokes) * sin(m * phi) * (1.0_JPRB - cos_z) / (1.0_JPRB - 0.6018_JPRB)
        phi_k = phi_k +                                                                                    &
          & Azimuth_Emi_k(istokes) * einterpolated(istokes, M) * m * cos(m * phi) * (1.0_JPRB - cos_z) /  &
    ...
    DO M = 1, 3
      IF (freq_ghz .LE. freqfixed(1)) THEN
        efixed_k(1, 3:4, M) = efixed_k(1, 3:4, M) + einterpolated_k(3:4, M)
      ELSE IF (freq_ghz .GE. freqfixed(4)) THEN
        efixed_k(4, 3:4, M) = efixed_k(4, 3:4, M) + einterpolated_k(3:4, M)
      ELSE
        IF (freq_ghz .LT. freqfixed(2)                                 ) ifreq = 2
        IF (freq_ghz .LT. freqfixed(3) .AND. freq_ghz .GE. freqfixed(2)) ifreq = 3
        IF (freq_ghz .GE. freqfixed(3)                                 ) ifreq = 4
        dfreq = (freq_ghz - freqfixed(ifreq - 1)) / (freqfixed(ifreq) - freqfixed(ifreq - 1))
        efixed_k(ifreq, 3:4, M)     = efixed_k(ifreq, 3:4, M) + einterpolated_k(3:4, M) * dfreq
        efixed_k(ifreq - 1, 3:4, M) = efixed_k(ifreq - 1, 3:4, M) + einterpolated_k(3:4, M) * (1.0 - dfreq)
      ENDIF
      DO istokes = 3, 4
    ...
    DO ich = 0, 15
      ...
      istokes = mod(ich, 4_jpim) + 1
      a3e_k    = tbfixed_k(istokes, i_freq, 3)
      a2e_k    = tbfixed_k(istokes, i_freq, 2)
      a1e_k    = tbfixed_k(istokes, i_freq, 1)
      u19_k    =      &
