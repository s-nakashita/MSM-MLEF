...
#include "rttov_calc_solar_spec_esd.interface"
  ...
  opts_coef%rt_mw%clw_data  = .FALSE.
  ...
  IF (asw == 1_jpim) THEN
    ...
    traj_sta%do_mfasis_lut = (opts%rt_ir%addaerosl .OR. opts%rt_ir%addclouds) .AND. &
                             opts%rt_ir%vis_scatt_model == vis_scatt_mfasis .AND. &
                             opts%rt_ir%addsolar
    traj_sta%do_mfasis_nn = (opts%rt_ir%addaerosl .OR. opts%rt_ir%addclouds) .AND. &
                            opts%rt_ir%vis_scatt_model == vis_scatt_mfasis_nn .AND. &
                            opts%rt_ir%addsolar
    traj_sta%do_mfasis = traj_sta%do_mfasis_lut .OR. traj_sta%do_mfasis_nn
        ...
        DO j = 1, nchanprof
          ...
          IF (profiles(prof)%sunzenangle >= 0.0_jprb .AND. &
            ...
            IF (opts%rt_ir%vis_scatt_model == vis_scatt_mfasis) THEN
              ...
              ENDIF
            ELSEIF (opts%rt_ir%vis_scatt_model == vis_scatt_mfasis_nn) THEN
              IF (opts%rt_ir%addclouds) THEN
                traj_sta%solar(j) = traj_sta%solar(j) .AND. &
                                    coefs%coef_mfasis_nn%channel_nn_index(chan) > 0
              ENDIF
            ENDIF
...
CONTAINS
  ...
  SUBROUTINE alloc_opdp_ref(err, od_ref)
    ...
    IF (asw == 1) THEN
      ...
      ALLOCATE(od_ref%od_tot_ref(nlayers_coef, nchanprof), STAT = err)
      THROW(err.NE.0)

      IF (opts%config%opdep13_gas_clip) THEN
        ALLOCATE(od_ref%od_mg_ref(nlayers_coef, nchanprof), &
