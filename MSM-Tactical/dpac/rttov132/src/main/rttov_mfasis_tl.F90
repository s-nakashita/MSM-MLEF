...
! REAL(jprb)                 :: iw_wv(3)
  ...
  REAL(jprb),    ALLOCATABLE :: od_v1(:,:), od_eff_v1(:), od_mpcorr_v1(:,:)
  REAL(jprb),    ALLOCATABLE :: od_tl(:,:), od_eff_tl(:), ed_tl(:,:), ed_eff_tl(:), ed_eff_v1(:), od_mpcorr_tl(:,:)
  REAL(jprb),    ALLOCATABLE :: odlay(:), odlay_tl(:)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  REAL(jprb),    ALLOCATABLE :: ed_eff_sscat_tl(:), fac_rm_tl(:)
  REAL(jprb),    ALLOCATABLE :: w_sscat(:,:)
  REAL(jprb),    ALLOCATABLE :: w_sscat_tl(:,:)
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_MFASIS_TL', 0_jpim, ZHOOK_HANDLE)
  ...
  ALLOCATE( od_eff    (npar)     , & ! Effective total optical depth per type of particle
            ...
            od_tl  (npar, nlay)  , & ! Effective optical depth per type of particle and layer
            ...
            od_mpcorr_tl(npar, nlay), &! Effective mixed-phase corrected optical depth per type of particle and layer
            odlay   (nlay)          , &   ! Effective mixed-phase corrected optical depth of water and ice
            odlay_tl(nlay)          , &   ! Effective mixed-phase corrected optical depth of water and ice
            STAT=err)
  THROWM(err.NE.0,"Allocation of memory for rttov_mfasis_tl failed")
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              ...
              w_sscat_tl(npar, nlay), &  ! Weighting factor for radius mixing approach (1.6um channel)
              ed_eff_sscat(npar), &      ! Effective diameter contribution in radius mixing approach (1.6um channel)
              ed_eff_sscat_tl(npar),  &  ! Effective diameter contribution in radius mixing approach (1.6um channel)
              fac_rm(npar),    &         ! Weighting factor for radius mixing approach (1.6um channel)
              fac_rm_tl(npar), &         ! Weighting factor for radius mixing approach (1.6um channel)
              q_mxr(nlev),         &
              q_mxr_tl(nlev),      &
              dvap (nlay),         &
              dvap_tl (nlay),      &
...
!==================================================================================
      ...
      IF ( mfasis_coefs%file_type .EQ. mfasis_cld ) THEN ! clouds
        ...
        DO j = 1, nlay
          ...
          IF ( ircld%icldarr(cc,j,prof) .EQ. 1 ) THEN
            ...
            IF(abs(ooo1(j)/opts%dev%o_del1) < 5._jprb) &
             ...
             od_mpcorr_tl(1,j) = od_tl(1,j) + fac_mx_tl(j)*od(2,j) + fac_mx(j)*od_tl(2,j)
             od_mpcorr_tl(2,j) = od_tl(2,j) - fac_mx_tl(j)*od(2,j) - fac_mx(j)*od_tl(2,j)
            IF (od_mpcorr_v1(2,j) < 0) od_mpcorr_tl(2,j) = 0._jprb
        ...
        !-----------------------------------------------------------------------
        ! water: use original optical depths for computing the effective diameter, then overwrite od_eff(1)
        od_eff_tl(1) = SUM( od_tl(1,:))
        IF ( od_eff_v1(1) .GT. 0. ) THEN
          ed_eff_tl(1) = SUM( od_tl(1,:)*ed(1,:) + od(1,:)*ed_tl(1,:))/od_eff_v1(1)   &
                           - ed_eff_v1(1) * od_eff_tl(1)/od_eff_v1(1)
        ELSE
          ed_eff_tl(1) = 0._jprb
        ENDIF
        od_eff_tl(1) = SUM( od_mpcorr_tl(1,:) )

        ! ice: use mixed-phase corrected optical depths for computing the effective diameter
        od_eff_tl(2) = SUM( od_mpcorr_tl(2,:))
        IF ( od_eff_v1(2) .GT. 0. ) THEN
          ed_eff_tl(2) = SUM( od_mpcorr_tl(2,:)*ed(2,:) + od_mpcorr(2,:)*ed_tl(2,:))/od_eff_v1(2) &
                           - ed_eff_v1(2) * od_eff_tl(2)/od_eff_v1(2)
        ELSE
          ed_eff_tl(2) = 0._jprb
        ENDIF
        ...
        IF (mfasis_coefs%channel_deff_mixing(chan) .EQ. 1) THEN
          ...
          IF (od_eff(2) > 0.1_jprb) THEN ! exclude special treatment for very thin clouds
            DO j = 1, nlay
              IF ( ircld%icldarr(cc,j,prof) .EQ. 1 ) THEN ! this check is probably redundant, od_eff > 0!
                odlay_tl(j) = od_mpcorr_tl(1,j) + od_mpcorr_tl(2,j)
                odint_tl = odint_tl + odlay_tl(j) ! total integraged optical depth (mpc corr.)
                IF ( odlay(j) > 1E-10 .and. a > 1E-10 ) THEN
                  w_sscat_tl(2,j) = a/odlay(j) * (EXP((odlay(j) - odint)/a) - EXP(-odint/a)) * od_mpcorr_tl(2,j) &
                                  + ( - a* od_mpcorr(2,j)/(odlay(j)**2) * (EXP((odlay(j) - odint)/a) - EXP(-odint/a)) &
                                      + od_mpcorr(2,j)/odlay(j) * EXP((odlay(j) - odint)/a) ) * odlay_tl(j) &
                                  + od_mpcorr(2,j)/odlay(j) * ( -EXP((odlay(j) - odint)/a) + EXP(-odint/a)) * odint_tl
                ENDIF
              ENDIF
            ENDDO
          ENDIF
          IF (sum(w_sscat(2,:)) > 0) THEN
            ed_eff_sscat_tl(2) = SUM( ed_tl(2,:) * w_sscat(2,:))/ SUM(w_sscat(2,:)) &
                                + 1/SUM(w_sscat(2,:)) *  SUM( (ed(2,:)-ed_eff_sscat(2)) * w_sscat_tl(2,:))
          ENDIF
          ...
          fac_rm_tl(:) = 0._jprb
          ! Ice phase:
          fac_rm_tl(2) = - ampl/(sigma*SQRT(2._jprb*pi)) * DEXP(-0.5*((LOG(od_eff(2)+delta) - 3.15)/sigma)**2) &
                          * (LOG(od_eff(2)+delta) - 3.15)/sigma * 1/sigma * 1/(od_eff(2)+delta) * od_eff_tl(2)
          
          ed_eff_tl(2) = (ed_eff_sscat(2) - ed_eff_v1(2)) * fac_rm_tl(2) &
                       + fac_rm(2) * ed_eff_sscat_tl(2) &
                       + (1 - fac_rm(2)) * ed_eff_tl(2)
        ENDIF
...
!         iw   (d)  = 1._jprb
    ...
    ! --------------------------------------------------------------------------
    ! Compute clear reflectance and add clear column to total reflectance
    ! --------------------------------------------------------------------------
...
!       iw   (d) = 1._jprb
  ...
  DEALLOCATE( ip, ip_tl, iw_tl, di, od, od_tl, od_v1, od_eff, od_eff_tl, od_eff_v1, &
              od_mpcorr, od_mpcorr_v1, od_mpcorr_tl, odlay, odlay_tl, STAT=err)
  THROWM(err.NE.0,"Deallocation of memory for rttov_mfasis_tl failed")
  IF (mfasis_coefs%file_type .EQ. mfasis_cld) THEN
    DEALLOCATE(ed, ed_tl, ed_eff, ed_aux, ed_eff_tl, ed_eff_v1, fac_mx, fac_mx_tl, &
               q_mxr, q_mxr_tl, dvap, dvap_tl, STAT=err)
    THROWM(err.NE.0,"Deallocation of memory for rttov_mfasis_tl failed")
    DEALLOCATE(w_sscat, w_sscat_tl, fac_rm, fac_rm_tl, ed_eff_sscat, ed_eff_sscat_tl, STAT=err)
    THROWM(err.NE.0,"Deallocation of memory for rttov_mfasis_tl failed")
  ENDIF

  IF (LHOOK) CALL DR_HOOK('RTTOV_MFASIS_TL', 1_jpim, ZHOOK_HANDLE)
