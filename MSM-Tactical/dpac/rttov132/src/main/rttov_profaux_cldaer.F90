...
!INTF_END
  ...
  REAL(jprb), PARAMETER :: rtou_upp = -20._jprb ! Equivalent to max Deff = 133.1007782 um
  REAL(jprb), PARAMETER :: rtou_low = -60._jprb ! Equivalent to min Deff = 22.00015420 um
          ...
          IF (profiles_int(iprof)%cloud(6,lay) > 0._jprb) THEN
            ...
            ELSE
              IF (profiles(iprof)%icede_param == 1) THEN
                ...
                !
                IF (ztempc < rtou_low) ztempc = rtou_low
                IF (ztempc > rtou_upp) ztempc = rtou_upp
