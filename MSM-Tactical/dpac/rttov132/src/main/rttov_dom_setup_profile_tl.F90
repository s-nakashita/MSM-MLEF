...
#include "rttov_errorreport.interface"
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_DOM_SETUP_PROFILE_TL', 0_jpim, ZHOOK_HANDLE)
  ...
  DO i = 1, nchanprof
    ...
    IF (opts%rt_ir%addclouds) THEN
      ...
      ENDWHERE
    ENDIF
    WHERE (ssa(:,:,i) >= 1._jprb - 1.E-6_jprb) ssa_tl(:,:,i) = 0._jprb
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_DOM_SETUP_PROFILE_TL', 1_jpim, ZHOOK_HANDLE)
