...
!! @param[in]     foam_fraction_tl      user foam fraction perturbation
!
! Copyright:
...
!
  ...
  SUBROUTINE rttov_fastem5_tl(fastem_version,       &  ! Input
                              ...
                              Rel_Azimuth_tl,       &  ! Input
                              Supply_Foam_Fraction, &  ! Input
                              Foam_Fraction,        &  ! Input
                              Foam_Fraction_tl)        ! Input
...
! TL internal variables
    ...
    REAL(jprb)    :: u19, u19_tl, freq_ghz, dfreq
    ...
    REAL(jprb)    :: tbfixed_tl(4,4,3)
    REAL(jprb)    :: efixed_tl(4,4,3)
    REAL(jprb)    :: einterpolated_tl(4,3)
    REAL(jprb)    :: a1e_tl, a2e_tl, a3e_tl
  ...
  + &
        ...
        DO ich = 0, 15
          ...
          tbfixed(istokes, i_freq, 1)    = a1e
          tbfixed(istokes, i_freq, 2)    = a2e
          tbfixed(istokes, i_freq, 3)    = a3e
          tbfixed_tl(istokes, i_freq, 1) = a1e_tl
          tbfixed_tl(istokes, i_freq, 2) = a2e_tl
          tbfixed_tl(istokes, i_freq, 3) = a3e_tl
        ENDDO
        efixed_tl(:,:,:)      = 0.0_JPRB
    ...
    ! Interpolate results to required frequency based on 7, 10, 19, 37 GHz
          IF (freq_ghz .LE. freqfixed(1)) THEN
            einterpolated(3:4, M)    = efixed(1, 3:4, M)
            einterpolated_tl(3:4, M) = efixed_tl(1, 3:4, M)
          ELSE IF (freq_ghz .GE. freqfixed(4)) THEN
            einterpolated(3:4, M)    = efixed(4, 3:4, M)
            einterpolated_tl(3:4, M) = efixed_tl(4, 3:4, M)
          ELSE
            ...
            einterpolated(3:4, M)    = efixed(ifreq - 1, 3:4, M) + dfreq * (efixed(ifreq, 3:4, M) - efixed(ifreq - 1, 3:4, M))
            einterpolated_tl(3:4, M) =      &
              & efixed_tl(ifreq - 1, 3:4, M) + dfreq * (efixed_tl(ifreq, 3:4, M) - efixed_tl(ifreq - 1, 3:4, M))
          ENDIF
        ENDDO
        DO istokes = 3, 4
          DO M = 1, 3
            ...
            Azimuth_Emi_tl(istokes) = Azimuth_Emi_tl(istokes) + &
              & (einterpolated_tl(istokes, M) * sin(m * phi) + einterpolated(istokes, M) * m * cos(m * phi) * phi_tl) *  &
              & (1.0_JPRB - cos_z) / (1.0_JPRB - 0.6018_JPRB)
          ENDDO
        ENDDO
    ...
    IF (fastem_version == 6) THEN
      ...
      Reflectivity_tl(3) = -0.5_jprb * (zreflmod_v_tl + zreflmod_h_tl) * Emissivity(3) - &
                            0.5_jprb * (zreflmod_v + zreflmod_h) * Emissivity_tl(3)
      Reflectivity(4) = -0.5_jprb * (zreflmod_v + zreflmod_h) * Emissivity(4)
      Reflectivity_tl(4) = -0.5_jprb * (zreflmod_v_tl + zreflmod_h_tl) * Emissivity(4) - &
                            0.5_jprb * (zreflmod_v + zreflmod_h) * Emissivity_tl(4)
    ELSE
  ...
  END SUBROUTINE rttov_fastem5_tl
