...
!! @param[in,out] tskin_eff_k         effective skin temperature increments
...
!subroutine arguments:
  ...
  REAL(KIND=jprb),              INTENT(INOUT)         :: tskin_eff_k(size(chanprof))
...
!local variables:
  ...
  INTEGER(KIND=jpim) :: i, j, iz, jz, chan, istokes, ifreq, m
  ...
  REAL   (KIND=jprb) :: wind10_direction_k, windangle_k, windratio_k, abs_windratio_k ! Note wind azimuth is in radians
...
! rttov_fastem4
  ...
  REAL(KIND=jprb) :: Transmittance_k, Rel_Azimuth_k
...
!-------------------------------------------
      ...
      CASE (0)
        emissstokes_k(i,1:2) = emissstokes_k(i,1:2) - reflectstokes_k(i,1:2)
        emissstokes_k(i,3:4) = 0._jprb
        reflectstokes_k(i,3:4) = 0._jprb
        CALL rttov_tessem_ad(freq_ghz, prof%zenangle, wind10, tskin_eff(i), &
                             prof%skin%salinity, wind10_k, &
                             tskin_eff_k(i), prof_k%skin%salinity, &
...
!------
        ...
        ELSE
          wind10_sq_k = 0._jprb
          fresnel_v_k = - emissstokes_k(i, 1)
          fresnel_h_k = - emissstokes_k(i, 2)
        ENDIF
!1.3.1) Fresnel reflection coefficients
!------
...
!Set values for temperature polynomials (convert from kelvin to celsius)
        ...
        tskin_eff_k(i)   = tskin_eff_k(i) + tcelsius_k
      ...
      CASE (7)
        ...
        zreflmod_v_k       = 0.0_JPRB
        zreflmod_h_k       = 0.0_JPRB
        opdpsfc_k          = 0.0_JPRB
        variance_k         = 0.0_JPRB
        varm_k             = 0.0_JPRB

        zreflmod_v_k = zreflmod_v_k + reflectstokes_k(i,1) * (1.0_JPRB - emissstokes(i,1)) 
        emissstokes_k(i,1) = emissstokes_k(i,1) - (zreflmod_v * reflectstokes_k(i,1))

        zreflmod_h_k = zreflmod_h_k + reflectstokes_k(i,2) * (1.0_JPRB - emissstokes(i,2)) 
        emissstokes_k(i,2) = emissstokes_k(i,2) - (zreflmod_h * reflectstokes_k(i,2))

        zreflmod_v_k = zreflmod_v_k - (0.5_JPRB * reflectstokes_k(i,3) * emissstokes(i,3))
        zreflmod_h_k = zreflmod_h_k - (0.5_JPRB * reflectstokes_k(i,3) * emissstokes(i,3)) 
        emissstokes_k(i,3) = emissstokes_k(i,3) - (0.5_JPRB * (zreflmod_v + zreflmod_h) * reflectstokes_k(i,3))    

        zreflmod_v_k = zreflmod_v_k - (0.5_JPRB * reflectstokes_k(i,4) * emissstokes(i,4))
        zreflmod_h_k = zreflmod_h_k - (0.5_JPRB * reflectstokes_k(i,4) * emissstokes(i,4))
        emissstokes_k(i,4) = emissstokes_k(i,4) - (0.5_JPRB * (zreflmod_v + zreflmod_h) * reflectstokes_k(i,4))  

        IF( Transmittance > transmittance_limit_lower .AND. Transmittance < transmittance_limit_upper) THEN
          zrough_v_k = 0.0_jprb
          zrough_h_k = 0.0_jprb
          Transmittance_k = Transmittance_k + zreflmod_v_k/(1.0_jprb-Transmittance)*(-zrough_v  &
              *Transmittance**(zrough_v-1.0_jprb) + zreflmod_v )
          zrough_v_k = zrough_v_k - (Transmittance**zrough_v * LOG(Transmittance) * zreflmod_v_k)/ &
              (1.0_jprb-Transmittance)
          Transmittance_k = Transmittance_k + zreflmod_h_k/(1.0_jprb-Transmittance)*(-zrough_h  &
              *Transmittance**(zrough_h-1.0_jprb) + zreflmod_h )
          zrough_h_k = zrough_h_k - (Transmittance**zrough_h * LOG(Transmittance) * zreflmod_h_k)/ &
              (1.0_jprb-Transmittance)
          zx_k(:) = 0.0_jprb
          DO iz = 1, 7
            jz = iz-1
            !Switched h to v Deblonde SSMIS june 7, 2001
            zx_k(9)  = zx_k(9)  + zrough_v_k * zx(iz) * t_c5(24+jz*3)
            zx_k(8)  = zx_k(8)  + zrough_v_k * zx(iz) * t_c5(23+jz*3)
            zx_k(iz) = zx_k(iz) + zrough_v_k * (t_c5(22+jz*3) + zx(8)*t_c5(23+jz*3) + zx(9)*t_c5(24+jz*3) )
            zx_k(9)  = zx_k(9)  + zrough_h_k * zx(iz) * t_c5(3+jz*3)
            zx_k(8)  = zx_k(8)  + zrough_h_k * zx(iz) * t_c5(2+jz*3)
            zx_k(iz) = zx_k(iz) + zrough_h_k * (t_c5(1+jz*3) + zx(8)*t_c5(2+jz*3) + zx(9)*t_c5(3+jz*3))
          END DO
          zrough_v_k = 0.0_jprb
          zrough_h_k = 0.0_jprb

          zx_k(8) = zx_k(8) + (2.0_jprb*zx_k(9)*zx(8))
          opdpsfc_k = opdpsfc_k + (zx_k(8) / opdpsfc)
          zx_k(2) = zx_k(2) + (2.0_jprb*zx_k(7)*zx(2))
          zx_k(3) = zx_k(3) + (2.0_jprb*zx_k(5)*zx(3))
          zx_k(2) = zx_k(2) + (zx_k(3)*zx(4))
          zx_k(4) = 0.0_jprb
          variance_k = zx_k(2)
          zx_k(1) = 0.0_jprb
          ...
          !Compute surface to space optical depth
          Transmittance_k = Transmittance_k - (opdpsfc_k * geom%coszen / Transmittance)
          IF (test_variance < varm) THEN
            varm_k = variance_k * (t_c5(44) * freq_ghz + t_c5(45))
          ELSE
            varm_k = variance_k
          ENDIF
          variance_k = varm_k * t_c5(43)
          wind10_k = wind10_k + (0.00512_jprb * variance_k)
        ELSE
           zreflmod_v_k = 0.0_jprb
           zreflmod_h_k = 0.0_jprb 
        END IF
        ...
        CALL rttov_surfem_ocean_ad(freq_ghz, prof%zenangle, wind10, tskin_eff(i), prof%skin%salinity, Rel_Azimuth, &
                                emissstokes_k(i,1), emissstokes_k(i,2), emissstokes_k(i,3), emissstokes_k(i,4), &
                                wind10_k, tskin_eff_k(i), prof_k%skin%salinity, Rel_Azimuth_k)

        wind10_direction_k = wind10_direction_k - (Rel_Azimuth_k * 180.0_JPRB / pi)
        transmission_aux_k%thermal_path1%tau_surf(0, i) = transmission_aux_k%thermal_path1%tau_surf(0, i) + &
                                                           Transmittance_k
        wind10_sq_k = 0._jprb

      END SELECT
      windangle_k     = wind10_direction_k * quadcof(iquadrant, 2)
      abs_windratio_k = windangle_k / (1.0_JPRB + abs_windratio * abs_windratio)
      windratio_k     = quadcof(iquadrant, 2) * abs_windratio_k
      IF (abs(prof%s2m%v) >= windlimit) THEN
