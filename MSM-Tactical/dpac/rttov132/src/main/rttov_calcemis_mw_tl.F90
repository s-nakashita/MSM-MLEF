...
!! @param[in]     tskin_eff_tl        effective skin temperature perturbations
...
!INTF_OFF
  ...
  USE mod_rttov_fastem5_coef, ONLY: t_c5, transmittance_limit_lower, transmittance_limit_upper
  USE rttov_tessem_mod, ONLY : rttov_tessem_tl
  USE rttov_surfem_ocean_tl_mod, ONLY : rttov_surfem_ocean_tl
!INTF_ON
  IMPLICIT NONE
!subroutine arguments:
  ...
  REAL(KIND=jprb),              INTENT(IN)            :: tskin_eff_tl(size(chanprof))
...
!local variables:
  ...
  INTEGER(KIND=jpim) :: i, j, iz, jz, chan    , istokes, m
...
! TL variables
  ...
  REAL   (KIND=jprb) :: wind10_direction_tl, windangle_tl, windratio_tl, abs_windratio_tl ! Note wind azimuth is in radians
...
! rttov_fastem4_tl   internal variables
  ...
  REAL(KIND=jprb) :: Transmittance_tl, Rel_Azimuth_tl
...
! no TL on wind direction, but TL on wind speed
      ...
      ELSE
        ...
        ENDIF
      ENDIF
      ! quadcof(iquadrant,2) is the same as sign(windratio),
      ! and the derivative of abs(x) wrt x is sign(x):
      abs_windratio_tl    = quadcof(iquadrant, 2) * windratio_tl
      windangle_tl        = abs_windratio_tl / (1.0_JPRB + abs_windratio * abs_windratio)
      wind10_direction_tl = windangle_tl * quadcof(iquadrant, 2)
      SELECT CASE (opts%rt_mw%fastem_version)
      CASE (0)
        CALL rttov_tessem_tl(freq_ghz, prof%zenangle, wind10, tskin_eff(i), &
                             prof%skin%salinity, wind10_tl, &
                             tskin_eff_tl(i), prof_tl%skin%salinity, &
                             emissstokes_tl(i,2), emissstokes_tl(i,1))
        emissstokes_tl(i,3:4) = 0._jprb
        reflectstokes_tl(i,1:2) = - emissstokes_tl(i,1:2)
        reflectstokes_tl(i,3:4) = 0._jprb
...
!           reflectstokes_tl(i,4)  =     -emissstokes_tl(i,4)
        ELSE
          reflectstokes_tl(i, 1:2) =  - emissstokes_tl(i, 1:2)
          reflectstokes_tl(i, 3:4) = 0.0_JPRB
        ENDIF
...
! relative azimuth angle in degree
      ...
      CASE (7)
        ...
        Transmittance = transmission_aux%thermal_path1%tau_surf(0, i)
        Transmittance_tl = transmission_aux_tl%thermal_path1%tau_surf(0, i)
        CALL rttov_surfem_ocean_tl(freq_ghz, prof%zenangle, wind10, tskin_eff(i), prof%skin%salinity, Rel_Azimuth, &
                                wind10_tl, tskin_eff_tl(i), prof_tl%skin%salinity, Rel_Azimuth_tl, &
                                emissstokes(i,1), emissstokes(i,2), emissstokes(i,3), emissstokes(i,4), &
                                emissstokes_tl(i,1), emissstokes_tl(i,2), emissstokes_tl(i,3), emissstokes_tl(i,4))
        ...
        IF( Transmittance > transmittance_limit_lower .and. Transmittance < transmittance_limit_upper) THEN
          ...
          variance_tl = 0.00512_jprb * wind10_tl
          varm_tl     = variance_tl * t_c5(43)
          variance_tl = varm_tl * ( t_c5(44) * freq_ghz + t_c5(45) )
          ...
          opdpsfc_tl = -Transmittance_tl * geom%coszen / ( Transmittance )
          !Define nine predictors for the effective angle calculation
          zx(1) = 1.0_jprb
          zx(2) = variance
          zx(4) = 1.0_jprb / geom%coszen
          ...
          zrough_v = 1.0_jprb
          zrough_h = 1.0_jprb
          zrough_v_tl = 0.0_jprb
          zrough_h_tl = 0.0_jprb
          ...
          DO iz = 1, 7
            ...
            zrough_h_tl = zrough_h_tl + zx_tl(iz) *(t_c5(1+jz*3) + zx(8)*t_c5(2+jz*3) &
                  + zx(9)*t_c5(3+jz*3) ) + zx(iz) *( zx_tl(8)*t_c5(2+jz*3) + zx_tl(9)*t_c5(3+jz*3) )
            zrough_v_tl = zrough_v_tl + zx_tl(iz) *(t_c5(22+jz*3)+ zx(8)*t_c5(23+jz*3)  &
                  + zx(9)*t_c5(24+jz*3) ) + zx(iz) *( zx_tl(8)*t_c5(23+jz*3)+ zx_tl(9)*t_c5(24+jz*3) )
          END DO

          zreflmod_v = (1.0_jprb-Transmittance ** zrough_v)/(1.0_jprb-Transmittance )
          zreflmod_v_tl = Transmittance_tl/(1.0_jprb-Transmittance)*(-zrough_v  &
              *Transmittance**(zrough_v-1.0_jprb) + zreflmod_v )
          zreflmod_v_tl = zreflmod_v_tl -Transmittance**zrough_v *log(Transmittance)  &
              * zrough_v_tl/(1.0_jprb-Transmittance)

          zreflmod_h = (1.0_jprb-Transmittance ** zrough_h)/(1.0_jprb-Transmittance )
          zreflmod_h_tl = Transmittance_tl/(1.0_jprb-Transmittance)*(-zrough_h  &
              *Transmittance**(zrough_h-1.0_jprb) + zreflmod_h )
          zreflmod_h_tl = zreflmod_h_tl -Transmittance**zrough_h *log(Transmittance)  &
              *  zrough_h_tl/(1.0_jprb-Transmittance)
        ELSE 
          zreflmod_v = 1._jprb
          zreflmod_h = 1._jprb
          zreflmod_v_tl = 0._jprb
          zreflmod_h_tl = 0._jprb
        END IF
        ! end correction term -----------------------------------------------------------

        reflectstokes_tl(i,1) = zreflmod_v_tl * (1.0_JPRB - emissstokes(i,1)) - (zreflmod_v * emissstokes_tl(i,1))
        reflectstokes_tl(i,2) = zreflmod_h_tl * (1.0_JPRB - emissstokes(i,2)) - (zreflmod_h * emissstokes_tl(i,2))
