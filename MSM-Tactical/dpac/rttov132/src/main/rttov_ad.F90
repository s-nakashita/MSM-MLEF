...
#include "rttov_mfasis_nn_ad.interface"
...
#include "rttov_transmit_ad.interface"
  ...
  INTEGER(KIND=jpim) :: nlevels, nprofiles, nchanprof, npcscores, i
...
!-------------
  ...
  IF (opts%rt_all%transmittances_only) THEN
    err = errorstatus_fatal
    THROWM(err.NE.0, "transmittances_only must be false for TL/AD/K")
  ENDIF
...
!-----------------------------------------------------------------------
  ...
  ! deallocate and return. In particular this can happen when using the parallel interface.
  ...
  IF (traj0_sta%dothermal .AND. .NOT. opts%rt_all%use_tskin_eff) THEN
    DO i = 1, nchanprof
      profiles_ad(chanprof(i)%prof)%skin%t = &
          profiles_ad(chanprof(i)%prof)%skin%t + &
          emissivity_ad(i)%tskin_eff
    ENDDO
    emissivity_ad(:)%tskin_eff = 0._jprb
  ENDIF
