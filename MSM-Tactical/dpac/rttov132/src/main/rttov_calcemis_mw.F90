...
!local variables:
  ...
  REAL(KIND=jprb)     :: wind10_sq       , windsec, windratio, abs_windratio
...
!-------------------------------------------
      ...
      CASE (0)
        CALL rttov_tessem(freq_ghz, prof%zenangle, wind10, tskin_eff(i), &
                          prof%skin%salinity, emissstokes(i,2), emissstokes(i,1))
        emissstokes(i,3:4) = 0._jprb
        reflectstokes(i,1:2) = 1._jprb - emissstokes(i,1:2)
        reflectstokes(i,3:4) = 0._jprb
...
!Switched h to v Deblonde SSMIS june 7, 2001
      ...
      CASE (7) ! SURFEM-Ocean model
        ...
        IF( Transmittance > transmittance_limit_lower .and. Transmittance < transmittance_limit_upper) THEN
          ...
          IF ( variance <= 0.0_JPRB ) variance = 0.0_JPRB
          !Compute surface to space optical depth
          opdpsfc = -log(Transmittance ) * geom%coszen ! note geom%coszen is recalculated in fastem5 sub so bit diff
          !Define nine predictors for the effective angle calculation
          zx(1) = 1.0_JPRB
          zx(2) = variance
          zx(4) = 1.0_JPRB / geom%coszen
