! Description:
!> @file
!!   TL of calculation of PC scores for PC-RTTOV simulations
!
!> @brief
!!   TL of calculation of PC scores for PC-RTTOV simulations
!!
...
!! @param[in,out] pccomp_tl       PC scores perturbations
!! @param[in]     coef_pccomp     PC-RTTOV coefficient structure
!! @param[in]     radiance_tl     radiance perturbations (for predictor channels)
!
! Copyright:
...
!-----End of header-------------------------------------------------------------
  IF (LHOOK) CALL DR_HOOK('RTTOV_PCSCORES_TL', 0_jpim, ZHOOK_HANDLE)
...
! by improving use of vectorisation
  DO prof = 1, nprofiles
    DO j = (prof-1) * npcscores_p + 1, prof * npcscores_p
      ...
      pccomp_tl%total_pcscores(j) = DOT_PRODUCT( &
        coef_pccomp%pcreg(opts%rt_ir%pc%ipcbnd, opts%rt_ir%pc%ipcreg)%coefficients(1:nchannels_p, chan_pc), &
        radiance_tl%total((prof-1)*nchannels_p + 1 : prof*nchannels_p) * noise_r(1:nchannels_p))
    ENDDO
  ENDDO

  IF (LHOOK) CALL DR_HOOK('RTTOV_PCSCORES_TL', 1_jpim, ZHOOK_HANDLE)
END SUBROUTINE rttov_pcscores_tl
