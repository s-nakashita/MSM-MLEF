...
!! @param[in,out] emissivity_k     emissivity increments
...
!- End of header --------------------------------------------------------
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_CALCRAD_K', 0_jpim, ZHOOK_HANDLE)
  ...
  DO ichan = 1, nchanprof
    ...
    IF (coef%ff_val_bc) THEN
      emissivity_k(ichan)%tskin_eff = emissivity_k(ichan)%tskin_eff + &
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_CALCRAD_K', 1_jpim, ZHOOK_HANDLE)
