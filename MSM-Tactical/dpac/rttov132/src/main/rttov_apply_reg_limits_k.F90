! Description:
!> @file
!!   Clip profile variables to regression limits K.
!
!> @brief
!!   Clip profile variables to regression limits K.
!!
...
!INTF_END
  ...
  INTEGER(KIND=jpim) :: nchanprof, ichan, iprof
  REAL(jphook) :: zhook_handle

!- End of header --------------------------------------------------------

  IF (LHOOK) CALL DR_HOOK('RTTOV_APPLY_REG_LIMITS_K',0_jpim,ZHOOK_HANDLE)
  ...
  DO ichan = 1, nchanprof
    ...
    ELSE ! addpc
      ...
      IF (wind < coef_pccomp%lim_pc_prfl_wsmin) THEN
        prof_k(ichan)%s2m%u = 0._jprb
        prof_k(ichan)%s2m%v = 0._jprb
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_APPLY_REG_LIMITS_K',1_jpim,ZHOOK_HANDLE)

END SUBROUTINE rttov_apply_reg_limits_k
