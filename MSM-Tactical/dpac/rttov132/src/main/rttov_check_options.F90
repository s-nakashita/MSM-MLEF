...
!!
!!   When called internally by RTTOV the chanprof and profiles arguments must
!!   be present. The rttov_user_options_checkinput routine calls this routine
!!   in which case the optional arguments are missing and only the checks on
...
!! @param[in]     profiles             input atmospheric and surface data, optional
...
!- End of header --------------------------------------------------------
  ...
  ! UV/VIS/IR instruments
  IF (coefs%coef%id_sensor == sensor_id_ir .OR. coefs%coef%id_sensor == sensor_id_hi) THEN
    IF (opts%rt_mw%clw_data) THEN
      err = errorstatus_fatal
      msg = 'Error: cloud liquid water absorption (clw_data) only applicable to MW instruments'
      THROWM(err.NE.0, msg)
    ENDIF
    ...
    IF (.NOT. opts%rt_ir%pc%addpc) THEN
      ...
      ! Sea emissivity model (if there are thermal channels)
      IF (ANY(coefs%coef%ff_cwn < 3000)) THEN
        ...
        IF (opts%rt_ir%ir_sea_emis_model == 1 .AND. coefs%coef%ssirem_ver == 0) THEN
          err = errorstatus_fatal
          msg = 'Error: coefficient file does not support ISEM (ir_sea_emis_model 1)'
          THROWM(err.NE.0, msg)
        ENDIF

        IF (opts%rt_ir%ir_sea_emis_model == 2 .AND. coefs%coef%iremis_version == 0) THEN
          err = errorstatus_fatal
          msg = 'Error: coefficient file does not support ir_sea_emis_model 2'
          THROWM(err.NE.0, msg)
        ENDIF
      ENDIF
    ...
    IF (opts%rt_ir%addclouds .OR. opts%rt_ir%addaerosl) THEN
      ...
      ! Checks for MFASIS-NN
      IF (opts%rt_ir%addsolar .AND. opts%rt_ir%vis_scatt_model == vis_scatt_mfasis_nn) THEN
        IF (opts%rt_ir%addclouds .AND. opts%rt_ir%addaerosl) THEN
          err = errorstatus_fatal
          msg = 'MFASIS-NN cannot be used with both cloud AND aerosol'
          THROWM(err.NE.0, msg)
        ENDIF
        IF (opts%rt_ir%addclouds .AND. coefs%coef_mfasis_nn%file_type /= mfasis_cld) THEN
          err = errorstatus_fatal
          msg = 'MFASIS cloud NN not loaded'
          THROWM(err.NE.0, msg)
        ENDIF
        IF (opts%rt_ir%addaerosl .AND. coefs%coef_mfasis_nn%file_type /= mfasis_aer) THEN
          err = errorstatus_fatal
          msg = 'MFASIS aerosol NN not loaded'
          THROWM(err.NE.0, msg)
        ENDIF
        IF (opts%rt_ir%addclouds .AND. opts%rt_ir%user_cld_opt_param) THEN
          err = errorstatus_fatal
          msg = 'MFASIS-NN is not compatible with explicit cloud optical property input'
          THROWM(err.NE.0, msg)
        ENDIF
        IF (opts%rt_ir%addaerosl .AND. opts%rt_ir%user_aer_opt_param) THEN
          err = errorstatus_fatal
          msg = 'MFASIS-NN is not compatible with explicit aerosol optical property input'
          THROWM(err.NE.0, msg)
        ENDIF
        IF (ALL(coefs%coef%ss_val_chn == 0)) THEN
          err = errorstatus_fatal
          msg = 'MFASIS-NN requires a solar-enabled optical depth coefficient file'
          THROWM(err.NE.0, msg)
        ENDIF
        IF (all_checks) THEN
          IF (ANY(profiles(:)%ice_scheme /= ice_scheme_baum)) THEN
            err = errorstatus_fatal
            THROWM(err.NE.0, 'MFASIS NN cannot be used with the Baran ice properties')
          ENDIF
        ENDIF
      ENDIF
  ...
  IF (opts%rt_ir%do_nlte_correction .AND. coefs%coef%nltecoef) THEN
    ...
    IF (coefs%coef%nlte_coef%nsat > 1) THEN
      err = errorstatus_fatal
      msg = 'Coefficient file appears to contain v11 NLTE coefficients which are incompatible with v12 and later'
      THROWM(err.NE.0, msg)
    ENDIF
  ENDIF
  ...
  IF (opts%rt_ir%pc%addpc) THEN
    ...
    IF (opts%rt_ir%addclouds) THEN
      IF (coefs%coef_pccomp%fmv_pc_cld == 0_jpim) THEN
        err = errorstatus_fatal
    ...
    IF (opts%rt_ir%addaerosl) THEN
      IF (coefs%coef_pccomp%fmv_pc_aer == 0_jpim) THEN
        err = errorstatus_fatal
    ...
    IF (opts%rt_all%use_tskin_eff) THEN
      err = errorstatus_fatal
      msg = 'PC-RTTOV: cannot be run with use_tskin_eff TRUE'
      THROWM(err.NE.0, msg)
    ENDIF

    IF (opts%rt_all%transmittances_only) THEN
      err = errorstatus_fatal
      msg = 'PC-RTTOV: cannot be run with transmittances_only TRUE'
      THROWM(err.NE.0, msg)
    ENDIF
  ENDIF
