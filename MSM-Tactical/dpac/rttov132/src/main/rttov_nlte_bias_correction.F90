! Description:
!> @file
!!   Compute non-LTE bias correction.
!
!> @brief
!!   Compute non-LTE bias correction.
!!
!! @details
!!   This implements the parameterisation developed by Marco Matricardi for
!!   RTTOV v12.
!!
...
!! @param[in,out] rad             output radiance structure
!
! Copyright:
...
!INTF_END
  ...
  REAL(jprb)    :: t(3,SIZE(profiles))
  REAL(jprb)    :: x(coef%nlte_coef%ncoef,SIZE(profiles)), path, znlte
...
!- End of header ------------------------------------------------------
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_NLTE_BIAS_CORRECTION', 0_jpim, ZHOOK_HANDLE)
...
!===============================================================================!
  DO iprof = 1, SIZE(profiles)
    ...
    ! find pressure bounds for average temperatures
    p = 0.; j = 1
    DO WHILE (p <= p4)
      p = profiles(iprof)%p(j)

      IF (p >= p1 .AND. p <= p2) THEN ! contribution to t1_avg
        IF (lb1 < 0) lb1 = j
        ub1 = j
...
!================================!
  DO iprof = 1, SIZE(profiles)
    IF (do_nlte(iprof)) THEN
      ...
      x(1,iprof) = 1._jprb
      x(2,iprof) = COS(profiles(iprof)%sunzenangle * deg2rad)
      x(3,iprof) = SQRT(COS(profiles(iprof)%sunzenangle * deg2rad))
      x(4,iprof) = COS(profiles(iprof)%sunzenangle * deg2rad) * path
      x(5,iprof) = COS(profiles(iprof)%sunzenangle * deg2rad) * t(1,iprof)
      x(6,iprof) = ((COS(profiles(iprof)%sunzenangle * deg2rad)) * path)**2
      x(7,iprof) = COS(profiles(iprof)%sunzenangle * deg2rad) * t(2,iprof)
      x(8,iprof) = path * t(1,iprof)
      x(9,iprof) = path * t(2,iprof)
    ENDIF
  ENDDO
    ...
    IF (do_nlte(prof)) THEN
      ...
      IF (chan >= nlte_coef%start_chan .AND. chan <= nlte_coef%end_chan) THEN
        ...
        DO npred = 1, coef%nlte_coef%ncoef
          znlte = znlte + nlte_coef%coef(npred,1,1,k) * x(npred,prof)
        ENDDO

        rad%clear(i)  = rad%clear(i)  + znlte
        rad%total(i)  = rad%total(i)  + znlte
        IF (.NOT. opts%rt_ir%pc%addpc) rad%cloudy(i) = rad%cloudy(i) + znlte
      ENDIF
    ENDIF
  ENDDO

  IF (LHOOK) CALL DR_HOOK('RTTOV_NLTE_BIAS_CORRECTION', 1_jpim, ZHOOK_HANDLE)
