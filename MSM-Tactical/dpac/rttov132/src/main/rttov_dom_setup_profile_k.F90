! Description:
!> @file
!!   Set up input profiles of optical depths and SSAs for DOM algorithm - K.
!
!> @brief
!!   Set up input profiles of optical depths and SSAs for DOM algorithm - K.
!!
!! @details
!!   See direct model for details. The members of the K profiles_dom array
...
!! @param[in,out] opdp_path_k               gas absorption optical depth increments
...
!! @param[in,out] transmission_scatt_ir_k   increments in aerosol/cloud optical depths and total layer optical depths and SSAs
!! @param[in]     profiles_dom              array of rttov_profile_dom structures
!! @param[in,out] profiles_dom_k            array of rttov_profile_dom structures containing increments
!
! Copyright:
...
#include "rttov_errorreport.interface"
  ...
  REAL(jprb)    :: clropdep_k(nlayers)
  REAL(jprb)    :: absopdep(nlayers)
  REAL(jprb)    :: absopdep_k(nlayers)
  REAL(jprb)    :: scaopdep(nlayers)
  REAL(jprb)    :: scaopdep_k(nlayers)
  REAL(jprb)    :: totopdep_k(nlayers)
  REAL(jprb)    :: surfclropdep_k
  REAL(jprb)    :: surfabsopdep_k
  REAL(jprb)    :: surfscaopdep
  REAL(jprb)    :: surfscaopdep_k
  REAL(jprb)    :: surftotopdep_k
  REAL(jprb)    :: totopdep_init              ! Nadir optical depth at top input level
  REAL(jprb)    :: totopdep_init_k
  REAL(jprb)    :: layerod_k(0:1,nlayers)

  REAL(jprb), POINTER :: ssa(:,:,:), layerod(:,:,:)
  REAL(jprb), POINTER :: ssa_k(:,:,:)
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_DOM_SETUP_PROFILE_K', 0_jpim, ZHOOK_HANDLE)

  nchanprof = SIZE(chanprof)

  IF (dosolar) THEN
    ssa => transmission_scatt_ir%ssa_solar
    ssa_k => transmission_scatt_ir_k%ssa_solar
    layerod => transmission_scatt_ir%layerod_solar
  ELSE
    ssa => transmission_scatt_ir%ssa_thermal
    ssa_k => transmission_scatt_ir_k%ssa_thermal
  ...
  DO i = 1, nchanprof
    ...
    DO col = ircld%ncolumn(prof), 0, -1
      ...
      IF (domnlayers /= profiles_dom(col,i)%nlayers) THEN
        ...
        THROWM(err.NE.0, 'K - domnlayers mismatch')
      ENDIF


      ! Adjoint calculations

      totopdep_init_k = 0._jprb
      absopdep_k      = 0._jprb
      scaopdep_k      = 0._jprb
      totopdep_k      = 0._jprb
      surfclropdep_k  = 0._jprb
      surfabsopdep_k  = 0._jprb
      surfscaopdep_k  = 0._jprb
      surftotopdep_k  = 0._jprb

      IF (.NOT. dosolar) THEN
        WHERE (profiles_dom(col,i)%layerod(:) <= min_od) profiles_dom_k(col,i)%layerod(:) = 0._jprb
      ENDIF

      IF (profiles_dom(col,i)%surface) THEN
        surftotopdep_k = surftotopdep_k + profiles_dom_k(col,i)%layerod(domnlayers)
      ENDIF

      DO lay = maxnlayers, 1, -1
        totopdep_k(lay) = totopdep_k(lay) + profiles_dom_k(col,i)%layerod(laymaptodom(lay))
      ENDDO

      totopdep_init_k = totopdep_init_k + profiles_dom_k(col,i)%layerod(1)

!       profiles_dom_k(col,i)%layerod(:) = 0._jprb


      IF (profiles_dom(col,i)%surface) THEN
        surfclropdep_k = surfclropdep_k + surftotopdep_k
        surfabsopdep_k = surfabsopdep_k + surftotopdep_k
        surfscaopdep_k = surfscaopdep_k + surftotopdep_k

        aux_prof_k%s(i)%pfraction_surf = aux_prof_k%s(i)%pfraction_surf - &
            surfscaopdep_k * scaopdep(laysurf)
        aux_prof_k%s(i)%pfraction_surf = aux_prof_k%s(i)%pfraction_surf - &
            surfabsopdep_k * absopdep(laysurf)
        aux_prof_k%s(i)%pfraction_surf = aux_prof_k%s(i)%pfraction_surf - &
            surfclropdep_k * clropdep(laysurf)

        scaopdep_k(laysurf) = scaopdep_k(laysurf) + &
            surfscaopdep_k * (1._jprb - aux_prof%s(prof)%pfraction_surf)
        absopdep_k(laysurf) = absopdep_k(laysurf) + &
            surfabsopdep_k * (1._jprb - aux_prof%s(prof)%pfraction_surf)
        clropdep_k(laysurf) = clropdep_k(laysurf) + &
            surfclropdep_k * (1._jprb - aux_prof%s(prof)%pfraction_surf)
      ENDIF

      clropdep_k = clropdep_k + totopdep_k
      absopdep_k = absopdep_k + totopdep_k
      scaopdep_k = scaopdep_k + totopdep_k

      DO lay = nlayers, 1, -1
        coli = ircld%icldarr(col,lay,prof)

        transmission_scatt_ir_k%opdpsca(coli,lay,i) = &
            transmission_scatt_ir_k%opdpsca(coli,lay,i) + scaopdep_k(lay)
        transmission_scatt_ir_k%opdpabs(coli,lay,i) = &
            transmission_scatt_ir_k%opdpabs(coli,lay,i) + absopdep_k(lay)
      ENDDO
    ...
    WHERE (ssa(:,:,i) >= 1._jprb - 1.E-6_jprb) ssa_k(:,:,i) = 0._jprb
    IF (opts%rt_ir%addaerosl .OR. do_rayleigh_dom) THEN
      WHERE (layerod(0,:,i) > 0._jprb)
        layerod_k(0,:) = layerod_k(0,:) - &
            ssa_k(0,:,i) * transmission_scatt_ir%opdpsca(0,:,i) / layerod(0,:,i) ** 2_jpim
        transmission_scatt_ir_k%opdpsca(0,:,i) = &
            transmission_scatt_ir_k%opdpsca(0,:,i) + ssa_k(0,:,i) / layerod(0,:,i)
      ENDWHERE
    ENDIF
    IF (opts%rt_ir%addclouds) THEN
      WHERE (layerod(1,:,i) > 0._jprb)
        layerod_k(1,:) = layerod_k(1,:) - &
            ssa_k(1,:,i) * transmission_scatt_ir%opdpsca(1,:,i) / layerod(1,:,i) ** 2_jpim
        transmission_scatt_ir_k%opdpsca(1,:,i) = &
            transmission_scatt_ir_k%opdpsca(1,:,i) + ssa_k(1,:,i) / layerod(1,:,i)
      ENDWHERE
    ENDIF
!     ssa_k(:,:,i) = 0._jprb

    DO coli = 0, su
      transmission_scatt_ir_k%opdpsca(coli,:,i) = &
          transmission_scatt_ir_k%opdpsca(coli,:,i) + layerod_k(coli,:)
      transmission_scatt_ir_k%opdpabs(coli,:,i) = &
          transmission_scatt_ir_k%opdpabs(coli,:,i) + layerod_k(coli,:)
      clropdep_k(:) = clropdep_k(:) + layerod_k(coli,:)
    ENDDO
    ...
    ! Note in the following that for plane parallel atmosphere pathsat and patheff are not active in the AD.

    IF (dosolar) THEN
      opdp_path_k(2:nlayers+1,i) = opdp_path_k(2:nlayers+1,i) - &
          clropdep_k(:) * coef%ff_gam(chan) * patheff_r
      opdp_path_k(1:nlayers,i) = opdp_path_k(1:nlayers,i) + &
          clropdep_k(:) * coef%ff_gam(chan) * patheff_r

      opdp_path_k(1,i) = opdp_path_k(1,i) - &
          totopdep_init_k * coef%ff_gam(chan) * patheff_r
    ELSE
      opdp_path_k(2:nlayers+1,i) = opdp_path_k(2:nlayers+1,i) - &
          clropdep_k(:) * coef%ff_gam(chan) * angles(prof)%coszen
      opdp_path_k(1:nlayers,i) = opdp_path_k(1:nlayers,i) + &
          clropdep_k(:) * coef%ff_gam(chan) * angles(prof)%coszen

      opdp_path_k(1,i) = opdp_path_k(1,i) - &
          totopdep_init_k * coef%ff_gam(chan) * angles(prof)%coszen
    ENDIF

  ENDDO ! chanprof

  IF (LHOOK) CALL DR_HOOK('RTTOV_DOM_SETUP_PROFILE_K', 1_jpim, ZHOOK_HANDLE)

  CATCH

  IF (LHOOK) CALL DR_HOOK('RTTOV_DOM_SETUP_PROFILE_K', 1_jpim, ZHOOK_HANDLE)

END SUBROUTINE rttov_dom_setup_profile_k
