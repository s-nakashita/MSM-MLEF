...
!INTF_ON
  ...
  LOGICAL(KIND=jplm),    OPTIONAL, INTENT(IN)  :: do_mfasis            ! if do_mfasis = .true., check only MFASIS-relevant
                                                                       ! profile input
...
TRY
  ...
  IF (.NOT. do_mfasis_l .AND. .NOT. opts%rt_all%use_tskin_eff) THEN
    IF (prof%skin%surftype == surftype_land) THEN
      tskin_max = tskin_land_max
    ELSE
      tskin_max = tmax
    ENDIF
    IF (prof%skin%t > tskin_max .OR. &
