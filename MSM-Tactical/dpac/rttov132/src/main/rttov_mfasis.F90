...
#include "rttov_errorreport.interface"
  ...
  REAL(jprb)                 :: od1_int, ooo1
  REAL(jprb)                 :: odint, a, zenmax, ampl, sigma, delta
  REAL(jprb),    ALLOCATABLE :: fac_mx(:), ed_eff_sscat(:), fac_rm(:)
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_MFASIS', 0_jpim, ZHOOK_HANDLE)
      ...
      IF ( mfasis_coefs%file_type .EQ. mfasis_cld ) THEN ! clouds
        ...
        DO j = 1, nlay
          ...
          IF ( ircld%icldarr(cc,j,prof) .EQ. 1 ) THEN
            ...
            IF (od_mpcorr(2,j) < 0) od_mpcorr(2,j) = 0._jprb
        ...
        od_eff(1) = SUM(od(1,:))
        IF ( od_eff(1) .GT. 0. ) THEN
          ed_eff(1) = SUM( od(1,:)*ed(1,:))/od_eff(1)
        ELSE
          ed_eff(1) = ed_aux(1)
        ENDIF
        ...
        IF ( od_eff(2) .GT. 0. ) THEN
          ed_eff(2) = SUM( od_mpcorr(2,:)*ed(2,:))/od_eff(2)
        ELSE
          ed_eff(2) = ed_aux(2)
        ENDIF
...
!   ! --------------------------------------------------------------------------
  ...
  DEALLOCATE(ip, lc_aux, iw, di, idx, ckp, skp, clm, od, od_eff, od_mpcorr, odlay, q_mxr, dvap, STAT=err)
  THROWM(err.NE.0,"Deallocation of memory for rttov_mfasis failed")
  IF ( mfasis_coefs%file_type .EQ. mfasis_cld ) THEN
    DEALLOCATE(ed, ed_eff, ed_aux, fac_mx, STAT=err)
    THROWM(err.NE.0,"Deallocation of memory for rttov_mfasis failed")
    DEALLOCATE(w_sscat, ed_eff_sscat, fac_rm, STAT=err)
    THROWM(err.NE.0,"Deallocation of memory for rttov_mfasis failed")
  ENDIF
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_MFASIS', 1_jpim, ZHOOK_HANDLE)
  ...
  CONTAINS
    ...
    SUBROUTINE rttov_mfasis_interpolate( &
      ...
      INTEGER(jpim)                :: nd, k, l, j, d, d2, m, chi, n
      ...
      DO iwv = 1, nwv   ! water vapour
        ...
        IF (ABS(refl_aux(3,iwv) - refl_aux(2,iwv)) > 1E-4) THEN
        ...
        ELSE 
          ! saturated case: use linear interpolation to avoid division by 0
          refl_tot(iwv) =  refl_aux(1,iwv) + alb*(refl_aux(3,iwv) - refl_aux(1,iwv))
        ENDIF
        ...
        IF (PRESENT(refl_lin_coef)) THEN
          ...
          DO d = 1, nd
            ...
            ELSE 
              ! saturated case: use linear interpolation to avoid division by 0
              refl_lin_coef(d) = refl_lin_coef(d) + iw_wv(iwv)*  &
                                                    (refl_coef(1,d2,iwv) + alb   * (refl_coef(3,d2,iwv) - refl_coef(1,d2,iwv)))
            ENDIF
