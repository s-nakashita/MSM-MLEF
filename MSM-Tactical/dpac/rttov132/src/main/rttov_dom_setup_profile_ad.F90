...
#include "rttov_errorreport.interface"
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_DOM_SETUP_PROFILE_AD', 0_jpim, ZHOOK_HANDLE)
...
!       profiles_dom_ad(col,i)%layerod(:) = 0._jprb
    ...
    WHERE (ssa(:,:,i) >= 1._jprb - 1.E-6_jprb) ssa_ad(:,:,i) = 0._jprb
...
!     ssa_ad(:,:,i) = 0._jprb
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_DOM_SETUP_PROFILE_AD', 1_jpim, ZHOOK_HANDLE)
