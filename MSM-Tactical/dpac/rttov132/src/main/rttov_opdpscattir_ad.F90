...
!   LOGICAL(KIND=jplm) :: do_dom, do_chou_scaling, do_single_scatt
  LOGICAL(KIND=jplm) :: do_aer_or_ray_dom
  LOGICAL(KIND=jplm) :: do_mfasis_lut, do_mfasis_nn, do_mfasis
  ...
  REAL(KIND=jprb)    :: clw, clw_ad, tmpval, tmpval2
  REAL(KIND=jprb)    :: gamma, ray_phup, ray_lcoef_2, cosscata
...
!   do_single_scatt = do_solar .AND. opts%rt_ir%vis_scatt_model == vis_scatt_single
  do_mfasis_lut = do_solar .AND. opts%rt_ir%vis_scatt_model == vis_scatt_mfasis
  do_mfasis_nn = do_solar .AND. opts%rt_ir%vis_scatt_model == vis_scatt_mfasis_nn
  do_mfasis = do_mfasis_lut .OR. do_mfasis_nn
  ...
  IF (do_thermal .OR. .NOT. do_mfasis) THEN
    ...
    !----------------------------------------------------------------------------
    DO j = 1, nchanprof
      IF (solar(j) .AND. do_mfasis) CYCLE
  ...
  IF (opts%rt_ir%addclouds) THEN
    ...
    ELSE
      ...
      DO j = 1, nchanprof
        ...
        do_chou_scaling_chan = thermal(j) .AND. opts%rt_ir%ir_scatt_model == ir_scatt_chou
        do_single_scatt_chan = solar(j) .AND. opts%rt_ir%vis_scatt_model == vis_scatt_single
        do_mfasis_chan = solar(j) .AND. do_mfasis
        do_chan = thermal(j) .OR. solar(j)

        IF (do_mfasis_chan .AND. do_mfasis_lut) THEN
              ...
              ELSE
                ...
                IF (do_single_scatt_chan .OR. do_dom_chan) THEN
                  ...
                  IF (do_dom_chan) THEN
                    ...
                    CALL phasefn_interp(thisnphangle, thiscosphangle(thisnphangle:1:-1), &
...
!                       legcoef(:) = legcoef(:) + thislegcoef(:) * sca
                ...
                IF (do_single_scatt_chan .OR. do_dom_chan) THEN
                  ...
                  IF (do_dom_chan) THEN
                    ...
                    CALL phasefn_interp_ad(thisnphangle, thiscosphangle(thisnphangle:1:-1), &
    ...
    ELSE
      ...
      DO j = 1, nchanprof
        ...
        IF (do_rayleigh_dom .AND. solar(j) .AND. &
          ...
          IF (opts%rt_ir%rayleigh_depol) THEN
            ! Account for depolarisation
            gamma = coef%rayleigh_depol_gamma(chan)
            ray_phup = (1._jprb + 3._jprb * gamma + (1._jprb - gamma) * cosscata**2) * &
                       0.75_jprb / (1._jprb + 2._jprb * gamma)
            ray_lcoef_2 = 0.5_jprb * (1._jprb - gamma) / (1._jprb + 2._jprb * gamma)
          ELSE
            ! Simple phase fn, no depolarisation
            ray_phup = 0.75_jprb * (1._jprb + cosscata * cosscata)
            ray_lcoef_2 = 0.5_jprb
          ENDIF
        ENDIF
...
!               ELSE
          ...
          IF (do_dom_chan) THEN
            IF (do_rayleigh_dom .AND. solar(j) .AND. &
              ...
              sca_ad = 1._jprb * legcoef_ad(0) + ray_lcoef_2 * legcoef_ad(2) + sumsca_ad
              raytracing_ad%ltick(lay,prof) = raytracing_ad%ltick(lay,prof) - &
...
CONTAINS
  ...
  SUBROUTINE calc_user_opt_param_ad(opt_param, opt_param_ad, aercld, aercld_ad, iaercld, do_rayleigh_dom)
    ...
    IF (do_rayleigh_dom) THEN
      ray_lcoef = 0.
      ray_lcoef(0) = 1._jprb
      ray_lcoef(2) = 0.5_jprb ! This is overriden below for depolarised option
    ENDIF
    ...
    DO j = 1, nchanprof
      ...
      IF (do_rayleigh_dom) THEN
        ...
        IF (opts%rt_ir%rayleigh_depol) THEN
          ...
          ray_lcoef(2) = 0.5_jprb * (1._jprb - gamma) / (1._jprb + 2._jprb * gamma)
        ELSE
          ! Simple phase fn, no depolarisation
          ray_phup = 0.75_jprb * (1._jprb + cosscata * cosscata)
        ENDIF
