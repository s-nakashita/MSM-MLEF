...
!! @param[in]     do_mfasis_lut  flag to indicate MFASIS-LUT simulation
...
!! @param[in]     emissivity     RTTOV emissivity structure, optional
...
!INTF_ON
  ...
  LOGICAL(KIND=jplm),      INTENT(IN)           :: do_mfasis_lut
...
#include "rttov_errorreport.interface"
  ...
  REAL(KIND=jprb)    :: wind, zmax, tskin_max
...
!$OMP END CRITICAL
    ...
    IF ((dothermal .OR. .NOT. do_mfasis) .AND. &
        .NOT. opts%rt_all%use_tskin_eff) THEN
      ! surface skin temperature
      IF (profiles(iprof)%skin%surftype == surftype_land) THEN
        tskin_max = tskin_land_max
      ELSE
        tskin_max = tmax
      ENDIF
      IF (profiles(iprof)%skin%t > tskin_max .OR. &
  ...
  IF (dothermal .AND. opts%rt_all%use_tskin_eff) THEN
    IF (ANY(emissivity%tskin_eff < tmin)) THEN
      err = errorstatus_fatal
      THROWM(err.NE.0, "invalid input effective Tskin"//sprof)
    ENDIF
    ...
    DO i = 1, SIZE(chanprof)
      ...
      ELSE
        tskin_max = tmax
      ENDIF
      IF (emissivity(i)%tskin_eff > tskin_max) THEN
        err = errorstatus_fatal
        THROWM(err.NE.0, "invalid input effective Tskin"//sprof)
      ENDIF
    ENDDO
  ...
  ! Set MFASIS-LUT quality flags on viewing geometry
  IF (do_mfasis_lut) THEN
    DO i = 1, SIZE(chanprof)
