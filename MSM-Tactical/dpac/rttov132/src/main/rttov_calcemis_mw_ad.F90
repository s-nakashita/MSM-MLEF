...
!! @param[in]     thermal             flags indicating active channels
...
!! @param[in]     tskin_eff           effective skin temperatures
!! @param[in,out] tskin_eff_ad        effective skin temperature increments
...
!INTF_OFF
  ...
  USE mod_rttov_fastem5_coef, ONLY : transmittance_limit_lower, transmittance_limit_upper, t_c5
  USE rttov_tessem_mod, ONLY : rttov_tessem_ad
  USE rttov_surfem_ocean_ad_mod, ONLY : rttov_surfem_ocean_ad
...
!subroutine arguments:
  ...
  REAL(KIND=jprb),              INTENT(IN)            :: tskin_eff   (size(chanprof))
  REAL(KIND=jprb),              INTENT(INOUT)         :: tskin_eff_ad(size(chanprof))
...
!local variables:
  ...
  REAL(KIND=jprb) :: wind10_direction, windangle, windratio, abs_windratio ! Note wind azimuth is in radians
  ...
  INTEGER(KIND=jpim) :: i, j, chan, istokes, ifreq, m, iz, jz
  ...
  REAL   (KIND=jprb) :: wind10_direction_ad, windangle_ad, windratio_ad, abs_windratio_ad ! Note wind azimuth is in radians
...
! rttov_fastem4
  REAL(KIND=jprb) :: Transmittance, Rel_Azimuth
  REAL(KIND=jprb), DIMENSION(4) :: jemissivity, jreflectivity
  REAL(KIND=jprb) :: Transmittance_ad, Rel_Azimuth_ad
...
!-------------------------------------------
      ...
      ELSE
        ...
        ENDIF
      ENDIF
      abs_windratio    = abs(windratio)
      windangle        = atan(abs_windratio)
      wind10_direction = quadcof(iquadrant, 1) * pi + windangle * quadcof(iquadrant, 2)
      SELECT CASE (opts%rt_mw%fastem_version)
      CASE (0)
        emissstokes_ad(i,1:2) = emissstokes_ad(i,1:2) - reflectstokes_ad(i,1:2)
        emissstokes_ad(i,3:4) = 0._jprb
        reflectstokes_ad(i,3:4) = 0._jprb
        CALL rttov_tessem_ad(freq_ghz, prof%zenangle, wind10, tskin_eff(i), &
                             prof%skin%salinity, wind10_ad, &
                             tskin_eff_ad(i), prof_ad%skin%salinity, &
...
!Set values for temperature polynomials (convert from kelvin to celsius)
        ...
        tcelsius       = tskin_eff(i) - 273.15_JPRB
...
! Angle between wind direction and satellite azimuth angle
          IF (opts%rt_mw%fastem3_rwd_fix) THEN
            phi = prof%azangle * pi / 180.0_JPRB - wind10_direction
          ELSE
            phi = pi - (wind10_direction - prof%azangle * pi / 180.0_JPRB) ! Incorrect version
          ENDIF
...
!------
        ...
        ELSE
          wind10_sq_ad = 0._jprb
          fresnel_v_ad = - emissstokes_ad(i, 1)
          fresnel_h_ad = - emissstokes_ad(i, 2)
        ENDIF
!1.3.1) Fresnel reflection coefficients
!------
...
!Set values for temperature polynomials (convert from kelvin to celsius)
        ...
        tskin_eff_ad(i)   = tskin_eff_ad(i) + tcelsius_ad
      ...
      CASE (4:6)
        ...
        Rel_Azimuth         = prof%azangle - wind10_direction * 180.0_JPRB / pi
        ...
        CALL rttov_fastem5_AD( &
              ...
              & Transmittance_ad,                &
              & Rel_Azimuth_ad,                  &
              & opts%rt_mw%supply_foam_fraction, &
              & prof%skin%foam_fraction,         &
              & prof_ad%skin%foam_fraction)
      ...
      CASE (7)
        !===direct code===
        Transmittance       = transmission_aux%thermal_path1%tau_surf(0, i)
        Rel_Azimuth         = prof%azangle - wind10_direction * 180.0_JPRB / pi ! relative azimuth angle in degrees

        CALL rttov_surfem_ocean(freq_ghz, prof%zenangle, wind10, tskin_eff(i), prof%skin%salinity, Rel_Azimuth, &
                                emissstokes(i,1), emissstokes(i,2), emissstokes(i,3), emissstokes(i,4))
        ...
        IF( Transmittance > transmittance_limit_lower .and. Transmittance < transmittance_limit_upper) THEN
          ...
          variance = varm * ( t_c5(44) * freq_ghz + t_c5(45) )
          test_variance = variance
          IF ( variance >= varm ) variance = varm
          IF ( variance <= 0.0_JPRB ) variance = 0.0_JPRB
          !Compute surface to space optical depth
          opdpsfc = -LOG(Transmittance ) * geom%coszen 
          !Define nine predictors for the effective angle calculation
          zx(1) = 1.0_JPRB
          zx(2) = variance
          zx(4) = 1.0_JPRB / geom%coszen
        ...
        zreflmod_v_ad       = 0.0_JPRB
        zreflmod_h_ad       = 0.0_JPRB
        opdpsfc_ad          = 0.0_JPRB
        variance_ad         = 0.0_JPRB
        varm_ad             = 0.0_JPRB

        zreflmod_v_ad = zreflmod_v_ad + reflectstokes_ad(i,1) * (1.0_JPRB - emissstokes(i,1)) 
        emissstokes_ad(i,1) = emissstokes_ad(i,1) - (zreflmod_v * reflectstokes_ad(i,1))

        zreflmod_h_ad = zreflmod_h_ad + reflectstokes_ad(i,2) * (1.0_JPRB - emissstokes(i,2)) 
        emissstokes_ad(i,2) = emissstokes_ad(i,2) - (zreflmod_h * reflectstokes_ad(i,2))

        zreflmod_v_ad = zreflmod_v_ad - (0.5_JPRB * reflectstokes_ad(i,3) * emissstokes(i,3))
        zreflmod_h_ad = zreflmod_h_ad - (0.5_JPRB * reflectstokes_ad(i,3) * emissstokes(i,3)) 
        emissstokes_ad(i,3) = emissstokes_ad(i,3) - (0.5_JPRB * (zreflmod_v + zreflmod_h) * reflectstokes_ad(i,3))    

        zreflmod_v_ad = zreflmod_v_ad - (0.5_JPRB * reflectstokes_ad(i,4) * emissstokes(i,4))
        zreflmod_h_ad = zreflmod_h_ad - (0.5_JPRB * reflectstokes_ad(i,4) * emissstokes(i,4))
        emissstokes_ad(i,4) = emissstokes_ad(i,4) - (0.5_JPRB * (zreflmod_v + zreflmod_h) * reflectstokes_ad(i,4))  

        IF( Transmittance > transmittance_limit_lower .AND. Transmittance < transmittance_limit_upper) THEN
          zrough_v_ad = 0.0_jprb
          zrough_h_ad = 0.0_jprb
          Transmittance_ad = Transmittance_ad + zreflmod_v_ad/(1.0_jprb-Transmittance)*(-zrough_v  &
              *Transmittance**(zrough_v-1.0_jprb) + zreflmod_v )
          zrough_v_ad = zrough_v_ad - (Transmittance**zrough_v * LOG(Transmittance) * zreflmod_v_ad)/ &
              (1.0_jprb-Transmittance)
          Transmittance_ad = Transmittance_ad + zreflmod_h_ad/(1.0_jprb-Transmittance)*(-zrough_h  &
              *Transmittance**(zrough_h-1.0_jprb) + zreflmod_h )
          zrough_h_ad = zrough_h_ad - (Transmittance**zrough_h * LOG(Transmittance) * zreflmod_h_ad)/ &
              (1.0_jprb-Transmittance)
          zx_ad(:) = 0.0_jprb
          DO iz = 1, 7
            jz = iz-1
            !Switched h to v Deblonde SSMIS june 7, 2001
            zx_ad(9) = zx_ad(9) + zrough_v_ad * zx(iz) * t_c5(24+jz*3)
            zx_ad(8) = zx_ad(8) + zrough_v_ad * zx(iz) * t_c5(23+jz*3)
            zx_ad(iz) = zx_ad(iz) + zrough_v_ad * (t_c5(22+jz*3) + zx(8)*t_c5(23+jz*3) + zx(9)*t_c5(24+jz*3) )
            zx_ad(9) = zx_ad(9) + zrough_h_ad * zx(iz) * t_c5(3+jz*3)
            zx_ad(8) = zx_ad(8) + zrough_h_ad * zx(iz) * t_c5(2+jz*3)
            zx_ad(iz) = zx_ad(iz) + zrough_h_ad * (t_c5(1+jz*3) + zx(8)*t_c5(2+jz*3) + zx(9)*t_c5(3+jz*3))
          END DO
          zrough_v_ad = 0.0_jprb
          zrough_h_ad = 0.0_jprb

          zx_ad(8) = zx_ad(8) + (2.0_jprb*zx_ad(9)*zx(8))
          opdpsfc_ad = opdpsfc_ad + (zx_ad(8) / opdpsfc)
          zx_ad(2) = zx_ad(2) + (2.0_jprb*zx_ad(7)*zx(2))
          zx_ad(3) = zx_ad(3) + (2.0_jprb*zx_ad(5)*zx(3))
          zx_ad(2) = zx_ad(2) + (zx_ad(3)*zx(4))
          zx_ad(4) = 0.0_jprb
          variance_ad = zx_ad(2)
          zx_ad(1) = 0.0_jprb
          ...
          !Compute surface to space optical depth
          Transmittance_ad = Transmittance_ad - (opdpsfc_ad * geom%coszen / Transmittance)
          IF (test_variance < varm) THEN
            varm_ad = variance_ad * (t_c5(44) * freq_ghz + t_c5(45))
          ELSE
            varm_ad = variance_ad
          ENDIF
          variance_ad = varm_ad * t_c5(43)
          wind10_ad   = wind10_ad + variance_ad * 0.00512_JPRB
        ELSE
          zreflmod_v_ad = 0.0_jprb
          zreflmod_h_ad = 0.0_jprb 
        ...
        CALL rttov_surfem_ocean_ad(freq_ghz, prof%zenangle, wind10, tskin_eff(i), prof%skin%salinity, Rel_Azimuth, &
                                emissstokes_ad(i,1), emissstokes_ad(i,2), emissstokes_ad(i,3), emissstokes_ad(i,4), &
                                wind10_ad, tskin_eff_ad(i), prof_ad%skin%salinity, Rel_Azimuth_ad)

        wind10_direction_ad = wind10_direction_ad - (Rel_Azimuth_ad * 180.0_JPRB / pi)
        transmission_aux_ad%thermal_path1%tau_surf(0, i) = transmission_aux_ad%thermal_path1%tau_surf(0, i) + &
                                                           Transmittance_ad
        wind10_sq_ad = 0._jprb

      END SELECT
      windangle_ad     = wind10_direction_ad * quadcof(iquadrant, 2)
      abs_windratio_ad = windangle_ad / (1.0_JPRB + abs_windratio * abs_windratio)
      windratio_ad     = quadcof(iquadrant, 2) * abs_windratio_ad
      IF (abs(prof%s2m%v) >= windlimit) THEN
