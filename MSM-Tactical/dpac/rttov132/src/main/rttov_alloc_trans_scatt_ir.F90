...
#include "rttov_init_trans_scatt_ir.interface"
  ...
  LOGICAL(KIND=jplm) :: do_mfasis, do_mfasis_lut, do_mfasis_nn
  LOGICAL(KIND=jplm) :: do_single_scatt, do_thermal
!- End of header --------------------------------------------------------
  TRY
  ...
  IF (asw == 1) THEN
    ...
    do_mfasis_lut = opts%rt_ir%addsolar .AND. opts%rt_ir%vis_scatt_model == vis_scatt_mfasis
    do_mfasis_nn = opts%rt_ir%addsolar .AND. opts%rt_ir%vis_scatt_model == vis_scatt_mfasis_nn
