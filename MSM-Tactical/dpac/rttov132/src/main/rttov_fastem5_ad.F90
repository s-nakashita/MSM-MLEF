...
!! @param[in,out] emissivity_ad         emissivity increment (4 Stokes components)
!! @param[in,out] reflectivity_ad       reflectivity increment (4 Stokes components)
!! @param[in,out] temperature_ad        profile skin temperature increment
!! @param[in,out] salinity_ad           profile salinity increment
!! @param[in,out] wind_speed_ad         profile wind speed increment
...
!! @param[in,out] transmittance_ad      surface-to-space transmittance increment
!! @param[in,out] rel_azimuth_ad        relative azimuth angle increment
!! @param[in]     supply_foam_fraction  flag to indicate user is supplying foam fraction
!! @param[in]     foam_fraction         user supplied foam fraction
!! @param[in,out] foam_fraction_ad      user foam fraction increment
!
! Copyright:
...
!
  SUBROUTINE rttov_fastem5_ad(fastem_version,       &  ! Input
                              ...
                              Supply_Foam_Fraction, &  ! Input
                              Foam_Fraction,        &  ! Input
                              Foam_Fraction_ad)        ! Output
...
!INTF_ON
    ...
    LOGICAL(jplm), INTENT(IN)    :: Supply_Foam_Fraction
...
!    REAL(fp) ::
    ...
    REAL(fp) :: asc(4,3), fre_c, phi, wind10
  ...
  ! ad part
    ...
    REAL(jprb)    :: u19, u19_ad, freq_ghz, dfreq
    ...
    REAL(jprb)    :: tbfixed_ad(4,4,3)
    REAL(jprb)    :: efixed_ad(4,4,3)
    REAL(jprb)    :: einterpolated_ad(4,3)
    REAL(jprb)    :: a1e_ad, a2e_ad, a3e_ad
  ...
  !  ****** start of adjoint code ******
    ...
    IF (fastem_version == 6) THEN
      zreflmod_v_ad = -0.5_jprb * Reflectivity_ad(3) * Emissivity(3) - &
                       0.5_jprb * Reflectivity_ad(4) * Emissivity(4)
      zreflmod_h_ad = zreflmod_v_ad
      Emissivity_ad(3) = Emissivity_ad(3) - &
        0.5_jprb * (zreflmod_v + zreflmod_h) * Reflectivity_ad(3)
      Emissivity_ad(4) = Emissivity_ad(4) - &
        0.5_jprb * (zreflmod_v + zreflmod_h) * Reflectivity_ad(4)
    ELSE
      ! FASTEM-4/5 set Stokes 3/4 refl to zero: this is not recommended
      Reflectivity_ad(3:4) = ZERO
    ENDIF
  ...
  IF (fastem_version == 6) THEN  !M.Kazumori azimuth model function
    ...
    DO istokes = 3, 4
      DO M = 1, 3
        einterpolated_ad(istokes, M) =      &
          & Azimuth_Emi_ad(istokes) * sin(m * phi) * (1.0_JPRB - cos_z) / (1.0_JPRB - 0.6018_JPRB)
        phi_ad = phi_ad +                                                                                   &
          & Azimuth_Emi_ad(istokes) * einterpolated(istokes, M) * m * cos(m * phi) * (1.0_JPRB - cos_z) /  &
    ...
    DO M = 1, 3
      IF (freq_ghz .LE. freqfixed(1)) THEN
        efixed_ad(1, 3:4, M) = efixed_ad(1, 3:4, M) + einterpolated_ad(3:4, M)
      ELSE IF (freq_ghz .GE. freqfixed(4)) THEN
        efixed_ad(4, 3:4, M) = efixed_ad(4, 3:4, M) + einterpolated_ad(3:4, M)
      ELSE
        IF (freq_ghz .LT. freqfixed(2)                                 ) ifreq = 2
        IF (freq_ghz .LT. freqfixed(3) .AND. freq_ghz .GE. freqfixed(2)) ifreq = 3
        IF (freq_ghz .GE. freqfixed(3)                                 ) ifreq = 4
        dfreq = (freq_ghz - freqfixed(ifreq - 1)) / (freqfixed(ifreq) - freqfixed(ifreq - 1))
        efixed_ad(ifreq, 3:4, M)     = efixed_ad(ifreq, 3:4, M) + einterpolated_ad(3:4, M) * dfreq
        efixed_ad(ifreq - 1, 3:4, M) = efixed_ad(ifreq - 1, 3:4, M) + einterpolated_ad(3:4, M) * (1.0 - dfreq)
      ENDIF
      DO istokes = 3, 4
    ...
    DO ich = 0, 15
      ...
      istokes = mod(ich, 4_jpim) + 1
      a3e_ad   = tbfixed_ad(istokes, i_freq, 3)
      a2e_ad   = tbfixed_ad(istokes, i_freq, 2)
      a1e_ad   = tbfixed_ad(istokes, i_freq, 1)
      u19_ad   =      &
