...
cd $RTTOV_GUI_PREFIX/test
coverage_report_file=coverage.report.log
 
has_coverage="no"
echo "python is:"
which_python=`which python`
echo "python is: " ${which_python}
pythonprefix=`dirname ${which_python}`
command -v ${pythonprefix}/coverage  > /dev/null 2>&1 &&  has_coverage="yes"
echo "has_coverage=${has_coverage}"
echo "want_coverage=${want_coverage}"
echo "python is:"
which python
...
do
  ...
  echo "log_file: " ${log_file} "for " ${mytest}
  ...
  then
    ...
    echo "lauching: python -m test.${mytest} >> ${log_file} 2>&1"
