...
class Test(rttovgui_unittest_class.RttovGuiUnitTest):
    ...
    def setUp(self):
        ...
        logging.basicConfig(filename=(self.p.config.ENV['GUI_WRK_DIR'] +
                            ...
                            datefmt="%Y:%m:%d %H:%M:%S",
                            filemode="w")
        logging.info("start test options")
    ...
    def test_solar1(self):
        self.p.myCoeffs.fileName["standard"] = self.p.config.ENV[
            "RTTOV_GUI_COEFF_DIR"] + "/rttov13pred54L/rtcoef_eos_2_modis_o3co2.dat"
    ...
    def test_N2O(self):
        ...
        coefFile = self.p.config.ENV[
            "RTTOV_GUI_COEFF_DIR"] + "/rttov7pred101L/rtcoef_metop_2_iasi.H5"
        self.p.myCoeffs.fileName["standard"] = coefFile
        print("coefFile", coefFile)
        err = self.p.loadCoefficients()
        self.assertEqual(err, 0)
        ...
        coefFile = self.p.config.ENV[
            "RTTOV_GUI_COEFF_DIR"] + "/rttov13pred54L/rtcoef_msg_2_seviri_o3co2.dat"
        self.p.myCoeffs.fileName["standard"] = coefFile
        err = self.p.loadCoefficients()
        self.assertEqual(err, 0)
        print("has N2O?")
