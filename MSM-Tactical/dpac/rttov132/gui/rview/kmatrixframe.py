...
class KMatrixFrame(util.GenericView):
    ...
    def __init__(self, parent, title, fname, runNumber=1, project=None):
        ...
        for ilev in range(0, levx):
            self.tabilevels[ilev] = round(ilev * self.lev // levx)

        if self.lev < levx:
            tabNP = np.zeros(levx)
            for ilev in range(0, levx):
                tabNP[ilev] = self.tabP[self.tabilevels[ilev]]
            self.tabP = tabNP
...
#
    ...
    def canDisplayKprofile(self):
        ...
        if len(self.myProject.aKmats.aKmat) > 0:
            return self.runNumber == self.myProject.aKmats.aKmat[-1].runNumber
        else:
            return False

    def onkeypress(self, evt):
        if evt.key not in ['p', 'P'] or not evt.inaxes:
            return
    ...
    def showkprof(self, c):
        ...
        parent = self.myParent if self.myParent is not None else self
    ...
    def extract_kgas(self, gas):
        # levels : put all profils on 101 levels

        tab101 = np.zeros((levx, self.chn))
        for ilev in range(levx):
            tab101[ilev][:] = self.kmat.kmatrix[gas][self.tabilevels[ilev]][:]
        ...
        kmoy = (kmax + kmin) / 2
        pc = 0
        for ic in range(self.chn):

            for _ in range(self.barwidth - 1):
