...
class PlotPage(wx.Panel):
    ...
    def OnPlot(self, theValues, i, theLabel="try 1"):
        n = 1 if len(theValues[i].shape) == 0 else theValues[i].shape[0]
        theMarker = "+" if n < 10 else None
        self.fig.clear()
    ...
    def PlotDif(self, diffRun):
        ...
        self.fig.clear()
        self.subP = self.fig.add_subplot(1, 1, 1)
        n = 1 if len(self.theValues[0].shape) == 0 else self.theValues[
            0].shape[0]
        theMarker = "+" if n < 10 else None
        x = list(range(1, n + 1))
...
class RBtView(util.GenericView):
    ...
    def __init__(self, parent, title, radFileNameT, radFileNameBg, counter=1):
        ...
        self.nchan = 1 if len(self.radT["BT"].shape) == 0 else self.radT[
            "BT"].shape[0]
...
if __name__ == "__main__":
    ...
    fileNameT = data_test_dir + '/radrTrue.h5'
    fileNameBg = data_test_dir + '/radrBg.h5'

    sv = RBtView(None,  "BT view", fileNameT, fileNameBg, 1)
