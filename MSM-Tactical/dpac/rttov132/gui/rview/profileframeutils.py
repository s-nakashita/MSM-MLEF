...
class Data(wxmpl.Channel):
    ...
    def __init__(self, x, y, theName="", theColor=None, theStyle="-",
        ...
        self.label = theName if theLabel is None else theLabel
        self.changed = False
...
class GenericPlotItemPanel(wx.Panel):
    ...
    def PrepareAxeAndData(self):
        ...
        if self.kind == "GASES":
            self.y = self.pression[
                ::1] if self.yInPressions else numpy.arange(
                    1, self.value.shape[0] + 1, 1)

        elif self.yInPressions:
            self.y = self.layer[::1]
        else:
        ...
        if self.theName == "T" and len(self.tskin) > 0:
            if self.yInPressions:
        ...
        if not self.layerstyle:
            ...
            for index in range(self.y.shape[0]):
                ...
                if dist < mini:
                    ...
                    mini = dist
            self.x[
                imin] = 0 if self.kind != "GASES" and not self.onInsert else e.xdata
            self.data.setChanged(True)
            self.data.myUpdate(self.x, self.y)
        else:
            ...
            if not self.onInsert:
                ...
                elif imin != self.xlayeritem.shape[0]:
                    self.xlayeritem[imin + 1] = 0
            else:
    ...
    def GetItem(self):
        ...
        self.MyReleaseMouse()
        myX = self.data.getX()
        return self.myLayeritem.getLayeritem(myX) if self.layerstyle else myX
...
class PlotItemPanelAll(wx.Panel):
    ...
    def __init__(self, parent, theProfile, kind="GASES", layer=None,
        ...
        self.itemsList = {'GASES': self.myProfile.gas_list,
                          'AEROSOLS': self.myProfile.aerosol_list,
                          'CLOUDS': self.myProfile.cloud_list}
        self.labelx = {}
    ...
    def PrepareAxeAndData(self, theProfile=None):
        ...
        if self.yInPressions:
            ...
            y = self.myProfile['P'][
                ::1] if self.kind == 'GASES' else self.layer[::1]
        ...
        if self.yInPressions:
            self.axesT.set_yscale("log")
            self.axes.set_ylim((self.pression[-1] + 150, self.pression[0]))
            self.axesT.set_ylim((self.pression[-1] + 150, self.pression[0]))
        else:
            self.axes.set_ylim(self.myProfile['T'].shape[0] + 2, 1)
            self.axesT.set_ylim(self.myProfile['T'].shape[0] + 2, 1)
    ...
    def PlotCurves(self):
        ...
        if lenAll > 0:
            ...
            for lindex in range(lenAll):
                c = self.myCurves[lindex]["line"]
                data = self.myCurves[lindex]["data"]
                c.set_xdata(data.x)
                self.fig.canvas.draw_idle()
        else:
            # first time plot
