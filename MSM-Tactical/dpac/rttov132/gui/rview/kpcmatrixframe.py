...
#
#  index equivalence table
        self.tabilevels = np.zeros(levx, int)
        for ilevel in range(0, levx):
            self.tabilevels[ilevel] = round(ilevel * self.lev / levx)

        self.tabP = self.kmat.profile['P']
        if self.lev < levx:
            tabNP = np.zeros(levx)
            for ilevel in range(0, levx):
                tabNP[ilevel] = self.tabP[self.tabilevels[ilevel]]
            self.tabP = tabNP
...
#
    ...
    def extract_kgas(self, gas):
        ...
        if self.lev < levx:
            ...
            for li in range(levx):
                tab101[li][:] = self.kmat.kpcmatrix[gas][self.tabilevels[
                    li]][:]
...
#
# min/max for the colour divider strip
#
        kmin = np.amin(self.kmat.kpcmatrix[gas])
        kmax = np.amax(self.kmat.kpcmatrix[gas])
        kmoy = (kmax + kmin) / 2
        ...
        for ic in range(self.chn):
            for ir in range(self.barwidth - 1):
    ...
    def draw_plot(self, plot, gas, comment):
        ...
        # trick for hide the 0
        my_ticks = self.subP[plot].get_xticks()
        xlabel = np.arange(len(my_ticks))
