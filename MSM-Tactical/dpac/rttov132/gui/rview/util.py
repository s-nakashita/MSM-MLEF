...
class MyCustomToolbar(ToolBar):
    ...
    def __init__(self, plotCanvas):
        ...
        ToolBar.__init__(self, plotCanvas, coordinates=False)
...
class GenericView(wx.Frame):
    ...
    def OnOpenFile(self, event, directory="", title="Choose a file",
        ...
        if directory == "":
            directory = os.getcwd()
        myflag = wx.FD_OPEN if wx.VERSION[0] >= 4 else wx.OPEN
        dlg = wx.FileDialog(self, title, directory,
                            defaultFile, wildcard, myflag)
        self.profileFile = None
        mypath = dlg.GetPath() if dlg.ShowModal() == wx.ID_OK else None
        dlg.DeletePendingEvents()
        wx.CallAfter(dlg.Destroy)
        return mypath
