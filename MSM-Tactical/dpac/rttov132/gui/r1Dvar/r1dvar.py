...
class Project1dvar(object):
    ...
    def readRmatrix(self, satellite, instrument, filename=None):
        ...
        if instrument in ("hirs", "amsua", "amsub", "mhs"):
            ...
            instrument = "CrIS" if instrument == "cris" else instrument.upper()
        ...
        if filename is not None:
            ...
            try:
                ...
                raise ValueError("wrong instrument" + str(instrument)) from e
            ...
            try:
                if len(list(self.Rmatrix.Rmatrix.keys())) == 1:
                    ...
                    if self.instrument == "amsua":
                        self.Rmat = self.Rmatrix.Rmatrix[
                            ...
                            20:35, 20:35]
                    elif self.instrument in ["amsub", "MHS", "mhs"]:
                        self.Rmat = self.Rmatrix.Rmatrix[
                            satellite + " " + "ATOVS"]["matrix"][
                            35:40, 35:40]
                        print(self.Rmat)
                    elif self.instrument == "hirs":
                ...
                raise ValueError("wrong satellite and instrument " + str(
                    satellite) + " " + str(instrument)) from exc

        # keep only selected channels

        if self.channel_list is None:
            logging.info("no channel list take all of the matrix")
            self.channel_list = list(range(self.Rmat.shape[0]))
        else:
    ...
    def assumptionOnSurfaceParameters(self, profileTrue, profileBg):
        ...
        print("assumptionOnSurfaceParameters")
    ...
    def retrieve1d(self):
        ...
        logging.info("apply scalling factorB to Bmat {}".format(self.factorB))
        ...
        logging.info("apply scalling factorR to Rmat {}".format(self.factorR))
...
#
        ...
        logging.info("set noise {}".format(self.MaxNoise))
