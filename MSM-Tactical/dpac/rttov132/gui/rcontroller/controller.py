...
class MainController (GenericController):
    ...
    def __init__(self, app, debug):
        ...
        pub.subscribe(self.NewInstrumentListener, "New INSTRUMENT")
        self.write("Welcome  ! ")
    ...
    def NewInstrumentListener(self, msg):
        """ must be called when the coefficient are loaded  """
        self.prepLog()
        if self.project.myCoeffs.loadCoeffs and self.myViews[radViewTitle]:
            self.mv.write("instrument change : close the radiance window")
            self.myViews[radViewTitle].ShowInfoMessage(
...
def main():
    ...
    parser.add_argument('-m', '--mfasis-lut', dest="mfasislut",
                        ...
                        metavar="FILE",
                        type=lambda x: is_valid_file(parser, x))
    parser.add_argument('-n', '--mfasis-nn', dest="mfasisnn",
                        help="mfasis nn file",  required=False,
                        metavar="FILE",
                        type=lambda x: is_valid_file(parser, x))
    ...
    if args.mfasislut is not None:
        controller.project.myCoeffs.fileName[
            "mfasis-cloud"] = args.mfasislut.name
    if args.mfasisnn is not None:
        controller.project.myCoeffs.fileName["mfasis-nn"] = args.mfasisnn.name
