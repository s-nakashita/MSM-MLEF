...
class OptionObject(object):
    ...
    def getValue(self):
        ...
        return self.odict[
            self.value] if self.value in theKeys else self.odict[
                list(theKeys)[0]]
    ...
    def __deepcopy__(self):
        ...
        return OptionObject(otype, value, min_max_values,
                            odict, comment, status,
                            theme, units, hidden)
...
class Option(OrderedDict, _V):
    ...
    optionsThemes = ['General configuration options',  # 0
                     'General RT options',  # 1,
                     'VIS/IR-only RT options',  # 2
                     'PC-RTTOV options',  # 3
    ...
    dictOptions = {"IPCREG": fullIPCREG,
                   ...
                   "VIS_SCATT_MODEL": {1: "DOM",
                                       2: "single scattering"},
                   "VIS_SCATT_MODEL_WITH_MFASIS": {1: "DOM",
                                                   2: "single scattering",
                                                   3: "MFASIS-LUT"},
                   "VIS_SCATT_MODEL_WITH_MFASIS_NN": {1: "DOM",
                                                      2: "single scattering",
                                                      4: "MFASIS-NN"},
    ...
    def __init__(self):
        ...
        self["DO_CHECKINPUT"] = OptionObject(
            ...
            "checking",
            theme=0)
        self["OPDEP13_GAS_CLIP"] = OptionObject(
            comment="Switch for v13pred individual gas -ve optical "
            "depth clipping",
            value=True,
            theme=0)
        ...
        self["RAD_DOWN_LIN_TAU"] = OptionObject(
            ...
            theme=1, value=True,
            hidden=False)
        self["TRANSMITTANCES_ONLY"] = OptionObject(
            comment="Switch to compute transmittances, no radiances",
            theme=1, value=False,
            hidden=True)
        self["USE_TSKIN_EFF"] = OptionObject(
            comment="Switch to enable per-channel effective Tskin inputs",
            theme=1, value=False,
            hidden=True)
        ...
        self["DOM_OPDEP_THRESHOLD"] = OptionObject(
            ...
            " not applied if <= 0",
            theme=2)
        self["DOM_RAYLEIGH"] = OptionObject(theme=2)
        self["RAYLEIGH_DEPOL"] = OptionObject(theme=2, value=True)
        self["OZONE_DATA"] = OptionObject(theme=1)
        self["CO2_DATA"] = OptionObject(theme=1)
        self["N2O_DATA"] = OptionObject(theme=1)
        self["CO_DATA"] = OptionObject(theme=1)
        self["CH4_DATA"] = OptionObject(theme=1)
        self["SO2_DATA"] = OptionObject(theme=1)
        self["CLOUD_OVERLAP"] = OptionObject(theme=2, hidden=True, value=1)
        ...
        self["FASTEM_VERSION"] = OptionObject(
            ...
            otype=dict,
            value=6,
            odict=self.dictOptions["FASTEM_VERSION"],
            comment="TESSEM2 model or FASTEM version (1-7)")
        self["FASTEM3_RWD_FIX"] = OptionObject(
            theme=4,
            value=True,
            hidden=True,
            comment="Enable the FASTEM-3 relative wind direction bug fix")
        ...
        self.optionsThemesList = {theme: [] for theme in self.optionsThemes}
...
# <type 'dict'>
        for option in list(self.keys()):
            self[f'{option}_ATTRIBUTE'] = {'COMMENT': self[option].comment,
                                           'UNITS': 'n/a'}
    ...
    def loadh5(self, h5):
        ...
        for cle in list(h5):
            if (cle in self.options_list_logical):
                ...
                self[f'{cle}_ATTRIBUTE'] = getAttributes(h5[cle])
            checkAttributes(self[f'{cle}_ATTRIBUTE'])
