...
class Profile(dict):
    ...
    profile_list = ['AEROSOLS', 'AZANGLE', 'BE', 'CFRAC', 'CFRACTION', 'CH4',
                    ...
                    'ICEDE_PARAM', 'ICE_SCHEME', 'CLW_SCHEME',
                    'LATITUDE', 'LONGITUDE',
    ...
    def checkAerosolKind(self):
        ...
        for item in self.keys():
            if item in self.opac_aerosol_list and self[item] is not None:
                kind_opac = kind_opac + 1
            if item in self.cams_aerosol_list and self[item] is not None:
    ...
    def checkProfileAsciiInput(self):
        ...
        if "GAS_UNITS" not in self or self["GAS_UNITS"] is None:
            self["GAS_UNITS"] = 2  # defaut case
        if "MMR_CLDAER" not in self or self["MMR_CLDAER"] is None:
            self["MMR_CLDAER"] = 1  # defaut case
        self.defineUnitsforCommentAttribute()
    ...
    def save(self, filename):
        """save the profile in filename in default group /PROFILES/0001/"""
        print("save profile on", filename)
        of = h5py.File(filename, 'w')
        self.saveh5(of, "/PROFILES/0001/")
        of.close()
    ...
    def addAerosol(self, aerosol, *conc):
        ...
        for i in range(self['NLAYERS']):
            ...
            if (self['P'][i] > pmin and self['P'][i] < pmax):
                self[aerosol][i] += conc or self.defaultaerdensity[
                    self["MMR_CLDAER"]]
...
if __name__ == '__main__':
    ...
    p1.save("rttov_profile_diroutput_profile_2.H5")
