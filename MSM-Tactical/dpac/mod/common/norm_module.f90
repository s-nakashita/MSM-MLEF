...
! 
  ...
  public :: calc_te, calc_tegrd, calc_teprof, &
            calc_te2, calc_tegrd2, calc_teprof2
  contains
!======================================================================
...
!======================================================================
! calculate moist total energy for each grids from 2 different perturbations
!======================================================================
subroutine calc_tegrd2(u1,u2,v1,v2,t1,t2,q1,q2,ps1,ps2,&
                epsq,clat,si,nlon,nlat,kmax,te)
  implicit none
  integer, intent(in) :: nlon, nlat, kmax! boundaries
  real(kind=dp), intent(in) :: u1(:,:,:),v1(:,:,:),t1(:,:,:),q1(:,:,:)
  real(kind=dp), intent(in) :: ps1(:,:)
  real(kind=dp), intent(in) :: u2(:,:,:),v2(:,:,:),t2(:,:,:),q2(:,:,:)
  real(kind=dp), intent(in) :: ps2(:,:)
  real(kind=dp), intent(in) :: epsq ! weight for moist term
  real(kind=dp), intent(in) :: clat(:),si(:)
  real(kind=dp), intent(out):: te(:,:,:,:)
  real(kind=dp) :: area,coef
  ...
  do k=1,kmax
    do j=1,nlat
      do i=1,nlon
        !KE
        te(i,j,k,1)=0.5d0*(u1(i,j,k)*u2(i,j,k)+v1(i,j,k)*v2(i,j,k))
        !PE(T)
        te(i,j,k,2)=0.5d0*cp/tr*t1(i,j,k)*t2(i,j,k)
        !LE
        te(i,j,k,3)=0.5d0*epsq*lh**2/cp/tr*q1(i,j,k)*q2(i,j,k)
      end do
    end do
  end do
  do j=1,nlat
    do i=1,nlon
      !PE(Ps)
      te(i,j,1,4)=0.5d0*rd*tr*ps1(i,j)*ps2(i,j)/pr/pr
    end do
  end do
  return
end subroutine calc_tegrd2
!======================================================================
! calculate moist total energy profile from 2 different perturbations
!======================================================================
subroutine calc_teprof2(u1,u2,v1,v2,t1,t2,q1,q2,ps1,ps2,&
                epsq,clat,si,nlon,nlat,kmax,vwgt,te)
  implicit none
  integer, intent(in) :: nlon, nlat, kmax ! boundaries
  real(kind=dp), intent(in) :: u1(:,:,:),v1(:,:,:),t1(:,:,:),q1(:,:,:)
  real(kind=dp), intent(in) :: ps1(:,:)
  ...
  do k=1,kmax
    ...
    do j=1,nlat
      ...
      do i=1,nlon
        !KE
        te(k,1)=te(k,1)+(u1(i,j,k)*u2(i,j,k)+v1(i,j,k)*v2(i,j,k))*coef
        !PE(T)
        te(k,2)=te(k,2)+cp/tr*t1(i,j,k)*t2(i,j,k)*coef
        !LE
        te(k,3)=te(k,3)+epsq*lh**2/cp/tr*q1(i,j,k)*q2(i,j,k)*coef
      end do
    end do
  end do
  do j=1,nlat
    ...
    do i=1,nlon
      !PE(Ps)
      te(1,4)=te(1,4)+rd*tr*ps1(i,j)*ps2(i,j)/pr/pr*coef
      area=area+coef
    end do
  end do
  do i=1,4
